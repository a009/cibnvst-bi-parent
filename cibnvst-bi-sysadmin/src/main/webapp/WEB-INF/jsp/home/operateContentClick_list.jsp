	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">运营统计 > 运营页内容点击 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/operateContentClick/findOperateContentClick.action" id="listForm" method="post">
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<select class="list-input1" id="vst_occ_type" name="vst_occ_type">
											...
											<li>
												...
												<select class="list-input1" id="vst_occ_cid" name="vst_occ_cid">
													<option value="">请选择</option>
											...
											<li>
												<span class="sertitle">影片ID：</span>
												<input class="list-input1" type="text" id="vst_occ_uuid" name="vst_occ_uuid" />
											</li>
											<li>
												<span class="sertitle">名称：</span>
												<input class="list-input1" type="text" id="vst_occ_name" name="vst_occ_name" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//文本框，按回车触发查询
		$("#vst_occ_uuid").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_occ_name").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		// 参数&符替换
		var vst_occ_name = $("#vst_occ_name").val().replace("&","<@_@>");
		
		var params = "startDay=" + startDay;
		params += "&endDay=" + endDay;
		params += "&pkgName=" + $("#pkgName").val();
		params += "&vst_occ_type=" + $("#vst_occ_type").val();
		params += "&vst_occ_cid=" + $("#vst_occ_cid").val();
		params += "&vst_occ_uuid=" + $("#vst_occ_uuid").val();
		// 参数特殊化处理
		params += "&" + paramFormat("vst_occ_name", vst_occ_name);

		var ajaxUrl;
		if($("#vst_occ_uuid").val() == '' && $("#vst_occ_name").val() == ''){
			ajaxUrl = "${ctx}/report/json?code=operate_content_click_bar";
		}else{
			ajaxUrl = "${ctx}/report/json?code=operate_content_click_line";
		}
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_occ_vv[i] = n.vst_occ_vv;
			vst_occ_uv[i] = n.vst_occ_uv;
			vst_occ_avg[i] = n.vst_occ_avg;
		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=operate_content_click_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_occ_date' id='vst_occ_date2' order='desc'>日期</a></th>";
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_occ_type' id='vst_occ_type2' order='desc' style='color:orange;'>类型</a></th>";
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_occ_cid' id='vst_occ_cid2' order='desc'>影片分类</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_occ_uuid' id='vst_occ_uuid2' order='desc'>影片ID</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_occ_name' id='vst_occ_name2' order='desc'>名称</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_occ_vv' id='vst_occ_vv2' order='desc'>点击量</a> | <a onclick='changeOrder(this)' orderBy='vst_occ_vv_dod' id='vst_occ_vv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_occ_vv_wow' id='vst_occ_vv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_occ_uv' id='vst_occ_uv2' order='desc'>独立用户</a> | <a onclick='changeOrder(this)' orderBy='vst_occ_uv_dod' id='vst_occ_uv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_occ_uv_wow' id='vst_occ_uv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_occ_avg' id='vst_occ_avg2' order='desc'>人均点击量</a></th>";
		
		table += "</tr></thead><tbody>";
		
		$.each(data,function(i,n){
			var row = i%2+1;
			var oecVvDodColor = "green";
			var oecVvWowColor = "green";
			var oecUvDodColor = "green";
			var oecUvWowColor = "green";
			
			if(parseFloat(n.vst_occ_vv_dod)<0){
				oecVvDodColor = "red";
			}
			if(parseFloat(n.vst_occ_vv_wow)<0){
				oecVvWowColor = "red";
			}
			if(parseFloat(n.vst_occ_uv_dod)<0){
				oecUvDodColor = "red";
			}
			if(parseFloat(n.vst_occ_uv_wow)<0){
				oecUvWowColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_occ_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_occ_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_occ_creator)
						+"<br>修改时间："+formatDate(n.vst_occ_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_occ_updator)
						+"<br>备注："+toString(n.vst_occ_summary)+"' class='enlargeTextTitle'>"
					+getMapValue('${types}', n.vst_occ_type)+"</td>";
			table += "<td>"+getMapValue('${cids}', n.vst_occ_cid)+"</td>";
			table += "<td>"+toString(n.vst_occ_uuid)+"</td>";
			table += "<td>"+toString(n.vst_occ_name)+"</td>";
			//点击量
			table += "<td>"+formatNumber(n.vst_occ_vv)+" | ";
			table += "<span class='"+oecVvDodColor+"'>"+toString(n.vst_occ_vv_dod)+"</span> | ";
			table += "<span class='"+oecVvWowColor+"'>"+toString(n.vst_occ_vv_wow)+"</span></td>";
			//独立用户
			table += "<td>"+formatNumber(n.vst_occ_uv)+" | ";
			table += "<span class='"+oecUvDodColor+"'>"+toString(n.vst_occ_uv_dod)+"</span> | ";
			table += "<span class='"+oecUvWowColor+"'>"+toString(n.vst_occ_uv_wow)+"</span></td>";
			//人均点击量
			table += "<td>"+toString(n.vst_occ_avg)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&vst_occ_cid=" + $("#vst_occ_cid").val();
					params += "&vst_occ_uuid=" + $("#vst_occ_uuid").val();
					params += "&vst_occ_name=" + encodeURI(encodeURI(vst_occ_name));
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					params += "&moduleId=" + $("#moduleId").val();
					
					/*location.href = "${ctx}/operateContentClick/exportData.action?" + params
					...
					$.ajax({
						...
						async : true,
						cache : false,
						url : "${ctx}/operateContentClick/exportData.action",
						dataType : "json",
