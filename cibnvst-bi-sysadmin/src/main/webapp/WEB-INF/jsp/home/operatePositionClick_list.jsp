	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">运营统计 > 运营页位置点击 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/operatePositionClick/findOperatePositionClick.action" id="listForm" method="post">
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<select class="list-input1" id="vst_opc_type" name="vst_opc_type">
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_opc_pos" name="vst_opc_pos" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//文本框，按回车触发查询
		$("#vst_opc_pos").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		params += "&vst_opc_type=" + $("#vst_opc_type").val();
		params += "&vst_opc_pos=" + $("#vst_opc_pos").val();

		var ajaxUrl;
		if($("#vst_opc_pos").val() == ''){
			ajaxUrl = "${ctx}/report/json?code=operate_position_click_bar";
		}else{
			ajaxUrl = "${ctx}/report/json?code=operate_position_click_line";
		}
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_opc_vv[i] = n.vst_opc_vv;
			vst_opc_uv[i] = n.vst_opc_uv;
			vst_opc_avg[i] = n.vst_opc_avg;
		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=operate_position_click_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_opc_date' id='vst_opc_date2' order='desc'>日期</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_opc_type' id='vst_opc_type2' order='desc' style='color:orange;'>类型</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_opc_pos' id='vst_opc_pos2' order='desc'>位置编号</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_opc_vv' id='vst_opc_vv2' order='desc'>点击量</a> | <a onclick='changeOrder(this)' orderBy='vst_opc_vv_dod' id='vst_opc_vv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_opc_vv_wow' id='vst_opc_vv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_opc_uv' id='vst_opc_uv2' order='desc'>独立用户</a> | <a onclick='changeOrder(this)' orderBy='vst_opc_uv_dod' id='vst_opc_uv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_opc_uv_wow' id='vst_opc_uv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_opc_avg' id='vst_opc_avg2' order='desc'>人均点击量</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_opc_vv_dod)<0){
				hecVvDodColor = "red";
			}
			if(parseFloat(n.vst_opc_vv_wow)<0){
				hecVvWowColor = "red";
			}
			if(parseFloat(n.vst_opc_uv_dod)<0){
				hecUvDodColor = "red";
			}
			if(parseFloat(n.vst_opc_uv_wow)<0){
				hecUvWowColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_opc_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_opc_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_opc_creator)
						+"<br>修改时间："+formatDate(n.vst_opc_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_opc_updator)
						+"<br>备注："+toString(n.vst_opc_summary)+"' class='enlargeTextTitle'>"
					+getMapValue('${types}', n.vst_opc_type)+"</td>";
			table += "<td>"+toString(n.vst_opc_pos)+"</td>";
			//点击量
			table += "<td>"+formatNumber(n.vst_opc_vv)+" | ";
			table += "<span class='"+hecVvDodColor+"'>"+toString(n.vst_opc_vv_dod)+"</span> | ";
			table += "<span class='"+hecVvWowColor+"'>"+toString(n.vst_opc_vv_wow)+"</span></td>";
			//独立用户
			table += "<td>"+formatNumber(n.vst_opc_uv)+" | ";
			table += "<span class='"+hecUvDodColor+"'>"+toString(n.vst_opc_uv_dod)+"</span> | ";
			table += "<span class='"+hecUvWowColor+"'>"+toString(n.vst_opc_uv_wow)+"</span></td>";
			//人均点击量
			table += "<td>"+toString(n.vst_opc_avg)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					/*location.href = "${ctx}/operatePositionClick/exportData.action?" + params
					...
					$.ajax({
						...
						async : true,
						cache : false,
						url : "${ctx}/operatePositionClick/exportData.action",
						dataType : "json",
