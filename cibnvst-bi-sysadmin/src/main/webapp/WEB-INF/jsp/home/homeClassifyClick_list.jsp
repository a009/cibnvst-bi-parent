	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">运营统计 > 首页分类点击 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/homeClassifyClick/findHomeClassifyClick.action" id="listForm" method="post">
								...
								<div class="box-body">
									<div class="box-search">
										<c:choose>
											...
											<li>
												<span class="sertitle">入口类别：</span>
												<input class="list-input1" type="text" id="vst_hcc_type" name="vst_hcc_type" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//文本框，按回车触发查询
		$("#vst_hcc_type").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		// 参数&符替换
		var vst_hcc_type = $("#vst_hcc_type").val().replace("&","<@_@>");
		...
		// 参数特殊化处理
		params += "&" + paramFormat("vst_hcc_type", vst_hcc_type);

		var ajaxUrl;
		if($("#vst_hcc_type").val() == ''){
			ajaxUrl = "${ctx}/report/json?code=home_classify_click_bar";
		}else{
			ajaxUrl = "${ctx}/report/json?code=home_classify_click_line";
		}
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_hcc_amount[i] = n.vst_hcc_amount;
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=home_classify_click_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_hcc_date' id='vst_hcc_date2' order='desc'>日期</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_hcc_cid' id='vst_hcc_cid2' order='desc' style='color:orange;'>影片分类</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_hcc_type' id='vst_hcc_type2' order='desc'>入口类别</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_hcc_amount' id='vst_hcc_amount2' order='desc'>点击量</a> | <a onclick='changeOrder(this)' orderBy='vst_hcc_amount_dod' id='vst_hcc_amount_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_hcc_amount_wow' id='vst_hcc_amount_wow2' order='desc'>周环比</a></th>";
		...
		$.each(data,function(i,n){
			var row = i%2+1;
			var hccAmountDodColor = "green";
			var hccAmountWowColor = "green";
			var hccUvDodColor = "green";
			var hccUvWowColor = "green";
			
			if(parseFloat(n.vst_hcc_amount_dod)<0){
				hccAmountDodColor = "red";
			}
			if(parseFloat(n.vst_hcc_amount_wow)<0){
				hccAmountWowColor = "red";
			}
			...
			table += "<td data-title='新增时间："+formatDate(n.vst_hcc_addtime, "yyyy-MM-dd HH:mm:ss")
					...
					+getMapValue('${classifys}', n.vst_hcc_cid)+"</td>";
			table += "<td>"+toString(n.vst_hcc_type)+"</td>";
			//点击量
			table += "<td>"+formatNumber(n.vst_hcc_amount)+" | ";
			table += "<span class='"+hccAmountDodColor+"'>"+toString(n.vst_hcc_amount_dod)+"</span> | ";
			table += "<span class='"+hccAmountWowColor+"'>"+toString(n.vst_hcc_amount_wow)+"</span></td>";
			//独立用户
			table += "<td>"+formatNumber(n.vst_hcc_uv)+" | ";
			table += "<span class='"+hccUvDodColor+"'>"+toString(n.vst_hcc_uv_dod)+"</span> | ";
			table += "<span class='"+hccUvWowColor+"'>"+toString(n.vst_hcc_uv_wow)+"</span></td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&pkgName=" + $("#pkgName").val();
					params += "&vst_hcc_cid=" + $("#vst_hcc_cid").val();
					params += "&vst_hcc_type=" + encodeURI(encodeURI(vst_hcc_type));
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					params += "&moduleId=" + $("#moduleId").val();
					
					/*location.href = "${ctx}/homeClassifyClick/exportData.action?" + params
					...
					$.ajax({
						...
						async : true,
						cache : false,
						url : "${ctx}/homeClassifyClick/exportData.action",
						dataType : "json",
