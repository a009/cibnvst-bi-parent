	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">运营统计 > 运营页入口点击 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/operateEntryClick/findOperateEntryClick.action" id="listForm" method="post">
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<select class="list-input1" id="vst_oec_type" name="vst_oec_type">
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_oec_name" name="vst_oec_name" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//文本框，按回车触发查询
		$("#vst_oec_name").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		// 参数&符替换
		var vst_oec_name = $("#vst_oec_name").val().replace("&","<@_@>");
		
		var params = "startDay=" + startDay;
		params += "&endDay=" + endDay;
		params += "&pkgName=" + $("#pkgName").val();
		params += "&vst_oec_type=" + $("#vst_oec_type").val();
		// 参数特殊化处理
		params += "&" + paramFormat("vst_oec_name", vst_oec_name);

		var ajaxUrl;
		if($("#vst_oec_name").val() == ''){
			ajaxUrl = "${ctx}/report/json?code=operate_entry_click_bar";
		}else{
			ajaxUrl = "${ctx}/report/json?code=operate_entry_click_line";
		}
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_oec_vv[i] = n.vst_oec_vv;
			vst_oec_uv[i] = n.vst_oec_uv;
			vst_oec_avg[i] = n.vst_oec_avg;
		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=operate_entry_click_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_oec_date' id='vst_oec_date2' order='desc'>日期</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_oec_type' id='vst_oec_type2' order='desc' style='color:orange;'>类型</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_oec_name' id='vst_oec_name2' order='desc'>名称</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_oec_vv' id='vst_oec_vv2' order='desc'>点击量</a> | <a onclick='changeOrder(this)' orderBy='vst_oec_vv_dod' id='vst_oec_vv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_oec_vv_wow' id='vst_oec_vv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_oec_uv' id='vst_oec_uv2' order='desc'>独立用户</a> | <a onclick='changeOrder(this)' orderBy='vst_oec_uv_dod' id='vst_oec_uv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_oec_uv_wow' id='vst_oec_uv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_oec_avg' id='vst_oec_avg2' order='desc'>人均点击量</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_oec_vv_dod)<0){
				oecVvDodColor = "red";
			}
			if(parseFloat(n.vst_oec_vv_wow)<0){
				oecVvWowColor = "red";
			}
			if(parseFloat(n.vst_oec_uv_dod)<0){
				oecUvDodColor = "red";
			}
			if(parseFloat(n.vst_oec_uv_wow)<0){
				oecUvWowColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_oec_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_oec_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_oec_creator)
						+"<br>修改时间："+formatDate(n.vst_oec_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_oec_updator)
						+"<br>备注："+toString(n.vst_oec_summary)+"' class='enlargeTextTitle'>"
					+getMapValue('${types}', n.vst_oec_type)+"</td>";
			table += "<td>"+toString(n.vst_oec_name)+"</td>";
			//点击量
			table += "<td>"+formatNumber(n.vst_oec_vv)+" | ";
			table += "<span class='"+oecVvDodColor+"'>"+toString(n.vst_oec_vv_dod)+"</span> | ";
			table += "<span class='"+oecVvWowColor+"'>"+toString(n.vst_oec_vv_wow)+"</span></td>";
			//独立用户
			table += "<td>"+formatNumber(n.vst_oec_uv)+" | ";
			table += "<span class='"+oecUvDodColor+"'>"+toString(n.vst_oec_uv_dod)+"</span> | ";
			table += "<span class='"+oecUvWowColor+"'>"+toString(n.vst_oec_uv_wow)+"</span></td>";
			//人均点击量
			table += "<td>"+toString(n.vst_oec_avg)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&pkgName=" + $("#pkgName").val();
					params += "&vst_oec_type=" + $("#vst_oec_type").val();
					params += "&vst_oec_name=" + encodeURI(encodeURI(vst_oec_name));
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					params += "&moduleId=" + $("#moduleId").val();
					
					/*location.href = "${ctx}/operateEntryClick/exportData.action?" + params
					...
					$.ajax({
						...
						async : true,
						cache : false,
						url : "${ctx}/operateEntryClick/exportData.action",
						dataType : "json",
