	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">运营统计 > 首页位置点击 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/homePositionClick/findHomePositionClick.action" id="listForm" method="post">
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<select class="list-input1" id="vst_hpc_type" name="vst_hpc_type">
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_hpc_pos" name="vst_hpc_pos" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//文本框，按回车触发查询
		$("#vst_hpc_pos").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		params += "&vst_hpc_type=" + $("#vst_hpc_type").val();
		params += "&vst_hpc_pos=" + $("#vst_hpc_pos").val();

		var ajaxUrl;
		if($("#vst_hpc_pos").val() == ''){
			ajaxUrl = "${ctx}/report/json?code=home_position_click_bar";
		}else{
			ajaxUrl = "${ctx}/report/json?code=home_position_click_line";
		}
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_hpc_vv[i] = n.vst_hpc_vv;
			vst_hpc_uv[i] = n.vst_hpc_uv;
			vst_hpc_avg[i] = n.vst_hpc_avg;
		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=home_position_click_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_hpc_date' id='vst_hpc_date2' order='desc'>日期</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_hpc_type' id='vst_hpc_type2' order='desc' style='color:orange;'>类型</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_hpc_pos' id='vst_hpc_pos2' order='desc'>位置编号</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_hpc_vv' id='vst_hpc_vv2' order='desc'>点击量</a> | <a onclick='changeOrder(this)' orderBy='vst_hpc_vv_dod' id='vst_hpc_vv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_hpc_vv_wow' id='vst_hpc_vv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_hpc_uv' id='vst_hpc_uv2' order='desc'>独立用户</a> | <a onclick='changeOrder(this)' orderBy='vst_hpc_uv_dod' id='vst_hpc_uv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_hpc_uv_wow' id='vst_hpc_uv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_hpc_avg' id='vst_hpc_avg2' order='desc'>人均点击量</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_hpc_vv_dod)<0){
				hecVvDodColor = "red";
			}
			if(parseFloat(n.vst_hpc_vv_wow)<0){
				hecVvWowColor = "red";
			}
			if(parseFloat(n.vst_hpc_uv_dod)<0){
				hecUvDodColor = "red";
			}
			if(parseFloat(n.vst_hpc_uv_wow)<0){
				hecUvWowColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_hpc_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_hpc_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_hpc_creator)
						+"<br>修改时间："+formatDate(n.vst_hpc_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_hpc_updator)
						+"<br>备注："+toString(n.vst_hpc_summary)+"' class='enlargeTextTitle'>"
					+getMapValue('${types}', n.vst_hpc_type)+"</td>";
			table += "<td>"+toString(n.vst_hpc_pos)+"</td>";
			//点击量
			table += "<td>"+formatNumber(n.vst_hpc_vv)+" | ";
			table += "<span class='"+hecVvDodColor+"'>"+toString(n.vst_hpc_vv_dod)+"</span> | ";
			table += "<span class='"+hecVvWowColor+"'>"+toString(n.vst_hpc_vv_wow)+"</span></td>";
			//独立用户
			table += "<td>"+formatNumber(n.vst_hpc_uv)+" | ";
			table += "<span class='"+hecUvDodColor+"'>"+toString(n.vst_hpc_uv_dod)+"</span> | ";
			table += "<span class='"+hecUvWowColor+"'>"+toString(n.vst_hpc_uv_wow)+"</span></td>";
			//人均点击量
			table += "<td>"+toString(n.vst_hpc_avg)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					/*location.href = "${ctx}/homePositionClick/exportData.action?" + params
					...
					$.ajax({
						...
						async : true,
						cache : false,
						url : "${ctx}/homePositionClick/exportData.action",
						dataType : "json",
