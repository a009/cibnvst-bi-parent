	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">运营统计 > 首页内容点击 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/homeContentClick/findHomeContentClick.action" id="listForm" method="post">
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<select class="list-input1" id="vst_hcc_type" name="vst_hcc_type">
											...
											<li>
												...
												<select class="list-input1" id="vst_hcc_cid" name="vst_hcc_cid">
													<option value="">请选择</option>
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_hcc_uuid" name="vst_hcc_uuid" />
											</li>
											<li>
												<span class="sertitle">名称：</span>
												<input class="list-input1" type="text" id="vst_hcc_name" name="vst_hcc_name" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//文本框，按回车触发查询
		$("#vst_hcc_uuid").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_hcc_name").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		// 参数&符替换
		var vst_hcc_name = $("#vst_hcc_name").val().replace("&","<@_@>");
		
		var params = "startDay=" + startDay;
		params += "&endDay=" + endDay;
		params += "&pkgName=" + $("#pkgName").val();
		params += "&vst_hcc_type=" + $("#vst_hcc_type").val();
		params += "&vst_hcc_cid=" + $("#vst_hcc_cid").val();
		params += "&vst_hcc_uuid=" + $("#vst_hcc_uuid").val();
		// 参数特殊化处理
		params += "&" + paramFormat("vst_hcc_name", vst_hcc_name);

		var ajaxUrl;
		if($("#vst_hcc_uuid").val() == '' && $("#vst_hcc_name").val() == ''){
			ajaxUrl = "${ctx}/report/json?code=home_content_click_bar";
		}else{
			ajaxUrl = "${ctx}/report/json?code=home_content_click_line";
		}
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_hcc_vv[i] = n.vst_hcc_vv;
			vst_hcc_uv[i] = n.vst_hcc_uv;
			vst_hcc_avg[i] = n.vst_hcc_avg;
		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=home_content_click_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_hcc_date' id='vst_hcc_date2' order='desc'>日期</a></th>";
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_hcc_type' id='vst_hcc_type2' order='desc' style='color:orange;'>类型</a></th>";
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_hcc_cid' id='vst_hcc_cid2' order='desc'>影片分类</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_hcc_uuid' id='vst_hcc_uuid2' order='desc'>影片ID</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_hcc_name' id='vst_hcc_name2' order='desc'>名称</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_hcc_vv' id='vst_hcc_vv2' order='desc'>点击量</a> | <a onclick='changeOrder(this)' orderBy='vst_hcc_vv_dod' id='vst_hcc_vv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_hcc_vv_wow' id='vst_hcc_vv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_hcc_uv' id='vst_hcc_uv2' order='desc'>独立用户</a> | <a onclick='changeOrder(this)' orderBy='vst_hcc_uv_dod' id='vst_hcc_uv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_hcc_uv_wow' id='vst_hcc_uv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_hcc_avg' id='vst_hcc_avg2' order='desc'>人均点击量</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_hcc_vv_dod)<0){
				hecVvDodColor = "red";
			}
			if(parseFloat(n.vst_hcc_vv_wow)<0){
				hecVvWowColor = "red";
			}
			if(parseFloat(n.vst_hcc_uv_dod)<0){
				hecUvDodColor = "red";
			}
			if(parseFloat(n.vst_hcc_uv_wow)<0){
				hecUvWowColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_hcc_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_hcc_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_hcc_creator)
						+"<br>修改时间："+formatDate(n.vst_hcc_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_hcc_updator)
						+"<br>备注："+toString(n.vst_hcc_summary)+"' class='enlargeTextTitle'>"
					+getMapValue('${types}', n.vst_hcc_type)+"</td>";
			table += "<td>"+getMapValue('${cids}', n.vst_hcc_cid)+"</td>";
			table += "<td>"+toString(n.vst_hcc_uuid)+"</td>";
			table += "<td>"+toString(n.vst_hcc_name)+"</td>";
			//点击量
			table += "<td>"+formatNumber(n.vst_hcc_vv)+" | ";
			table += "<span class='"+hecVvDodColor+"'>"+toString(n.vst_hcc_vv_dod)+"</span> | ";
			table += "<span class='"+hecVvWowColor+"'>"+toString(n.vst_hcc_vv_wow)+"</span></td>";
			//独立用户
			table += "<td>"+formatNumber(n.vst_hcc_uv)+" | ";
			table += "<span class='"+hecUvDodColor+"'>"+toString(n.vst_hcc_uv_dod)+"</span> | ";
			table += "<span class='"+hecUvWowColor+"'>"+toString(n.vst_hcc_uv_wow)+"</span></td>";
			//人均点击量
			table += "<td>"+toString(n.vst_hcc_avg)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&vst_hcc_cid=" + $("#vst_hcc_cid").val();
					params += "&vst_hcc_uuid=" + $("#vst_hcc_uuid").val();
					params += "&vst_hcc_name=" + encodeURI(encodeURI(vst_hcc_name));
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					params += "&moduleId=" + $("#moduleId").val();
					
					/*location.href = "${ctx}/homeContentClick/exportData.action?" + params
					...
					$.ajax({
						...
						async : true,
						cache : false,
						url : "${ctx}/homeContentClick/exportData.action",
						dataType : "json",
