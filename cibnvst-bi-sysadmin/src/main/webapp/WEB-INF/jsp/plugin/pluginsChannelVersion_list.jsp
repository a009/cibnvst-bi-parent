	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品监控 > 插件分布(渠道)统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/pluginsChannelVersion/findPluginsChannelVersion.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 产品监控 > 插件分布(渠道)统计
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_pcv_channel" name="vst_pcv_channel" />
											</li>
											<li>
												<span class="sertitle">版本：</span>
												<input class="list-input1" type="text" id="vst_pcv_version" name="vst_pcv_version" />
											</li>
											<li>
												<span class="sertitle">插件包：</span>
												<input class="list-input1" type="text" id="vst_pcv_plugin_pkg" name="vst_pcv_plugin_pkg" />
											</li>
											<li>
												<span class="sertitle">插件版本：</span>
												<input class="list-input1" type="text" id="vst_pcv_plugin_version" name="vst_pcv_plugin_version" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//文本框，按回车触发查询
		$("#vst_pcv_channel").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_pcv_version").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_pcv_plugin_pkg").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_pcv_plugin_version").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		// 参数特殊化处理
		params += "&" + paramFormat("vst_pcv_channel", $("#vst_pcv_channel").val());
		params += "&" + paramFormat("vst_pcv_version", $("#vst_pcv_version").val());
		params += "&" + paramFormat("vst_pcv_plugin_pkg", $("#vst_pcv_plugin_pkg").val());
		params += "&vst_pcv_plugin_version=" + $("#vst_pcv_plugin_version").val();

		var ajaxUrl;
		if(startDay == endDay){
			// 饼状图
			if($("#vst_pcv_channel").val() == ''){
				ajaxUrl = "${ctx}/report/json?code=plugins_channel_version_pieByChannel";
			}else if($("#vst_pcv_version").val() == ''){
				ajaxUrl = "${ctx}/report/json?code=plugins_channel_version_pieByVersion";
			}else if($("#vst_pcv_plugin_pkg").val() == ''){
				ajaxUrl = "${ctx}/report/json?code=plugins_channel_version_pieByPluginPkg";
			}else if($("#vst_pcv_plugin_version").val() == ''){
				ajaxUrl = "${ctx}/report/json?code=plugins_channel_version_pieByPluginVersion";
			}else{
				// 默认折线图
				ajaxUrl = "${ctx}/report/json?code=plugins_channel_version_line";
			}
		}else{
			// 百分比折线图
			if($("#vst_pcv_channel").val() == ''){
				ajaxUrl = "${ctx}/pluginsChannelVersion/ajaxGetReportByChannel.action";
			}else if($("#vst_pcv_version").val() == ''){
				ajaxUrl = "${ctx}/pluginsChannelVersion/ajaxGetReportByVersion.action";
			}else if($("#vst_pcv_plugin_pkg").val() == ''){
				ajaxUrl = "${ctx}/pluginsChannelVersion/ajaxGetReportByPluginPkg.action";
			}else if($("#vst_pcv_plugin_version").val() == ''){
				ajaxUrl = "${ctx}/pluginsChannelVersion/ajaxGetReportByPluginVersion.action";
			}else{
				// 默认折线图
				ajaxUrl = "${ctx}/report/json?code=plugins_channel_version_line";
			}
		...
		$.ajax({
			...
			success : function(msg){
				...
				if(msg.code == 100){
					if($("#vst_pcv_channel").val() == '' || $("#vst_pcv_version").val() == '' || $("#vst_pcv_plugin_pkg").val() == '' || $("#vst_pcv_plugin_version").val() == ''){
						if(startDay == endDay){
							pie(msg);
						}else{
							linePercent(msg);
	...
	function pie(msg){
		...
		if(data.length > 20){
   		...
   		$.each(top20data, function(i,n){
   			series +="{name:\""+n["value"]+"："+n.vst_pcv_uv+"\",y:"+n.vst_pcv_uv+"},";
   		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=plugins_channel_version_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_pcv_date' id='vst_pcv_date2' order='desc'>日期</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_pcv_channel' id='vst_pcv_channel2' order='desc' style='color:orange;'>渠道</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_pcv_version' id='vst_pcv_version2' order='desc'>版本</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_pcv_plugin_pkg' id='vst_pcv_plugin_pkg2' order='desc'>插件包</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_pcv_plugin_version' id='vst_pcv_plugin_version2' order='desc'>插件版本</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_pcv_uv' id='vst_pcv_uv2' order='desc'>用户数</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_pcv_dod' id='vst_pcv_dod2' order='desc'>天环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_pcv_wow' id='vst_pcv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_pcv_mom' id='vst_pcv_mom2' order='desc'>月环比</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_pcv_dod)<0){
				dodColor = "red";
			}
			if(parseFloat(n.vst_pcv_wow)<0){
				wowColor = "red";
			}
			if(parseFloat(n.vst_pcv_mom)<0){
				momColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_pcv_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_pcv_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_pcv_creator)
						+"<br>修改时间："+formatDate(n.vst_pcv_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_pcv_updator)
						+"<br>备注："+toString(n.vst_pcv_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_pcv_channel)+"</td>";
			table += "<td>"+toString(n.vst_pcv_version)+"</td>";
			table += "<td>"+toString(n.vst_pcv_plugin_pkg)+"</td>";
			table += "<td>"+toString(n.vst_pcv_plugin_version)+"</td>";
			//用户数
			table += "<td>"+formatNumber(n.vst_pcv_uv)+"</td>";
			table += "<td class='"+dodColor+"'>"+toString(n.vst_pcv_dod)+"</td>";
			table += "<td class='"+wowColor+"'>"+toString(n.vst_pcv_wow)+"</td>";
			table += "<td class='"+momColor+"'>"+toString(n.vst_pcv_mom)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&pkgName=" + $("#pkgName").val();
					params += "&vst_pcv_channel=" + encodeURI(encodeURI($("#vst_pcv_channel").val()));
					params += "&vst_pcv_version=" + encodeURI(encodeURI($("#vst_pcv_version").val()));
					params += "&vst_pcv_plugin_pkg=" + encodeURI(encodeURI($("#vst_pcv_plugin_pkg").val()));
					params += "&vst_pcv_plugin_version=" + encodeURI(encodeURI($("#vst_pcv_plugin_version").val()));
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					
					location.href = "${ctx}/pluginsChannelVersion/exportData.action?" + params
