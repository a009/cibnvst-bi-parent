	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
			...
			<div class="content-roll">
				<form action="${ctx}/topicPlayRank/findTopicPlayRank.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 排行统计 > 专题播放统计</span>
					</section>
...
<script type="text/javascript">
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/topicPlayRank/ajaxGetCutPage.action",
			dataType:"json",
	...
	function showTable(msg){
		...
		table += "<th width='10%'>分类</th>";
		table += "<th width='10%'>模版类型</th>";
		table += "<th width='10%'>专题ID</th>";
		table += "<th width='20%'>专题名称</th>";
		table += "<th width='10%'>总播放量</th>";
		table += "<th width='10%'>总播放时长(小时)</th>";
		...
		$.each(data,function(i,n){
			...
			table += "<td>"+getMapValue('${templates}', n.vst_tp_template)+"</td>";
			table += "<td>"+toString(n.vst_tp_topicId)+"</td>";
			table += "<td title='"+n.vst_tp_name+"'>"+toString(n.vst_tp_name)+"</td>";
			table += "<td>"+formatNumber(n.vst_tp_vv)+"</td>";
	...
	function button_export(){
		...
		if(!isNotEmpty(startDay) || !isNotEmpty(endDay) || getDateDiff(startDay, endDay, 'day') > 92){
			...
			location.href = "${ctx}/topicPlayRank/exportData.action?" + params
