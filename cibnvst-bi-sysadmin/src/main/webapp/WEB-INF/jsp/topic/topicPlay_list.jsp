	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">运营统计 > 专题播放统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/topicPlay/findTopicPlay.action" id="listForm" method="post">
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<select class="list-input1" id="vst_tp_special_type" name="vst_tp_special_type">
											...
											<li>
												...
												<select class="list-input1" id="vst_tp_cid" name="vst_tp_cid">
													<option value="">请选择</option>
											...
											<li>
												...
												<select class="list-input1" id="vst_tp_template" name="vst_tp_template">
													<option value="">请选择</option>
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_tp_topicId" name="vst_tp_topicId" />
											</li>
											<li>
												<span class="sertitle">专题名称：</span>
												<input class="list-input1" type="text" id="vst_tp_name" name="vst_tp_name" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//文本框，按回车触发查询
		$("#vst_tp_topicId").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_tp_name").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		params += "&vst_tp_special_type=" + $("#vst_tp_special_type").val();
		params += "&vst_tp_cid=" + $("#vst_tp_cid").val();
		params += "&vst_tp_template=" + $("#vst_tp_template").val();
		params += "&vst_tp_topicId=" + $("#vst_tp_topicId").val();
		// 参数特殊化处理
		params += "&" + paramFormat("vst_tp_name", $("#vst_tp_name").val());
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=topic_play_line",
			dataType:"json",
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_tp_vv[i] = n.vst_tp_vv;
			vst_tp_playtime[i] = toHour(n.vst_tp_playtime*1000);
		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=topic_play_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_tp_date' id='vst_tp_date2' order='desc'>日期</a></th>";
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_tp_special_type' id='vst_tp_special_type2' order='desc'>专区类型</a></th>";
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_tp_cid' id='vst_tp_cid2' order='desc'>分类</a></th>";
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_tp_template' id='vst_tp_template2' order='desc'>模版类型</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_tp_topicId' id='vst_tp_topicId2' order='desc' style='color:orange;'>专题ID</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_tp_name' id='vst_tp_name2' order='desc'>专题名称</a></th>";
		
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_tp_vv' id='vst_tp_vv2' order='desc'>播放量</a> | <a onclick='changeOrder(this)' orderBy='vst_tp_vv_dod' id='vst_tp_vv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_tp_vv_wow' id='vst_tp_vv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_tp_uv' id='vst_tp_uv2' order='desc'>播放独立用户</a> | <a onclick='changeOrder(this)' orderBy='vst_tp_uv_dod' id='vst_tp_uv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_tp_uv_wow' id='vst_tp_uv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_tp_avg' id='vst_tp_avg2' order='desc'>人均播放量</a></th>";
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_tp_playtime' id='vst_tp_playtime2' order='desc'>播放时长(小时)</a></th>";
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_tp_playtime_avg' id='vst_tp_playtime_avg2' order='desc'>人均播放时长(分钟)</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_tp_vv_dod)<0){
				vvDodColor = "red";
			}
			if(parseFloat(n.vst_tp_vv_wow)<0){
				vvWowColor = "red";
			}
			if(parseFloat(n.vst_tp_uv_dod)<0){
				uvDodColor = "red";
			}
			if(parseFloat(n.vst_tp_uv_wow)<0){
				uvWowColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_tp_date+'', 'yyyyMMdd')+"</td>";
			table += "<td>"+getMapValue('${specialTypes}', n.vst_tp_special_type)+"</td>";
			table += "<td>"+getMapValue('${classifys}', n.vst_tp_cid)+"</td>";
			table += "<td>"+toString(n.vst_tp_template)+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_tp_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_tp_creator)
						+"<br>修改时间："+formatDate(n.vst_tp_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_tp_updator)
						+"<br>备注："+toString(n.vst_tp_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_tp_topicId)+"</td>";
			table += "<td title='"+n.vst_tp_name+"'>"+formatString(n.vst_tp_name)+"</td>";
			//播放量
			table += "<td>"+formatNumber(n.vst_tp_vv)+" | ";
			table += "<span class='"+vvDodColor+"'>"+toString(n.vst_tp_vv_dod)+"</span> | ";
			table += "<span class='"+vvWowColor+"'>"+toString(n.vst_tp_vv_wow)+"</span></td>";
			//点击独立用户
			table += "<td>"+formatNumber(n.vst_tp_uv)+" | ";
			table += "<span class='"+uvDodColor+"'>"+toString(n.vst_tp_uv_dod)+"</span> | ";
			table += "<span class='"+uvWowColor+"'>"+toString(n.vst_tp_uv_wow)+"</span></td>";
			//人均播放量
			table += "<td>"+formatNumber(n.vst_tp_avg)+"</td>";
			//播放时长
			table += "<td>"+toHour(n.vst_tp_playtime*1000)+"</td>";
			table += "<td>"+toMinute(n.vst_tp_playtime_avg*1000)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&vst_tp_template=" + $("#vst_tp_template").val();
					params += "&vst_tp_topicId=" + $("#vst_tp_topicId").val();
					params += "&vst_tp_name=" + encodeURI(encodeURI($("#vst_tp_name").val()));
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					
					location.href = "${ctx}/topicPlay/exportData.action?" + params
