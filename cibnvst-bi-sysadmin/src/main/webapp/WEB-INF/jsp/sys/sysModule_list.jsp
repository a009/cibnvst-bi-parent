...
<script type="text/javascript">
  	$(function(){
  		//下拉列表框，赋值
  		$("#vst_module_state").val($("#hidden_module_state").val());
  	...
  	// 新增
	function button_add(){
		$("#listForm").attr("action", "${ctx}/sysModule/toAdd.action");
		$("#listForm").submit();
	}
	...
	function button_edit(){
	    ...
	    if(modifyChecked==""){
	        ...
	        $("#listForm").attr("action", "${ctx}/sysModule/toEdit.action");
			$("#listForm").submit();
	    }
	}
	...
	function button_delete(){
	    ...
	    if(modifyChecked==""){
	    	...
	    	if(window.confirm("您确定要删除所选的"+modifyChecked.split(',').length+"条记录吗?")){
		    	$("#recordId").val(modifyChecked);
		        $("#listForm").attr("action", "${ctx}/sysModule/deleteModule.action");
				$("#listForm").submit();
			}
	    }
	}
	...
	function button_stop(){
		...
		if(confirm("您确定要禁用所选的"+modifyChecked.split(',').length+"条记录吗？")){
			$("#recordId").val(modifyChecked);
			$("#recordState").val(2);
			$("#listForm").attr("action", "${ctx}/sysModule/modifyModuleState.action");
			$("#listForm").submit();
		}
	}
	...
	function makeIndex(){
		if($("#doIndex").attr("disabled") == undefined || $("#doIndex").attr("disabled") == ""){
			if(!checkIndexs()){
				...
				$.ajax({
					url:"${ctx}/sysModule/modifyModuleIndexs.action",
					type:"POST",
	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
			<div class="content-roll">
				<form action="${ctx}/sysModule/findModules.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 系统管理 > 系统模块查询</span>
					</section>
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<input type="hidden" id="hidden_module_state" value="${cutPage._queryParam.vst_module_state }" />
												<select class="list-input1" id="vst_module_state" name="vst_module_state">
											...
											<li>
												<span class="sertitle">父级：</span>
												<input type="hidden" id="hidden_module_parent" value="${cutPage._queryParam.vst_module_parent }" />
							  	 				<select class="list-input1" name="vst_module_parent" id="vst_module_parent">
							  	 					<option value="">请选择</option>
											...
											<li>
												<span class="sertitle">URL：</span>
												<input class="list-input1" type="text" name="vst_module_url" value="${cutPage._queryParam.vst_module_url }" />
											</li>
											<li>
												<span class="sertitle">图标：</span>
												<input class="list-input1" type="text" name="vst_module_icon" value="${cutPage._queryParam.vst_module_icon }" />
											</li>
										</ul>
										<c:choose>
						    				<c:when test="${hidden_search=='1' || hidden_search==1}">
														...
														<ul id="rowHead" style="width: 1820px; background-color:#d2d6de;">
															...
															<li style="width:150px;">
																<c:url value="../share/order.jsp" var="vst_module_id_order">
																    ...
																    <c:param name="fieldName" value="模块ID"/>
															    </c:url>
															...
															<li style="width:200px;">
																<c:url value="../share/order.jsp" var="vst_module_parent_order">
																    <c:param name="orderBy" value="vst_module_parent"/>
																    <c:param name="fieldName" value="父级"/>
															    </c:url>
															    <c:import url="${vst_module_parent_order}"/>
															</li>
															<li style="width:250px;">
																<c:url value="../share/order.jsp" var="vst_module_url_order">
																    <c:param name="orderBy" value="vst_module_url"/>
																    <c:param name="fieldName" value="URL"/>
															    </c:url>
															    <c:import url="${vst_module_url_order}"/>
															</li>
															<li style="width:150px;">
																<c:url value="../share/order.jsp" var="vst_module_icon_order">
																    <c:param name="orderBy" value="vst_module_icon"/>
																    <c:param name="fieldName" value="图标"/>
															    </c:url>
															    <c:import url="${vst_module_icon_order}"/>
															</li>
															<li style="width:100px;">
																<c:url value="../share/order.jsp" var="vst_module_index_order">
																    <c:param name="orderBy" value="vst_module_index"/>
																    <c:param name="fieldName" value="排序"/>
															    </c:url>
															    <c:import url="${vst_module_index_order}"/>
															</li>
															<li style="width:100px;">
																<c:url value="../share/order.jsp" var="vst_module_state_order">
																    <c:param name="orderBy" value="vst_module_state"/>
																    <c:param name="fieldName" value="状态"/>
															    </c:url>
															    <c:import url="${vst_module_state_order}"/>
															</li>
															<li style="width:150px;">
																<c:url value="../share/order.jsp" var="vst_module_addtime_order">
																    <c:param name="orderBy" value="vst_module_addtime"/>
																    <c:param name="fieldName" value="新增时间"/>
															    </c:url>
															    <c:import url="${vst_module_addtime_order}"/>
															</li>
															<li style="width:150px;">
																<c:url value="../share/order.jsp" var="vst_module_uptime_order">
																    <c:param name="orderBy" value="vst_module_uptime"/>
																    <c:param name="fieldName" value="修改时间"/>
															    </c:url>
															    <c:import url="${vst_module_uptime_order}"/>
															</li>
															<li style="width:150px;">
																<c:url value="../share/order.jsp" var="vst_module_operator_order">
																    <c:param name="orderBy" value="vst_module_operator"/>
																    <c:param name="fieldName" value="操作人"/>
															    </c:url>
															    <c:import url="${vst_module_operator_order}"/>
															</li>
															<li style="width:150px;">
																<c:url value="../share/order.jsp" var="vst_module_summary_order">
																    <c:param name="orderBy" value="vst_module_summary"/>
																    <c:param name="fieldName" value="备注"/>
															    </c:url>
															    <c:import url="${vst_module_summary_order}"/>
															</li>
														</ul>
													</div>
														...
														<ul style="width: 1820px;" <c:if test="${(stuts.index+1)%2!=0}">class="row1"</c:if><c:if test="${(stuts.index+1)%2==0}">class="row2"</c:if>>
															<li style="width:70px;">
																<input type="checkbox" id="recordIds" name="recordIds"
																	value="${bean.vst_module_id}"
																	state="${bean.vst_module_state}"
																	index="${bean.vst_module_index}" />
															</li>
															<li style="width:150px;">${bean.vst_module_id}</li>
															<li style="width:200px;">${bean.vst_module_name}</li>
															<li style="width:200px;">
												    			<c:choose>
												    				<c:when test="${bean.vst_module_parent == '0'}">
												    					无
												    				</c:when>
												    				<c:otherwise>
												    					[${bean.vst_module_parent}]${bean.parentName}
												    				</c:otherwise>
												    			</c:choose>
												    		</li>
												    		<li style="width:250px;" title="${bean.vst_module_url}">
												    			${bean.vst_module_url}
												    		</li>
												    		<li style="width:150px;">${bean.vst_module_icon}</li>
															<li style="width:100px;" class="myClick">${bean.vst_module_index}</li>
															<li style="width:100px;">
												    			<c:choose>
												    				<c:when test="${bean.vst_module_state == 1}">
												    					<span style="color: green;">正常</span>
												    				</c:when>
												    				<c:when test="${bean.vst_module_state == 2}">
												    					<span style="color: red;">禁用</span>
												    				</c:when>
												    				<c:otherwise>
												    					${bean.vst_module_state}
												    				</c:otherwise>
												    			</c:choose>
												    		</li>
															<li style="width:150px;">${bean.vst_module_addtime}</li>
												    		<li style="width:150px;">${bean.vst_module_uptime}</li>
												    		<li style="width:150px;">${bean.vst_module_operator}</li>
												    		<li style="width:150px;">
												    			<c:url value="../share/remark_handler.jsp" var="vst_module_summary_long">
																    <c:param name="remark" value="${bean.vst_module_summary}"/>
															    </c:url>
															    <c:import url="${vst_module_summary_long}"/>
												    		</li>
														</ul>
													</c:forEach>
													</div>
