	...
	<div class="wrapper">
		...
		<div class="content-wrapper">
			<div class="content-roll">
				<form id="listForm" action="${ctx}/sysRole/editRole.action" method="post" focus="cutPage\\._param['vst_role_name']">
					<input type="hidden" name="vst_role_id" value="${formMap.vst_role_id }" />
					<input type="hidden" id="oldRoleName" value="${formMap.vst_role_name }" />
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 系统管理 > 系统角色修改</span>
					</section>
						...
						<div id="move">
							<ul id='example2' class="line03">
								<li>
									...
									<div class="add_m">
										<div class="add_m_l">
											<input type="text" class="input" id="vst_role_name" name="vst_role_name" value="${formMap.vst_role_name }" placeholder="请输入角色名称，不可重复" />
										</div>
										<div class="add_m_r">
											<span class="red">*</span>
										</div>
									...
									<div class="add_m">
										<div class="add_m_l">
											<textarea class="input" id="vst_role_summary" name="vst_role_summary"
												style="resize: none; height: 100px;" placeholder="请输入备注">${formMap.vst_role_summary }</textarea>
										</div>
										<div class="add_m_r">
										...
										<div class="add_m_l">
											<script type="text/javascript">
												...
												<c:forEach items="${requestScope.modules}" var="p">
													var check1 = "";
													<c:forEach items="${formMap.oldPowerList}" var="list1">
														<c:if test="${list1 == p.moduleId}">check1='checked'</c:if>
													</c:forEach>
				
													d.add('${p.moduleId}',0,'${p.moduleName}','javascript:selectPowerChb(\'${p.moduleId}\');','','','','','',true,'powerList','${p.moduleId}','${p.moduleId}_0_0','0',check1,'selectPowerChb(\'${p.moduleId}\')');
													<c:forEach items="${p.moduleList}" var="c">
														var check2 = "";
														<c:forEach items="${formMap.oldPowerList}" var="list1">
															var id = '${p.moduleId}_${c.moduleId}_0';
															var currentId = '${list1}';
															if(id == currentId) check2='checked';
														</c:forEach>
														
														d.add('${c.moduleId}','${p.moduleId}', '${c.moduleName}','javascript:selectPowerChb(\'${c.moduleId}\');','','','','','',true,'powerList','${c.moduleId}','${p.moduleId}_${c.moduleId}_0','${p.moduleId}',check2,'selectPowerChb(\'${c.moduleId}\')');
														<c:forEach items="${c.buttonList}" var="button">
															var check3 = "";
															<c:forEach items="${formMap.oldPowerList}" var="list1">
																var id = '${p.moduleId}_${c.moduleId}_${button.vst_button_id}';
																var currentId = '${list1}';
																if(id == currentId) check3='checked';
															</c:forEach>
														
															d.add('${button.vst_button_id}','${c.moduleId}', '${button.vst_button_name}' ,'javascript:selectPowerChb(\'${button.vst_button_id}\');','','','','','',true,'powerList','${button.vst_button_id}','${p.moduleId}_${c.moduleId}_${button.vst_button_id}','${c.moduleId}',check3,'selectPowerChb(\'${button.vst_button_id}\')');
														</c:forEach>
													</c:forEach>
												</c:forEach>
												document.write(d);
