	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 活跃用户(版本&设备)统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userActiveVersionBdmodel/findUserActiveVersionBdmodel.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 产品统计 > 活跃用户(版本&设备)统计
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_uavb_version" name="vst_uavb_version" />
											</li>
											<li>
												<span class="sertitle">设备名称：</span>
												<input class="list-input1" type="text" id="vst_uavb_bdmodel" name="vst_uavb_bdmodel" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
        ...
        $(".pkgName-menu>li").click(function () {
      	...
      	//文本框，按回车触发查询
		$("#vst_uavb_version").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_uavb_bdmodel").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		// 参数特殊化处理
		params += "&" + paramFormat("vst_uavb_version", $("#vst_uavb_version").val());
		params += "&" + paramFormat("vst_uavb_bdmodel", $("#vst_uavb_bdmodel").val());

		var ajaxUrl;
		if($("#vst_uavb_version").val() == ''){
			ajaxUrl = "${ctx}/report/json?code=user_active_version_bdmodel_barByVersion";
		}else if($("#vst_uavb_bdmodel").val() == ''){
			if(startDay == endDay){
				ajaxUrl = "${ctx}/report/json?code=user_active_version_bdmodel_pieByBdmodel";
			}else{
				ajaxUrl = "${ctx}/userActiveVersionBdmodel/ajaxGetReportByBdmodel.action";
			}
		}else{
			ajaxUrl = "${ctx}/report/json?code=user_active_version_bdmodel_line";
		}
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_uavb_amount[i] = n.vst_uavb_amount;
			vst_uavb_week[i] = n.vst_uavb_week;
		});
	...
	function pieByBdmodel(msg){
		...
		if(data.length > 20){
   		...
   		$.each(top20data, function(i,n){
   			series +="{name:\""+n["value"]+"："+n.vst_uavb_amount+"\",y:"+n.vst_uavb_amount+"},";
   		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_active_version_bdmodel_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uavb_date' id='vst_uavb_date2' order='desc'>日期</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uavb_version' id='vst_uavb_version2' order='desc' style='color:orange;'>版本</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uavb_bdmodel' id='vst_uavb_bdmodel2' order='desc'>设备名称</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uavb_amount' id='vst_uavb_amount2' order='desc'>用户数量</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uavb_rate' id='vst_uavb_rate2' order='desc'>用户占比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uavb_dod' id='vst_uavb_dod2' order='desc'>天环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uavb_wow' id='vst_uavb_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uavb_mom' id='vst_uavb_mom2' order='desc'>月环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uavb_week' id='vst_uavb_week2' order='desc'>周活跃</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_uavb_dod)<0){
				uabDodColor = "red";
			}
			if(parseFloat(n.vst_uavb_wow)<0){
				uabWowColor = "red";
			}
			if(parseFloat(n.vst_uavb_mom)<0){
				uabMomColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_uavb_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_uavb_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_uavb_creator)
						+"<br>修改时间："+formatDate(n.vst_uavb_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_uavb_updator)
						+"<br>备注："+toString(n.vst_uavb_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_uavb_version)+"</td>";
			table += "<td>"+toString(n.vst_uavb_bdmodel)+"</td>";
			table += "<td>"+formatNumber(n.vst_uavb_amount)+"</td>";
			table += "<td>"+toString(n.vst_uavb_rate)+"</td>";
			table += "<td class='"+uabDodColor+"'>"+toString(n.vst_uavb_dod)+"</td>";
			table += "<td class='"+uabWowColor+"'>"+toString(n.vst_uavb_wow)+"</td>";
			table += "<td class='"+uabMomColor+"'>"+toString(n.vst_uavb_mom)+"</td>";
			table += "<td>"+formatNumber(n.vst_uavb_week)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&pkgName=" + $("#pkgName").val();
					params += "&vst_uavb_version=" + $("#vst_uavb_version").val();
					params += "&vst_uavb_bdmodel=" + $("#vst_uavb_bdmodel").val();
					params += "&orderBy=" + $("#orderBy").val();
					params += "&order=" + $("#order").val();
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					
					location.href = "${ctx}/userActiveVersionBdmodel/exportData.action?" + params
