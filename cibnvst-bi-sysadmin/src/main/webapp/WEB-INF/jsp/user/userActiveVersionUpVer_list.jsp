	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 活跃用户(版本&升级版本)统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userActiveVersionUpVer/findUserActiveVersionUpVer.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 产品统计 > 活跃用户(版本&升级版本)统计
								...
								<div class="box-body">
									<div class="box-search">
										<c:choose>
											...
											<li>
												<span class="sertitle">显示版本：</span>
												<input class="list-input1" type="text" id="vst_uavu_version" name="vst_uavu_version" />
											</li>
											<li>
												<span class="sertitle">升级版本：</span>
												<input class="list-input1" type="text" id="vst_uavu_upVer" name="vst_uavu_upVer" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//文本框，按回车触发查询
		$("#vst_uavu_version").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_uavu_upVer").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		// 参数特殊化处理
		params += "&" + paramFormat("vst_uavu_version", $("#vst_uavu_version").val());
		params += "&" + paramFormat("vst_uavu_upVer", $("#vst_uavu_upVer").val());

		var ajaxUrl;
		if($("#vst_uavu_version").val() == ''){
			ajaxUrl = "${ctx}/report/json?code=user_active_version_upVer_barByVersion";
		}/*else if($("#vst_uavu_upVer").val() == ''){
			ajaxUrl = "${ctx}/userActiveVersionUpVer/ajaxGetReportByUpVer.action";
		}*/else{
			ajaxUrl = "${ctx}/report/json?code=user_active_version_upVer_line";
		}
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_uavu_amount[i] = n.vst_uavu_amount;
			vst_uavu_week[i] = n.vst_uavu_week;
			vst_uavu_month[i] = n.vst_uavu_month;
		});
	...
	function lineByUpVer(msg){
		var data = msg.data;
		var upVers = data.upVers;	// Top10升级版本
		
		var xaxis = [];
		var amountArr = new Array();
		for(var i=0; i<upVers.length; i++){
		...
		var series = [];
		for(var i=0; i<upVers.length; i++){
			var json = {name : upVers[i], data : amountArr[i]};
			series.push(json);
		}
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_active_version_upVer_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uavu_date' id='vst_uavu_date2' order='desc'>日期</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uavu_version' id='vst_uavu_version2' order='desc' style='color:orange;'>显示版本</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uavu_upVer' id='vst_uavu_upVer2' order='desc'>升级版本</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_uavu_amount' id='vst_uavu_amount2' order='desc'>日活跃</a> | <a onclick='changeOrder(this)' orderBy='vst_uavu_dod' id='vst_uavu_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uavu_wow' id='vst_uavu_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uavu_mom' id='vst_uavu_mom2' order='desc'>月环比</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_uavu_week' id='vst_uavu_week2' order='desc'>周活跃</a> | <a onclick='changeOrder(this)' orderBy='vst_uavu_week_dod' id='vst_uavu_week_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uavu_week_wow' id='vst_uavu_week_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uavu_week_mom' id='vst_uavu_week_mom2' order='desc'>月环比</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_uavu_month' id='vst_uavu_month2' order='desc'>月活跃</a> | <a onclick='changeOrder(this)' orderBy='vst_uavu_month_dod' id='vst_uavu_month_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uavu_month_wow' id='vst_uavu_month_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uavu_month_mom' id='vst_uavu_month_mom2' order='desc'>月环比</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_uavu_dod)<0){
				uaDodColor = "red";
			}
			if(parseFloat(n.vst_uavu_wow)<0){
				uaWowColor = "red";
			}
			if(parseFloat(n.vst_uavu_mom)<0){
				uaMomColor = "red";
			}
			if(parseFloat(n.vst_uavu_week_dod)<0){
				uaWeekDodColor = "red";
			}
			if(parseFloat(n.vst_uavu_week_wow)<0){
				uaWeekWowColor = "red";
			}
			if(parseFloat(n.vst_uavu_week_mom)<0){
				uaWeekMomColor = "red";
			}
			if(parseFloat(n.vst_uavu_month_dod)<0){
				uaMonthDodColor = "red";
			}
			if(parseFloat(n.vst_uavu_month_wow)<0){
				uaMonthWowColor = "red";
			}
			if(parseFloat(n.vst_uavu_month_mom)<0){
				uaMonthMomColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_uavu_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_uavu_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_uavu_creator)
						+"<br>修改时间："+formatDate(n.vst_uavu_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_uavu_updator)
						+"<br>备注："+toString(n.vst_uavu_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_uavu_version)+"</td>";
			table += "<td>"+toString(n.vst_uavu_upVer)+"</td>";
			//日活跃
			table += "<td>"+formatNumber(n.vst_uavu_amount)+" | ";
			table += "<span class='"+uaDodColor+"'>"+toString(n.vst_uavu_dod)+"</span> | ";
			table += "<span class='"+uaWowColor+"'>"+toString(n.vst_uavu_wow)+"</span> | ";
			table += "<span class='"+uaMomColor+"'>"+toString(n.vst_uavu_mom)+"</span></td>";
			//周活跃
			table += "<td>"+formatNumber(n.vst_uavu_week)+" | ";
			table += "<span class='"+uaWeekDodColor+"'>"+toString(n.vst_uavu_week_dod)+"</span> | ";
			table += "<span class='"+uaWeekWowColor+"'>"+toString(n.vst_uavu_week_wow)+"</span> | ";
			table += "<span class='"+uaWeekMomColor+"'>"+toString(n.vst_uavu_week_mom)+"</span></td>";
			//月活跃
			table += "<td>"+formatNumber(n.vst_uavu_month)+" | ";
			table += "<span class='"+uaMonthDodColor+"'>"+toString(n.vst_uavu_month_dod)+"</span> | ";
			table += "<span class='"+uaMonthWowColor+"'>"+toString(n.vst_uavu_month_wow)+"</span> | ";
			table += "<span class='"+uaMonthMomColor+"'>"+toString(n.vst_uavu_month_mom)+"</span></td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&pkgName=" + $("#pkgName").val();
					params += "&vst_uavu_version=" + $("#vst_uavu_version").val();
					params += "&vst_uavu_upVer=" + $("#vst_uavu_upVer").val();
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					
					location.href = "${ctx}/userActiveVersionUpVer/exportData.action?" + params
