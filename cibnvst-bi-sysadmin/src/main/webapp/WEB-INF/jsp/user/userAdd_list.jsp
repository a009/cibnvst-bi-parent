	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size: 14px;">产品统计 > 新增用户统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userAdd/findUserAdd.action" id="listForm"
					method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
					<i class="fa fa-home"></i>
					<a href="${ctx}/sysMain/index.action">首页</a>
					<span> > 产品统计 > 新增用户统计 <i name="showTitle"
...
<script type="text/javascript">
	...
	function ajaxGetReport(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_add_line",
			dataType:"json",
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			/*if(data.length <= 31){
				// 1、日期为一个月以内，全部显示
				xaxis[index] = getDateWeek(n.date+'', 'yyyyMMdd');
				vst_ua_amount[index] = n.vst_ua_amount;
				index++;
			}else if(data.length <= 92){
				// 2、日期为三个月以内，显示每个月的1 5 10 15 20 25 30号的数据
				var day = n.date % 100;
				if(day == 1 || day % 5 == 0){
					xaxis[index] = getDateWeek(n.date+'', 'yyyyMMdd');
					vst_ua_amount[index] = n.vst_ua_amount;
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_add_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_ua_date' id='vst_ua_date2' order='desc'>日期</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_ua_amount' id='vst_ua_amount2' order='desc' style='color:orange;'>新增用户</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_ua_dod' id='vst_ua_dod2' order='desc'>天环比</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_ua_wow' id='vst_ua_wow2' order='desc'>周环比</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_ua_mom' id='vst_ua_mom2' order='desc'>月环比</a></th>";
		...
		$.each(data,function(i,n){
			...
			table += "<td>"+getDateWeek(n.vst_ua_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_ua_addtime, "yyyy-MM-dd HH:mm:ss")
					...
					+formatNumber(n.vst_ua_amount)+"</td>";
			table += "<td class='"+uaDodColor+"'>"+toString(n.vst_ua_dod)+"</td>";
			table += "<td class='"+uaWowColor+"'>"+toString(n.vst_ua_wow)+"</td>";
			table += "<td class='"+uaMomColor+"'>"+toString(n.vst_ua_mom)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					location.href = "${ctx}/userAdd/exportData.action?" + params
