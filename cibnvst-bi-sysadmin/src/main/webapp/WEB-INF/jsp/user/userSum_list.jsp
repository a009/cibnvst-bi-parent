	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 累计用户统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userSum/findUserSum.action" id="listForm" method="post">
...
<script type="text/javascript">
	...
	function ajaxGetReport(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_sum_line",
			dataType:"json",
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_us_dod[i] = n.vst_us_dod;
			vst_us_wow[i] = n.vst_us_wow;
			vst_us_mom[i] = n.vst_us_mom;
		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_sum_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_us_date' id='vst_us_date2' order='desc'>日期</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_us_amount' id='vst_us_amount2' order='desc' style='color:orange;'>累计用户</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_us_dod' id='vst_us_dod2' order='desc'>天环比</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_us_wow' id='vst_us_wow2' order='desc'>周环比</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_us_mom' id='vst_us_mom2' order='desc'>月环比</a></th>";
		...
		$.each(data,function(i,n){
			...
			table += "<td>"+getDateWeek(n.vst_us_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_us_addtime, "yyyy-MM-dd HH:mm:ss")
					...
					+formatNumber(n.vst_us_amount)+"</td>";
			table += "<td class='"+usDodColor+"'>"+toString(n.vst_us_dod)+"</td>";
			table += "<td class='"+usWowColor+"'>"+toString(n.vst_us_wow)+"</td>";
			table += "<td class='"+usMomColor+"'>"+toString(n.vst_us_mom)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					location.href = "${ctx}/userSum/exportData.action?" + params
