	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 累计用户(版本&升级版本)统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userSumVersionUpVer/findUserSumVersionUpVer.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 产品统计 > 累计用户(版本&升级版本)统计
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_usvu_version" name="vst_usvu_version" />
											</li>
											<li>
												<span class="sertitle">升级版本：</span>
												<input class="list-input1" type="text" id="vst_usvu_upVer" name="vst_usvu_upVer" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//文本框，按回车触发查询
		$("#vst_usvu_version").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_usvu_upVer").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		// 参数特殊化处理
		params += "&" + paramFormat("vst_usvu_version", $("#vst_usvu_version").val());
		params += "&" + paramFormat("vst_usvu_upVer", $("#vst_usvu_upVer").val());

		var ajaxUrl;
		if($("#vst_usvu_version").val() == ''){
			ajaxUrl = "${ctx}/report/json?code=user_sum_version_upVer_barByVersion";
		}/*else if($("#vst_usvu_upVer").val() == ''){
			ajaxUrl = "${ctx}/userSumVersionUpVer/ajaxGetReportByUpVer.action";
		}*/else{
			ajaxUrl = "${ctx}/report/json?code=user_sum_version_upVer_line";
		}
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_usvu_dod[i] = n.vst_usvu_dod;
			vst_usvu_wow[i] = n.vst_usvu_wow;
			vst_usvu_mom[i] = n.vst_usvu_mom;
		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_sum_version_upVer_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_usvu_date' id='vst_usvu_date2' order='desc'>日期</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_usvu_version' id='vst_usvu_version2' order='desc' style='color:orange;'>显示版本</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_usvu_upVer' id='vst_usvu_upVer2' order='desc'>升级版本</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_usvu_amount' id='vst_usvu_amount2' order='desc'>累计用户</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_usvu_dod' id='vst_usvu_dod2' order='desc'>天环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_usvu_wow' id='vst_usvu_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_usvu_mom' id='vst_usvu_mom2' order='desc'>月环比</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_usvu_dod)<0){
				usDodColor = "red";
			}
			if(parseFloat(n.vst_usvu_wow)<0){
				usWowColor = "red";
			}
			if(parseFloat(n.vst_usvu_mom)<0){
				usMomColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_usvu_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_usvu_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_usvu_creator)
						+"<br>修改时间："+formatDate(n.vst_usvu_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_usvu_updator)
						+"<br>备注："+toString(n.vst_usvu_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_usvu_version)+"</td>";
			table += "<td>"+toString(n.vst_usvu_upVer)+"</td>";
			table += "<td>"+formatNumber(n.vst_usvu_amount)+"</td>";
			table += "<td class='"+usDodColor+"'>"+toString(n.vst_usvu_dod)+"</td>";
			table += "<td class='"+usWowColor+"'>"+toString(n.vst_usvu_wow)+"</td>";
			table += "<td class='"+usMomColor+"'>"+toString(n.vst_usvu_mom)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&pkgName=" + $("#pkgName").val();
					params += "&vst_usvu_version=" + $("#vst_usvu_version").val();
					params += "&vst_usvu_upVer=" + $("#vst_usvu_upVer").val();
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					
					location.href = "${ctx}/userSumVersionUpVer/exportData.action?" + params
