	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size: 14px;">产品统计 > 二次活跃用户统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userSecondaryActive/findUserSecondaryActive.action" id="listForm"
					method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
					<i class="fa fa-home"></i>
					<a href="${ctx}/sysMain/index.action">首页</a>
					<span> > 产品统计 > 二次活跃用户统计 <i name="showTitle"
						class="fa fa-question-circle  enlargeTextTitle" data-title=""
						style="height: 30px; line-height: 30px; font-size: 16px;"></i> </span>
					</section>
								...
								<div class="box-body">
									<div class="box-search">
										...
										<li>
											...
											<input id="daterangepickerInputDate" type="text"
												class="list-input1" readonly="readonly">
										</li>
										<li style="display: none;">
											<span class="sertitle">起始日期：</span>
											<input type="text" class="list-input1" id="startDay"
												name="startDay" placeholder="yyyy-MM-dd"
												style="width: 150px;" maxlength="10" />
										</li>
										<li style="display: none;">
											<span class="sertitle">结束日期：</span>
											<input type="text" class="list-input1" id="endDay"
												name="endDay" placeholder="yyyy-MM-dd"
												style="width: 150px;" maxlength="10" />
										</li>
										</ul>
									</div>
									<div style="text-align: center;">
										<input type="button" class="queryBtn" value="查询"
											onclick="javascript:button_getdata();">
									</div>
								</div>
							</div>
						</div>
					...
					<div class="row">
						<div class="col-xs-12">
							...
							<div class="box box-primary">
								...
								<div id="loading_line" class="loding_img"
									style="display: none;">
								...
								<div class="box-header">
									<ul class="contlist" style="cursor: pointer;">
										<c:forEach items="${buttonList}" var="button">
											<li class="action">
												<img src="${ctx}${button.vst_button_img}" width="14"
													height="14" alt="${button.vst_button_summary }">
												<a onclick="${button.vst_button_onclick}">${button.vst_button_name
													}</a>
											</li>
										</c:forEach>
										<i name="showTitle"
											class="fa fa-question-circle  enlargeTextTitle"
											data-title=""
											style="height: 30px; line-height: 30px; font-size: 16px;"></i>
									</ul>
								</div>
								<div class="box-body">
									<div id="move_loading" class="loding_img"
										style="display: none;">
										<img class="loadImg" src='${ctx}/pub/images/loading1.gif' />
									</div>
									<div id="move">
...
<script type="text/javascript">
	...
	function ajaxGetReport(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_secondary_active_line",
			dataType:"json",
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_usa_amount[i] = n.vst_usa_amount;
		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_secondary_active_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_usa_date' id='vst_usa_date2' order='desc'>日期</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_usa_amount' id='vst_usa_amount2' order='desc' style='color:orange;'>二次活跃用户</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_usa_dod' id='vst_usa_dod2' order='desc'>天环比</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_usa_wow' id='vst_usa_wow2' order='desc'>周环比</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_usa_mom' id='vst_usa_mom2' order='desc'>月环比</a></th>";
		
		table += "</tr></thead><tbody>";
		
		$.each(data,function(i,n){
			var row = i%2+1;
			var usaDodColor = "green";
			var usaWowColor = "green";
			var usaMomColor = "green";
			
			if(parseFloat(n.vst_usa_dod)<0){
				usaDodColor = "red";
			}
			if(parseFloat(n.vst_usa_wow)<0){
				usaWowColor = "red";
			}
			if(parseFloat(n.vst_usa_mom)<0){
				usaMomColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_usa_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_usa_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_usa_creator)
						+"<br>修改时间："+formatDate(n.vst_usa_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_usa_updator)
						+"<br>备注："+toString(n.vst_usa_summary)+"' class='enlargeTextTitle'>"
					+formatNumber(n.vst_usa_amount)+"</td>";
			table += "<td class='"+usaDodColor+"'>"+toString(n.vst_usa_dod)+"</td>";
			table += "<td class='"+usaWowColor+"'>"+toString(n.vst_usa_wow)+"</td>";
			table += "<td class='"+usaMomColor+"'>"+toString(n.vst_usa_mom)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					location.href = "${ctx}/userSecondaryActive/exportData.action?" + params
