	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 活跃用户(设备)统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userActiveBdmodel/findUserActiveBdmodel.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 产品统计 > 活跃用户(设备)统计
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_uab_bdmodel" name="vst_uab_bdmodel" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
        ...
        $(".pkgName-menu>li").click(function () {
      	...
      	//文本框，按回车触发查询
		$("#vst_uab_bdmodel").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		// 参数特殊化处理
		params += "&" + paramFormat("vst_uab_bdmodel", $("#vst_uab_bdmodel").val());

		var ajaxUrl;
		if($("#vst_uab_bdmodel").val() == ''){
			ajaxUrl = "${ctx}/report/json?code=user_active_bdmodel_bar";
		}else{
			ajaxUrl = "${ctx}/report/json?code=user_active_bdmodel_line";
		}
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_uab_amount[i] = n.vst_uab_amount;
			vst_uab_week[i] = n.vst_uab_week;
		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_active_bdmodel_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uab_date' id='vst_uab_date2' order='desc'>日期</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uab_bdmodel' id='vst_uab_bdmodel2' order='desc' style='color:orange;'>设备名称</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uab_amount' id='vst_uab_amount2' order='desc'>用户数量</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uab_rate' id='vst_uab_rate2' order='desc'>用户占比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uab_dod' id='vst_uab_dod2' order='desc'>天环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uab_wow' id='vst_uab_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uab_mom' id='vst_uab_mom2' order='desc'>月环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uab_week' id='vst_uab_week2' order='desc'>周活跃</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_uab_dod)<0){
				uabDodColor = "red";
			}
			if(parseFloat(n.vst_uab_wow)<0){
				uabWowColor = "red";
			}
			if(parseFloat(n.vst_uab_mom)<0){
				uabMomColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_uab_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_uab_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_uab_creator)
						+"<br>修改时间："+formatDate(n.vst_uab_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_uab_updator)
						+"<br>备注："+toString(n.vst_uab_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_uab_bdmodel)+"</td>";
			table += "<td>"+formatNumber(n.vst_uab_amount)+"</td>";
			table += "<td>"+toString(n.vst_uab_rate)+"</td>";
			table += "<td class='"+uabDodColor+"'>"+toString(n.vst_uab_dod)+"</td>";
			table += "<td class='"+uabWowColor+"'>"+toString(n.vst_uab_wow)+"</td>";
			table += "<td class='"+uabMomColor+"'>"+toString(n.vst_uab_mom)+"</td>";
			table += "<td>"+formatNumber(n.vst_uab_week)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&pkgName=" + $("#pkgName").val();
					params += "&vst_uab_bdmodel=" + $("#vst_uab_bdmodel").val();
					params += "&orderBy=" + $("#orderBy").val();
					params += "&order=" + $("#order").val();
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					
					location.href = "${ctx}/userActiveBdmodel/exportData.action?" + params
