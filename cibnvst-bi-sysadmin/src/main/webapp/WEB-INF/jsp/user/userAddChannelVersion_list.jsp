	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 新增用户(渠道&版本)统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userAddChannelVersion/findUserAddChannelVersion.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 产品统计 > 新增用户(渠道&版本)统计
...
<script type="text/javascript">
	...
	function ajaxGetReport(){
		...
		if($("#vst_uacv_channel").val() == ''){
			ajaxUrl = "${ctx}/report/json?code=user_add_channel_version_barByChannel";
		}else if($("#vst_uacv_version").val() == ''){
			if(startDay == endDay){
				ajaxUrl = "${ctx}/report/json?code=user_add_channel_version_pieByVersion";
			}else{
				ajaxUrl = "${ctx}/userAddChannelVersion/ajaxGetReportByVersion.action";
			}
		}else{
			ajaxUrl = "${ctx}/report/json?code=user_add_channel_version_line";
		}
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_add_channel_version_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uacv_channel' id='vst_uacv_channel2' order='desc' style='color:orange;'>渠道</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uacv_version' id='vst_uacv_version2' order='desc'>版本</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uacv_amount' id='vst_uacv_amount2' order='desc'>新增用户</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uacv_dod' id='vst_uacv_dod2' order='desc'>天环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uacv_wow' id='vst_uacv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uacv_mom' id='vst_uacv_mom2' order='desc'>月环比</a></th>";
		...
		$.each(data,function(i,n){
			...
			table += "<td>"+formatNumber(n.vst_uacv_amount)+"</td>";
			table += "<td class='"+uaDodColor+"'>"+toString(n.vst_uacv_dod)+"</td>";
			table += "<td class='"+uaWowColor+"'>"+toString(n.vst_uacv_wow)+"</td>";
			table += "<td class='"+uaMomColor+"'>"+toString(n.vst_uacv_mom)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					location.href = "${ctx}/userAddChannelVersion/exportData.action?" + params
