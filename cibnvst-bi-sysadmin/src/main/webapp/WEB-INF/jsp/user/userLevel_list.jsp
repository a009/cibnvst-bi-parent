	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 季质量用户统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userLevel/findUserLevel.action" id="listForm" method="post">
...
<script type="text/javascript">
	...
	function ajaxGetReport(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_level_line",
			dataType:"json",
	...
	function line(msg){
		var data = msg.data;
		var xaxis = [];
		var vst_ul_add_amount = [];
		var vst_ul_season_amount = [];
		var vst_ul_season_dod = [];
		var vst_ul_season_wow = [];
		var vst_ul_season_radio = [];

		$.each(data,function(i,n){
			xaxis[i] = getDateWeek(n.date+'', 'yyyyMMdd');
			vst_ul_add_amount[i] = n.vst_ul_add_amount;
			vst_ul_season_amount[i] = n.vst_ul_season_amount;
			vst_ul_season_dod[i] = n.vst_ul_season_dod;
			vst_ul_season_wow[i] = n.vst_ul_season_wow;
			vst_ul_season_radio[i] = n.vst_ul_season_radio;
		});
		...
		$("#line-chart").highcharts({
			...
			{
				name : '季新增用户数',
				data : vst_ul_add_amount,
				visible : false
			},
			{
				name : '季质量用户数',
				data : vst_ul_season_amount,
			},
			{
				name : '天环比(%)',
				data : vst_ul_season_dod,
				visible : false
			},
			{
				name : '周环比(%)',
				data : vst_ul_season_wow,
				visible : false
			},
			{
				name : '占比(%)',
				data : vst_ul_season_radio,
				visible : false
			}
			],
			plotOptions: {
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_level_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_ul_date' id='vst_ul_date2' order='desc'>日期</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_ul_add_amount' id='vst_ul_add_amount2' order='desc' style='color:orange;'>季新增用户</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_ul_season_amount' id='vst_ul_season_amount2' order='desc'>季质量用户</a> | <a onclick='changeOrder(this)' orderBy='vst_ul_season_dod' id='vst_ul_season_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_ul_season_wow' id='vst_ul_season_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_ul_season_radio' id='vst_ul_season_radio2' order='desc'>占比</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_ul_season_dod)<0){
				ulSeasonDodColor = "red";
			}
			if(parseFloat(n.vst_ul_season_wow)<0){
			...
			table += "<td>"+getDateWeek(n.vst_ul_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_ul_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_ul_creator)
						+"<br>修改时间："+formatDate(n.vst_ul_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_ul_updator)
						+"<br>备注："+toString(n.vst_ul_summary)+"' class='enlargeTextTitle'>"
					+formatNumber(n.vst_ul_add_amount)+"</td>";
			
			//季质量用户
			table += "<td>"+formatNumber(n.vst_ul_season_amount)+" | ";
			table += "<span class='"+ulSeasonDodColor+"'>"+toString(n.vst_ul_season_dod)+"</span> | ";
			table += "<span class='"+ulSeasonWowColor+"'>"+toString(n.vst_ul_season_wow)+"</span></td>";
			//占比
			table += "<td>"+toString(n.vst_ul_season_radio)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					location.href = "${ctx}/userLevel/exportData.action?" + params
