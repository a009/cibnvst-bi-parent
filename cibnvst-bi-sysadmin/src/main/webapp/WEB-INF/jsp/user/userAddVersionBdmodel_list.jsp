	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 新增用户(版本&设备)统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userAddVersionBdmodel/findUserAddVersionBdmodel.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 产品统计 > 新增用户(版本&设备)统计
...
<script type="text/javascript">
	...
	function ajaxGetReport(){
		...
		if($("#vst_uavb_version").val() == ''){
			ajaxUrl = "${ctx}/report/json?code=user_add_version_bdmodel_barByVersion";
		}else if($("#vst_uavb_bdmodel").val() == ''){
			if(startDay == endDay){
				ajaxUrl = "${ctx}/report/json?code=user_add_version_bdmodel_pieByBdmodel";
			}else{
				ajaxUrl = "${ctx}/userAddVersionBdmodel/ajaxGetReportByBdmodel.action";
			}
		}else{
			ajaxUrl = "${ctx}/report/json?code=user_add_version_bdmodel_line";
		}
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_add_version_bdmodel_table",
			dataType:"json",
			success:function(msg){
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					location.href = "${ctx}/userAddVersionBdmodel/exportData.action?" + params
