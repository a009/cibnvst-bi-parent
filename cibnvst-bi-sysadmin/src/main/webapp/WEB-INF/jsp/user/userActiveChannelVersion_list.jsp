	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 活跃用户(渠道&版本)统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userActiveChannelVersion/findUserActiveChannelVersion.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 产品统计 > 活跃用户(渠道&版本)统计
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_uacv_channel" name="vst_uacv_channel" />
											</li>
											<li>
												<span class="sertitle">版本：</span>
												<input class="list-input1" type="text" id="vst_uacv_version" name="vst_uacv_version" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//文本框，按回车触发查询
		$("#vst_uacv_channel").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_uacv_version").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		// 参数特殊化处理
		params += "&" + paramFormat("vst_uacv_channel", $("#vst_uacv_channel").val());
		params += "&" + paramFormat("vst_uacv_version", $("#vst_uacv_version").val());

		var ajaxUrl;
		if($("#vst_uacv_channel").val() == ''){
			ajaxUrl = "${ctx}/report/json?code=user_active_channel_version_barByChannel";
		}else if($("#vst_uacv_version").val() == ''){
			if(startDay == endDay){
				ajaxUrl = "${ctx}/report/json?code=user_active_channel_version_pieByVersion";
			}else{
				ajaxUrl = "${ctx}/userActiveChannelVersion/ajaxGetReportByVersion.action";
			}
		}else{
			ajaxUrl = "${ctx}/report/json?code=user_active_channel_version_line";
		}
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_uacv_amount[i] = n.vst_uacv_amount;
			vst_uacv_week[i] = n.vst_uacv_week;
			vst_uacv_month[i] = n.vst_uacv_month;
		});
	...
	function pieByVersion(msg){
		...
		if(data.length > 20){
   		...
   		$.each(top20data, function(i,n){
   			series +="{name:\""+n["value"]+"："+n.vst_uacv_amount+"\",y:"+n.vst_uacv_amount+"},";
   		});
	...
	function lineByVersion(msg){
		var data = msg.data;
		var dataLength  = Object.keys(data.dateMap).length;
		var versions = data.versions;	// Top10版本
		...
		var series = [];
		for(var i=0; i<versions.length; i++){
			var json = {name : versions[i], data : amountArr[i]};
			series.push(json);
		}
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_active_channel_version_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uacv_date' id='vst_uacv_date2' order='desc'>日期</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uacv_channel' id='vst_uacv_channel2' order='desc' style='color:orange;'>渠道</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uacv_version' id='vst_uacv_version2' order='desc'>版本</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_uacv_amount' id='vst_uacv_amount2' order='desc'>日活跃</a> | <a onclick='changeOrder(this)' orderBy='vst_uacv_dod' id='vst_uacv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uacv_wow' id='vst_uacv_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uacv_mom' id='vst_uacv_mom2' order='desc'>月环比</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_uacv_week' id='vst_uacv_week2' order='desc'>周活跃</a> | <a onclick='changeOrder(this)' orderBy='vst_uacv_week_dod' id='vst_uacv_week_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uacv_week_wow' id='vst_uacv_week_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uacv_week_mom' id='vst_uacv_week_mom2' order='desc'>月环比</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_uacv_month' id='vst_uacv_month2' order='desc'>月活跃</a> | <a onclick='changeOrder(this)' orderBy='vst_uacv_month_dod' id='vst_uacv_month_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uacv_month_wow' id='vst_uacv_month_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uacv_month_mom' id='vst_uacv_month_mom2' order='desc'>月环比</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_uacv_dod)<0){
				uaDodColor = "red";
			}
			if(parseFloat(n.vst_uacv_wow)<0){
				uaWowColor = "red";
			}
			if(parseFloat(n.vst_uacv_mom)<0){
				uaMomColor = "red";
			}
			if(parseFloat(n.vst_uacv_week_dod)<0){
				uaWeekDodColor = "red";
			}
			if(parseFloat(n.vst_uacv_week_wow)<0){
				uaWeekWowColor = "red";
			}
			if(parseFloat(n.vst_uacv_week_mom)<0){
				uaWeekMomColor = "red";
			}
			if(parseFloat(n.vst_uacv_month_dod)<0){
				uaMonthDodColor = "red";
			}
			if(parseFloat(n.vst_uacv_month_wow)<0){
				uaMonthWowColor = "red";
			}
			if(parseFloat(n.vst_uacv_month_mom)<0){
				uaMonthMomColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_uacv_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_uacv_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_uacv_creator)
						+"<br>修改时间："+formatDate(n.vst_uacv_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_uacv_updator)
						+"<br>备注："+toString(n.vst_uacv_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_uacv_channel)+"</td>";
			table += "<td>"+toString(n.vst_uacv_version)+"</td>";
			//日活跃
			table += "<td>"+formatNumber(n.vst_uacv_amount)+" | ";
			table += "<span class='"+uaDodColor+"'>"+toString(n.vst_uacv_dod)+"</span> | ";
			table += "<span class='"+uaWowColor+"'>"+toString(n.vst_uacv_wow)+"</span> | ";
			table += "<span class='"+uaMomColor+"'>"+toString(n.vst_uacv_mom)+"</span></td>";
			//周活跃
			table += "<td>"+formatNumber(n.vst_uacv_week)+" | ";
			table += "<span class='"+uaWeekDodColor+"'>"+toString(n.vst_uacv_week_dod)+"</span> | ";
			table += "<span class='"+uaWeekWowColor+"'>"+toString(n.vst_uacv_week_wow)+"</span> | ";
			table += "<span class='"+uaWeekMomColor+"'>"+toString(n.vst_uacv_week_mom)+"</span></td>";
			//月活跃
			table += "<td>"+formatNumber(n.vst_uacv_month)+" | ";
			table += "<span class='"+uaMonthDodColor+"'>"+toString(n.vst_uacv_month_dod)+"</span> | ";
			table += "<span class='"+uaMonthWowColor+"'>"+toString(n.vst_uacv_month_wow)+"</span> | ";
			table += "<span class='"+uaMonthMomColor+"'>"+toString(n.vst_uacv_month_mom)+"</span></td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&pkgName=" + $("#pkgName").val();
					params += "&vst_uacv_channel=" + $("#vst_uacv_channel").val();
					params += "&vst_uacv_version=" + $("#vst_uacv_version").val();
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					
					location.href = "${ctx}/userActiveChannelVersion/exportData.action?" + params
