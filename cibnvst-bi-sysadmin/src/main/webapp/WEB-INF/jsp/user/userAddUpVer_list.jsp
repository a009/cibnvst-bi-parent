	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 新增用户(升级版本)统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userAddUpVer/findUserAddUpVer.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 产品统计 > 新增用户(升级版本)统计
...
<script type="text/javascript">
	...
	function ajaxGetReport(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_add_upVer_line",
			dataType:"json",
	...
	function ajaxGetReportByDim(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_add_upVer_dim",
			dataType:"json",
			success:function(msg){
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_add_upVer_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uau_verNum' id='vst_uau_verNum2' order='desc' style='color:orange;'>升级版本</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uau_amount' id='vst_uau_amount2' order='desc'>新增用户</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uau_dod' id='vst_uau_dod2' order='desc'>天环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uau_wow' id='vst_uau_wow2' order='desc'>周环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uau_mom' id='vst_uau_mom2' order='desc'>月环比</a></th>";
		...
		$.each(data,function(i,n){
			...
			table += "<td>"+formatNumber(n.vst_uau_amount)+"</td>";
			table += "<td class='"+uaDodColor+"'>"+toString(n.vst_uau_dod)+"</td>";
			table += "<td class='"+uaWowColor+"'>"+toString(n.vst_uau_wow)+"</td>";
			table += "<td class='"+uaMomColor+"'>"+toString(n.vst_uau_mom)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					location.href = "${ctx}/userAddUpVer/exportData.action?" + params
