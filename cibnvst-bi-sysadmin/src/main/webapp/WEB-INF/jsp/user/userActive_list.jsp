					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 活跃用户统计 > 导出</span>
							</h4>
						</div>
...
<script type="text/javascript">
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_ua_amount[i] = n.vst_ua_amount;
			vst_ua_week[i] = n.vst_ua_week;
			vst_ua_month[i] = n.vst_ua_month;
		});
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_ua_date' id='vst_ua_date2' order='desc' style='color:orange;'>日期</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_ua_amount' id='vst_ua_amount2' order='desc'>日活跃</a> | <a onclick='changeOrder(this)' orderBy='vst_ua_dod' id='vst_ua_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_ua_wow' id='vst_ua_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_ua_mom' id='vst_ua_mom2' order='desc'>月环比</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_ua_week' id='vst_ua_week2' order='desc'>周活跃</a> | <a onclick='changeOrder(this)' orderBy='vst_ua_week_dod' id='vst_ua_week_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_ua_week_wow' id='vst_ua_week_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_ua_week_mom' id='vst_ua_week_mom2' order='desc'>月环比</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_ua_month' id='vst_ua_month2' order='desc'>月活跃</a> | <a onclick='changeOrder(this)' orderBy='vst_ua_month_dod' id='vst_ua_month_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_ua_month_wow' id='vst_ua_month_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_ua_month_mom' id='vst_ua_month_mom2' order='desc'>月环比</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_ua_dod)<0){
				uaDodColor = "red";
			}
			if(parseFloat(n.vst_ua_wow)<0){
				uaWowColor = "red";
			}
			if(parseFloat(n.vst_ua_mom)<0){
				uaMomColor = "red";
			}
			if(parseFloat(n.vst_ua_week_dod)<0){
				uaWeekDodColor = "red";
			}
			if(parseFloat(n.vst_ua_week_wow)<0){
				uaWeekWowColor = "red";
			}
			if(parseFloat(n.vst_ua_week_mom)<0){
				uaWeekMomColor = "red";
			}
			if(parseFloat(n.vst_ua_month_dod)<0){
				uaMonthDodColor = "red";
			}
			if(parseFloat(n.vst_ua_month_wow)<0){
				uaMonthWowColor = "red";
			}
			if(parseFloat(n.vst_ua_month_mom)<0){
				uaMonthMomColor = "red";
			}
			...
			table += "<td data-title='新增时间："+formatDate(n.vst_ua_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_ua_creator)
						+"<br>修改时间："+formatDate(n.vst_ua_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_ua_updator)
						+"<br>备注："+toString(n.vst_ua_summary)+"' class='enlargeTextTitle'>"
					+getDateWeek(n.vst_ua_date+'', 'yyyyMMdd')+"</td>";
			//日活跃
			table += "<td>"+formatNumber(n.vst_ua_amount)+" | ";
			table += "<span class='"+uaDodColor+"'>"+toString(n.vst_ua_dod)+"</span> | ";
			table += "<span class='"+uaWowColor+"'>"+toString(n.vst_ua_wow)+"</span> | ";
			table += "<span class='"+uaMomColor+"'>"+toString(n.vst_ua_mom)+"</span></td>";
			//周活跃
			table += "<td>"+formatNumber(n.vst_ua_week)+" | ";
			table += "<span class='"+uaWeekDodColor+"'>"+toString(n.vst_ua_week_dod)+"</span> | ";
			table += "<span class='"+uaWeekWowColor+"'>"+toString(n.vst_ua_week_wow)+"</span> | ";
			table += "<span class='"+uaWeekMomColor+"'>"+toString(n.vst_ua_week_mom)+"</span></td>";
			//月活跃
			table += "<td>"+formatNumber(n.vst_ua_month)+" | ";
			table += "<span class='"+uaMonthDodColor+"'>"+toString(n.vst_ua_month_dod)+"</span> | ";
			table += "<span class='"+uaMonthWowColor+"'>"+toString(n.vst_ua_month_wow)+"</span> | ";
			table += "<span class='"+uaMonthMomColor+"'>"+toString(n.vst_ua_month_mom)+"</span></td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					location.href = "${ctx}/userActive/exportData.action?" + params
