	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 启动次数统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userStartup/findUserStartup.action" id="listForm" method="post">
...
<script type="text/javascript">
	...
	function ajaxGetReport(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_startup_line",
			dataType:"json",
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_us_amount[i] = n.vst_us_amount;
			vst_us_uv[i] = n.vst_us_uv;
			vst_us_avg[i] = n.vst_us_avg;
		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_startup_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_us_date' id='vst_us_date2' order='desc' style='color:orange;'>日期</a></th>";
		table += "<th width='25%'><a onclick='changeOrder(this)' orderBy='vst_us_amount' id='vst_us_amount2' order='desc'>启动次数</a> | <a onclick='changeOrder(this)' orderBy='vst_us_dod' id='vst_us_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_us_wow' id='vst_us_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_us_mom' id='vst_us_mom2' order='desc'>月环比</a></th>";
		table += "<th width='25%'><a onclick='changeOrder(this)' orderBy='vst_us_uv' id='vst_us_uv2' order='desc'>启动人数</a> | <a onclick='changeOrder(this)' orderBy='vst_us_uv_dod' id='vst_us_uv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_us_uv_wow' id='vst_us_uv_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_us_uv_mom' id='vst_us_uv_mom2' order='desc'>月环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_us_avg' id='vst_us_avg2' order='desc'>人均启动次数</a></th>";
		...
		$.each(data,function(i,n){
			...
			var usUvMomColor = "green";
			
			if(parseFloat(n.vst_us_dod)<0){
				usDodColor = "red";
			}
			if(parseFloat(n.vst_us_wow)<0){
				usWowColor = "red";
			}
			if(parseFloat(n.vst_us_mom)<0){
				usMomColor = "red";
			}
			if(parseFloat(n.vst_us_uv_dod)<0){
				usUvDodColor = "red";
			}
			if(parseFloat(n.vst_us_uv_wow)<0){
				usUvWowColor = "red";
			}
			if(parseFloat(n.vst_us_uv_mom)<0){
				usUvMomColor = "red";
			}
			...
			table += "<td data-title='新增时间："+formatDate(n.vst_us_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_us_creator)
						+"<br>修改时间："+formatDate(n.vst_us_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_us_updator)
						+"<br>备注："+toString(n.vst_us_summary)+"' class='enlargeTextTitle'>"
					+getDateWeek(n.vst_us_date+'', 'yyyyMMdd')+"</td>";
			//启动次数
			table += "<td>"+formatNumber(n.vst_us_amount)+" | ";
			table += "<span class='"+usDodColor+"'>"+toString(n.vst_us_dod)+"</span> | ";
			table += "<span class='"+usWowColor+"'>"+toString(n.vst_us_wow)+"</span> | ";
			table += "<span class='"+usMomColor+"'>"+toString(n.vst_us_mom)+"</span></td>";
			//启动人数
			table += "<td>"+formatNumber(n.vst_us_uv)+" | ";
			table += "<span class='"+usUvDodColor+"'>"+toString(n.vst_us_uv_dod)+"</span> | ";
			table += "<span class='"+usUvWowColor+"'>"+toString(n.vst_us_uv_wow)+"</span> | ";
			table += "<span class='"+usUvMomColor+"'>"+toString(n.vst_us_uv_mom)+"</span></td>";
			//人均启动次数
			table += "<td>"+toString(n.vst_us_avg)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					location.href = "${ctx}/userStartup/exportData.action?" + params
