	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
			...
			<div class="content-roll">
				<form action="${ctx}/userActiveProvince/findUserActiveProvince.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 产品统计 > 活跃用户(省份)统计
...
<script type="text/javascript">
	...
	function ajaxGetReport(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			// url : "${ctx}/report/json?code=user_active_province_line",
			url : "${ctx}/userActiveProvince/ajaxGetReportByProvince.action",
			dataType : "json",
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_uap_week[i] = n.vst_uap_week;
			vst_uap_month[i] = n.vst_uap_month;
		});
	...
	function lineByProvince(msg){
		var data = msg.data;
		var dataLength  = Object.keys(data.dateMap).length;
		var provinces = data.provinces;	// 获取省份
		...
		$.each(data.dateMap, function(date,amounts){
			...
			for(var i=0; i<provinces.length; i++){
				amountArr[i][index] = Number(amounts[i]);
			}
			index++;
		});
		...
		var series = [];
		for(var i=0; i<provinces.length; i++){
			var json = {name : provinces[i], data : amountArr[i]};
			series.push(json);
		}
	...
	function ajaxGetReportByDim(){
		...
		params += "&pkgName=" + $("#pkgName").val();
		// 参数特殊化处理
		if($("#vst_uap_province").val() != ""){
			var provinces = $("#vst_uap_province").val().split(",");
			if(provinces.length > 1){
				var list_provinces = "";
				for(var i=0; i<provinces.length; i++){
					list_provinces += "'" + provinces[i] + "',";
				}
				list_provinces = list_provinces.substring(0,list_provinces.length-1);
				params += "&list_provinces=" + list_provinces;
			}else{
				params += "&" + paramFormat("vst_uap_province", $("#vst_uap_province").val());
			}
		}
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url : "${ctx}/report/json?code=user_active_province_dim",
			dataType : "json",
			success : function(msg){
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url : "${ctx}/report/json?code=user_active_province_table",
			dataType : "json",
			success : function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uap_province' id='vst_uap_province2' order='desc' style='color:orange;'>省份</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_uap_amount' id='vst_uap_amount2' order='desc'>日活跃</a> | <a onclick='changeOrder(this)' orderBy='vst_uap_dod' id='vst_uap_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uap_wow' id='vst_uap_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uap_mom' id='vst_uap_mom2' order='desc'>月环比</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_uap_week' id='vst_uap_week2' order='desc'>周活跃</a> | <a onclick='changeOrder(this)' orderBy='vst_uap_week_dod' id='vst_uap_week_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uap_week_wow' id='vst_uap_week_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uap_week_mom' id='vst_uap_week_mom2' order='desc'>月环比</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_uap_month' id='vst_uap_month2' order='desc'>月活跃</a> | <a onclick='changeOrder(this)' orderBy='vst_uap_month_dod' id='vst_uap_month_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uap_month_wow' id='vst_uap_month_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uap_month_mom' id='vst_uap_month_mom2' order='desc'>月环比</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_uap_week_dod)<0){
				uaWeekDodColor = "red";
			}
			if(parseFloat(n.vst_uap_week_wow)<0){
				uaWeekWowColor = "red";
			}
			if(parseFloat(n.vst_uap_week_mom)<0){
				uaWeekMomColor = "red";
			}
			if(parseFloat(n.vst_uap_month_dod)<0){
				uaMonthDodColor = "red";
			}
			if(parseFloat(n.vst_uap_month_wow)<0){
				uaMonthWowColor = "red";
			}
			if(parseFloat(n.vst_uap_month_mom)<0){
				uaMonthMomColor = "red";
			}
			...
			//日活跃
			table += "<td>"+formatNumber(n.vst_uap_amount)+" | ";
			table += "<span class='"+uaDodColor+"'>"+toString(n.vst_uap_dod)+"</span> | ";
			table += "<span class='"+uaWowColor+"'>"+toString(n.vst_uap_wow)+"</span> | ";
			table += "<span class='"+uaMomColor+"'>"+toString(n.vst_uap_mom)+"</span></td>";
			//周活跃
			table += "<td>"+formatNumber(n.vst_uap_week)+" | ";
			table += "<span class='"+uaWeekDodColor+"'>"+toString(n.vst_uap_week_dod)+"</span> | ";
			table += "<span class='"+uaWeekWowColor+"'>"+toString(n.vst_uap_week_wow)+"</span> | ";
			table += "<span class='"+uaWeekMomColor+"'>"+toString(n.vst_uap_week_mom)+"</span></td>";
			//月活跃
			table += "<td>"+formatNumber(n.vst_uap_month)+" | ";
			table += "<span class='"+uaMonthDodColor+"'>"+toString(n.vst_uap_month_dod)+"</span> | ";
			table += "<span class='"+uaMonthWowColor+"'>"+toString(n.vst_uap_month_wow)+"</span> | ";
			table += "<span class='"+uaMonthMomColor+"'>"+toString(n.vst_uap_month_mom)+"</span></td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					location.href = "${ctx}/userActiveProvince/exportData.action?" + params
