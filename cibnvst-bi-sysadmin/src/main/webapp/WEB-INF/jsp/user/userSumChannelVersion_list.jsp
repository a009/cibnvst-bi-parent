	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 累计用户(渠道&版本)统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userSumChannelVersion/findUserSumChannelVersion.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 产品统计 > 累计用户(渠道&版本)统计
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_uscv_channel" name="vst_uscv_channel" />
											</li>
											<li>
												<span class="sertitle">版本：</span>
												<input class="list-input1" type="text" id="vst_uscv_version" name="vst_uscv_version" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//文本框，按回车触发查询
		$("#vst_uscv_channel").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_uscv_version").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		// 参数特殊化处理
		params += "&" + paramFormat("vst_uscv_channel", $("#vst_uscv_channel").val());
		params += "&" + paramFormat("vst_uscv_version", $("#vst_uscv_version").val());

		var ajaxUrl;
		if($("#vst_uscv_channel").val() == ''){
			ajaxUrl = "${ctx}/report/json?code=user_sum_channel_version_barByChannel";
		}else if($("#vst_uscv_version").val() == ''){
			if(startDay == endDay){
				ajaxUrl = "${ctx}/report/json?code=user_sum_channel_version_pieByVersion";
			}else{
				ajaxUrl = "${ctx}/userSumChannelVersion/ajaxGetReportByVersion.action";
			}
		}else{
			ajaxUrl = "${ctx}/report/json?code=user_sum_channel_version_line";
		}
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_uscv_dod[i] = n.vst_uscv_dod;
			vst_uscv_wow[i] = n.vst_uscv_wow;
			vst_uscv_mom[i] = n.vst_uscv_mom;
		});
	...
	function pieByVersion(msg){
		...
		if(data.length > 20){
   		...
   		$.each(top20data, function(i,n){
   			series +="{name:\""+n["value"]+"："+n.vst_uscv_amount+"\",y:"+n.vst_uscv_amount+"},";
   		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_sum_channel_version_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uscv_date' id='vst_uscv_date2' order='desc'>日期</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uscv_channel' id='vst_uscv_channel2' order='desc' style='color:orange;'>渠道</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uscv_version' id='vst_uscv_version2' order='desc'>版本</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uscv_amount' id='vst_uscv_amount2' order='desc'>累计用户</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uscv_dod' id='vst_uscv_dod2' order='desc'>天环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uscv_wow' id='vst_uscv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uscv_mom' id='vst_uscv_mom2' order='desc'>月环比</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_uscv_dod)<0){
				usDodColor = "red";
			}
			if(parseFloat(n.vst_uscv_wow)<0){
				usWowColor = "red";
			}
			if(parseFloat(n.vst_uscv_mom)<0){
				usMomColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_uscv_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_uscv_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_uscv_creator)
						+"<br>修改时间："+formatDate(n.vst_uscv_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_uscv_updator)
						+"<br>备注："+toString(n.vst_uscv_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_uscv_channel)+"</td>";
			table += "<td>"+toString(n.vst_uscv_version)+"</td>";
			table += "<td>"+formatNumber(n.vst_uscv_amount)+"</td>";
			table += "<td class='"+usDodColor+"'>"+toString(n.vst_uscv_dod)+"</td>";
			table += "<td class='"+usWowColor+"'>"+toString(n.vst_uscv_wow)+"</td>";
			table += "<td class='"+usMomColor+"'>"+toString(n.vst_uscv_mom)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&pkgName=" + $("#pkgName").val();
					params += "&vst_uscv_channel=" + $("#vst_uscv_channel").val();
					params += "&vst_uscv_version=" + $("#vst_uscv_version").val();
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					
					location.href = "${ctx}/userSumChannelVersion/exportData.action?" + params
