	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 新增用户(版本&升级版本)统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userAddVersionUpVer/findUserAddVersionUpVer.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 产品统计 > 新增用户(版本&升级版本)统计
...
<script type="text/javascript">
	...
	function ajaxGetReport(){
		...
		if($("#vst_uavu_version").val() == ''){
			ajaxUrl = "${ctx}/report/json?code=user_add_version_upVer_barByVersion";
		}/*else if($("#vst_uavu_upVer").val() == ''){
			ajaxUrl = "${ctx}/userAddVersionUpVer/ajaxGetReportByUpVer.action";
		}*/else{
			ajaxUrl = "${ctx}/report/json?code=user_add_version_upVer_line";
		}
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_add_version_upVer_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uavu_version' id='vst_uavu_version2' order='desc' style='color:orange;'>显示版本</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uavu_upVer' id='vst_uavu_upVer2' order='desc'>升级版本</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uavu_amount' id='vst_uavu_amount2' order='desc'>新增用户</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uavu_dod' id='vst_uavu_dod2' order='desc'>天环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uavu_wow' id='vst_uavu_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uavu_mom' id='vst_uavu_mom2' order='desc'>月环比</a></th>";
		...
		$.each(data,function(i,n){
			...
			table += "<td>"+formatNumber(n.vst_uavu_amount)+"</td>";
			table += "<td class='"+uaDodColor+"'>"+toString(n.vst_uavu_dod)+"</td>";
			table += "<td class='"+uaWowColor+"'>"+toString(n.vst_uavu_wow)+"</td>";
			table += "<td class='"+uaMomColor+"'>"+toString(n.vst_uavu_mom)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					location.href = "${ctx}/userAddVersionUpVer/exportData.action?" + params
