	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 累计用户(省份)统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userSumProvince/findUserSumProvince.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 产品统计 > 累计用户(省份)统计
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_usp_province" name="vst_usp_province" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//文本框，按回车触发查询
		$("#vst_usp_province").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		// 参数特殊化处理
		params += "&" + paramFormat("vst_usp_province", $("#vst_usp_province").val());
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_sum_province_line",
			dataType:"json",
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_usp_dod[i] = Math.round(n.vst_usp_dod*100)/100;
			vst_usp_wow[i] = Math.round(n.vst_usp_wow*100)/100;
			vst_usp_mom[i] = Math.round(n.vst_usp_mom*100)/100;
		});
	...
	function ajaxGetReportByDim(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_sum_province_dim",
			dataType:"json",
			success:function(msg){
	...
	function chinamap(msg){
        ...
        var data_province = "[";
   		$.each(data,function(i,n){
   			//data_province += "{value:"+n.vst_usp_amount+",text:\""+n.vst_usp_amount+"\","+"name:\""+n.value+"\",code:\""+n.code+"\",enname:\""+n.enname+"\"},";
   			data_province += "{value:"+n.vst_usp_amount+",text:\""+n.vst_usp_amount+"\","+"name:\""+n.value+"\"},";
   			if(n.vst_usp_amount > maxTotal){
   	   			maxTotal = n.vst_usp_amount;
   	   		}
   		});
   		data_province+="]";
	...
	function pie(msg){
		...
		if(data.length > 20){
   		...
   		$.each(top20data,function(i,n){
   			series +="{name:\""+getValue(n["value"])+"："+n.vst_usp_amount+"\",y:"+n.vst_usp_amount+"},";
   		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_sum_province_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_usp_date' id='vst_usp_date2' order='desc'>日期</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_usp_province' id='vst_usp_province2' order='desc' style='color:orange;'>省份</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_usp_amount' id='vst_usp_amount2' order='desc'>累计用户</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_usp_dod' id='vst_usp_dod2' order='desc'>天环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_usp_wow' id='vst_usp_wow2' order='desc'>周环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_usp_mom' id='vst_usp_mom2' order='desc'>月环比</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_usp_dod)<0){
				usDodColor = "red";
			}
			if(parseFloat(n.vst_usp_wow)<0){
				usWowColor = "red";
			}
			if(parseFloat(n.vst_usp_mom)<0){
				usMomColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_usp_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_usp_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_usp_creator)
						+"<br>修改时间："+formatDate(n.vst_usp_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_usp_updator)
						+"<br>备注："+toString(n.vst_usp_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_usp_province)+"</td>";
			table += "<td>"+formatNumber(n.vst_usp_amount)+"</td>";
			table += "<td class='"+usDodColor+"'>"+toString(n.vst_usp_dod)+"</td>";
			table += "<td class='"+usWowColor+"'>"+toString(n.vst_usp_wow)+"</td>";
			table += "<td class='"+usMomColor+"'>"+toString(n.vst_usp_mom)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&pkgName=" + $("#pkgName").val();
					params += "&vst_usp_province=" + $("#vst_usp_province").val();
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					
					location.href = "${ctx}/userSumProvince/exportData.action?" + params
