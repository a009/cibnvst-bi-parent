	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 累计用户(升级版本)统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userSumUpVer/findUserSumUpVer.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 产品统计 > 累计用户(升级版本)统计
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_usu_verNum" name="vst_usu_verNum" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//文本框，按回车触发查询
		$("#vst_usu_verNum").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		// 参数特殊化处理
		params += "&" + paramFormat("vst_usu_verNum", $("#vst_usu_verNum").val());
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_sum_upVer_line",
			dataType:"json",
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_usu_dod[i] = Math.round(n.vst_usu_dod*100)/100;
			vst_usu_wow[i] = Math.round(n.vst_usu_wow*100)/100;
			vst_usu_mom[i] = Math.round(n.vst_usu_mom*100)/100;
		});
	...
	function ajaxGetReportByDim(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_sum_upVer_dim",
			dataType:"json",
			success:function(msg){
	...
	function pie(msg){
		...
		if(data.length > 20){
   		...
   		$.each(top20data,function(i,n){
   			series +="{name:\""+getValue(n["value"])+"："+n.vst_usu_amount+"\",y:"+n.vst_usu_amount+"},";
   		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_sum_upVer_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_usu_date' id='vst_usu_date2' order='desc'>日期</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_usu_verNum' id='vst_usu_verNum2' order='desc' style='color:orange;'>升级版本</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_usu_amount' id='vst_usu_amount2' order='desc'>累计用户</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_usu_dod' id='vst_usu_dod2' order='desc'>天环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_usu_wow' id='vst_usu_wow2' order='desc'>周环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_usu_mom' id='vst_usu_mom2' order='desc'>月环比</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_usu_dod)<0){
				usDodColor = "red";
			}
			if(parseFloat(n.vst_usu_wow)<0){
				usWowColor = "red";
			}
			if(parseFloat(n.vst_usu_mom)<0){
				usMomColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_usu_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_usu_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_usu_creator)
						+"<br>修改时间："+formatDate(n.vst_usu_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_usu_updator)
						+"<br>备注："+toString(n.vst_usu_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_usu_verNum)+"</td>";
			table += "<td>"+formatNumber(n.vst_usu_amount)+"</td>";
			table += "<td class='"+usDodColor+"'>"+toString(n.vst_usu_dod)+"</td>";
			table += "<td class='"+usWowColor+"'>"+toString(n.vst_usu_wow)+"</td>";
			table += "<td class='"+usMomColor+"'>"+toString(n.vst_usu_mom)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&pkgName=" + $("#pkgName").val();
					params += "&vst_usu_verNum=" + $("#vst_usu_verNum").val();
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					
					location.href = "${ctx}/userSumUpVer/exportData.action?" + params
