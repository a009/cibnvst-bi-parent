	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 累计用户(渠道&设备)统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userSumChannelBdmodel/findUserSumChannelBdmodel.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 产品统计 > 累计用户(渠道&设备)统计
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_uscb_channel" name="vst_uscb_channel" />
											</li>
											<li>
												<span class="sertitle">设备名称：</span>
												<input class="list-input1" type="text" id="vst_uscb_bdmodel" name="vst_uscb_bdmodel" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
        ...
        $(".pkgName-menu>li").click(function () {
      	...
      	//文本框，按回车触发查询
		$("#vst_uscb_channel").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_uscb_bdmodel").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		// 参数特殊化处理
		params += "&" + paramFormat("vst_uscb_channel", $("#vst_uscb_channel").val());
		params += "&" + paramFormat("vst_uscb_bdmodel", $("#vst_uscb_bdmodel").val());

		var ajaxUrl;
		if($("#vst_uscb_channel").val() == ''){
			ajaxUrl = "${ctx}/report/json?code=user_sum_channel_bdmodel_barByChannel";
		}else if($("#vst_uscb_bdmodel").val() == ''){
			if(startDay == endDay){
				ajaxUrl = "${ctx}/report/json?code=user_sum_channel_bdmodel_pieByBdmodel";
			}else{
				ajaxUrl = "${ctx}/userSumChannelBdmodel/ajaxGetReportByBdmodel.action";
			}
		}else{
			ajaxUrl = "${ctx}/report/json?code=user_sum_channel_bdmodel_line";
		}
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_uscb_amount[i] = n.vst_uscb_amount;
		});
	...
	function pieByBdmodel(msg){
		...
		if(data.length > 20){
   		...
   		$.each(top20data, function(i,n){
   			series +="{name:\""+n["value"]+"："+n.vst_uscb_amount+"\",y:"+n.vst_uscb_amount+"},";
   		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_sum_channel_bdmodel_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uscb_date' id='vst_uscb_date2' order='desc'>日期</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uscb_channel' id='vst_uscb_channel2' order='desc' style='color:orange;'>渠道</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uscb_bdmodel' id='vst_uscb_bdmodel2' order='desc'>设备名称</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uscb_amount' id='vst_uscb_amount2' order='desc'>用户数量</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uscb_rate' id='vst_uscb_rate2' order='desc'>用户占比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uscb_dod' id='vst_uscb_dod2' order='desc'>天环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uscb_wow' id='vst_uscb_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_uscb_mom' id='vst_uscb_mom2' order='desc'>月环比</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_uscb_dod)<0){
				uabDodColor = "red";
			}
			if(parseFloat(n.vst_uscb_wow)<0){
				uabWowColor = "red";
			}
			if(parseFloat(n.vst_uscb_mom)<0){
				uabMomColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_uscb_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_uscb_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_uscb_creator)
						+"<br>修改时间："+formatDate(n.vst_uscb_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_uscb_updator)
						+"<br>备注："+toString(n.vst_uscb_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_uscb_channel)+"</td>";
			table += "<td>"+toString(n.vst_uscb_bdmodel)+"</td>";
			table += "<td>"+formatNumber(n.vst_uscb_amount)+"</td>";
			table += "<td>"+toString(n.vst_uscb_rate)+"</td>";
			table += "<td class='"+uabDodColor+"'>"+toString(n.vst_uscb_dod)+"</td>";
			table += "<td class='"+uabWowColor+"'>"+toString(n.vst_uscb_wow)+"</td>";
			table += "<td class='"+uabMomColor+"'>"+toString(n.vst_uscb_mom)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&pkgName=" + $("#pkgName").val();
					params += "&vst_uscb_channel=" + $("#vst_uscb_channel").val();
					params += "&vst_uscb_bdmodel=" + $("#vst_uscb_bdmodel").val();
					params += "&orderBy=" + $("#orderBy").val();
					params += "&order=" + $("#order").val();
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					
					location.href = "${ctx}/userSumChannelBdmodel/exportData.action?" + params
