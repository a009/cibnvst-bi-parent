	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 播放时长分布 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userPlay/findUserPlay.action" id="listForm" method="post">
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<select class="list-input1" id="vst_up_type" name="vst_up_type">
...
<script type="text/javascript">
	...
	function ajaxGetReport(){
		...
		params += "&vst_up_type=" + $("#vst_up_type").val();

		var ajaxUrl;
		if($("#vst_up_type").val() == ''){
			if(startDay == endDay){
				ajaxUrl = "${ctx}/report/json?code=user_play_bar";
			}else{
				ajaxUrl = "${ctx}/userPlay/ajaxGetReportByType.action";
			}
		}else{
			ajaxUrl = "${ctx}/report/json?code=user_play_line";
		}
	...
	function bar(msg){
		...
		var series = [];
		$.each(data, function(i,n){
			xaxis[i] = getMapValue('${types}', n.vst_up_type);
			series[i] = n.vst_up_uv;
		});
	...
	function lineByType(msg){
		var data = msg.data;
		var types = data.types;	//Top类型
		...
		var series = [];
		for(var i=0; i<=types.length; i++){
			var tname = getMapValue('${types}', types[i]);
			var json = {name : tname, data : uvArr[i]};
			series.push(json);
		}
	...
	/*function barByType(msg){
		var data = msg.data;
		
   		var xaxis = [];
   		var uvArr = new Array();
   		for(var type=0; type<=8; type++){
   	   		uvArr[type] = new Array();
   		}
		// 遍历数据
		var index = 0;
		$.each(data.dateMap, function(date,uvs){
		...
		var series = [];
		for(var type=0; type<=8; type++){
			var tname = getMapValue('${types}', type);
			var json = {name : tname, data : uvArr[type], stack : 'uv'+type};
			series.push(json);
		}
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_play_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_up_date' id='vst_up_date2' order='desc'>日期</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_up_type' id='vst_up_type2' order='desc' style='color:orange;'>类型</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_up_vv' id='vst_up_vv2' order='desc'>播放量</a> | <a onclick='changeOrder(this)' orderBy='vst_up_vv_dod' id='vst_up_vv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_up_vv_wow' id='vst_up_vv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_up_uv' id='vst_up_uv2' order='desc'>播放人数</a> | <a onclick='changeOrder(this)' orderBy='vst_up_uv_dod' id='vst_up_uv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_up_uv_wow' id='vst_up_uv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_up_avg' id='vst_up_avg2' order='desc'>人均播放量</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_up_playtime' id='vst_up_playtime2' order='desc'>播放时长(小时)</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_up_playtime_avg' id='vst_up_playtime_avg2' order='desc'>人均播放时长(分钟)</a></th>";
		
		table += "</tr></thead><tbody>";
		
		$.each(data,function(i,n){
			var row = i%2+1;
			var usVvDodColor = "green";
			var usVvWowColor = "green";
			var usUvDodColor = "green";
			var usUvWowColor = "green";
			
			if(parseFloat(n.vst_up_vv_dod)<0){
				usVvDodColor = "red";
			}
			if(parseFloat(n.vst_up_vv_wow)<0){
				usVvWowColor = "red";
			}
			if(parseFloat(n.vst_up_uv_dod)<0){
				usUvDodColor = "red";
			}
			if(parseFloat(n.vst_up_uv_wow)<0){
				usUvWowColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_up_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_up_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_up_creator)
						+"<br>修改时间："+formatDate(n.vst_up_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_up_updator)
						+"<br>备注："+toString(n.vst_up_summary)+"' class='enlargeTextTitle'>"
					+getMapValue('${types}', n.vst_up_type)+"</td>";
			//播放量
			table += "<td>"+formatNumber(n.vst_up_vv)+" | ";
			table += "<span class='"+usVvDodColor+"'>"+toString(n.vst_up_vv_dod)+"</span> | ";
			table += "<span class='"+usVvWowColor+"'>"+toString(n.vst_up_vv_wow)+"</span></td>";
			//播放人数
			table += "<td>"+formatNumber(n.vst_up_uv)+" | ";
			table += "<span class='"+usUvDodColor+"'>"+toString(n.vst_up_uv_dod)+"</span> | ";
			table += "<span class='"+usUvWowColor+"'>"+toString(n.vst_up_uv_wow)+"</span></td>";
			//人均播放量
			table += "<td>"+toString(n.vst_up_avg)+"</td>";
			//播放时长
			table += "<td>"+toHour(n.vst_up_playtime*1000)+"</td>";
			table += "<td>"+toMinute(n.vst_up_playtime_avg*1000)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					location.href = "${ctx}/userPlay/exportData.action?" + params
