	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 累计用户(设备)统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userSumBdmodel/findUserSumBdmodel.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 产品统计 > 累计用户(设备)统计
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_usb_bdmodel" name="vst_usb_bdmodel" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
        ...
        $(".pkgName-menu>li").click(function () {
      	...
      	//文本框，按回车触发查询
		$("#vst_usb_bdmodel").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		// 参数特殊化处理
		params += "&" + paramFormat("vst_usb_bdmodel", $("#vst_usb_bdmodel").val());

		var ajaxUrl;
		if($("#vst_usb_bdmodel").val() == ''){
			ajaxUrl = "${ctx}/report/json?code=user_sum_bdmodel_bar";
		}else{
			ajaxUrl = "${ctx}/report/json?code=user_sum_bdmodel_line";
		}
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_usb_amount[i] = n.vst_usb_amount;
		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_sum_bdmodel_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_usb_date' id='vst_usb_date2' order='desc'>日期</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_usb_bdmodel' id='vst_usb_bdmodel2' order='desc' style='color:orange;'>设备名称</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_usb_amount' id='vst_usb_amount2' order='desc'>用户数量</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_usb_rate' id='vst_usb_rate2' order='desc'>用户占比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_usb_dod' id='vst_usb_dod2' order='desc'>天环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_usb_wow' id='vst_usb_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_usb_mom' id='vst_usb_mom2' order='desc'>月环比</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_usb_dod)<0){
				uabDodColor = "red";
			}
			if(parseFloat(n.vst_usb_wow)<0){
				uabWowColor = "red";
			}
			if(parseFloat(n.vst_usb_mom)<0){
				uabMomColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_usb_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_usb_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_usb_creator)
						+"<br>修改时间："+formatDate(n.vst_usb_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_usb_updator)
						+"<br>备注："+toString(n.vst_usb_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_usb_bdmodel)+"</td>";
			table += "<td>"+formatNumber(n.vst_usb_amount)+"</td>";
			table += "<td>"+toString(n.vst_usb_rate)+"</td>";
			table += "<td class='"+uabDodColor+"'>"+toString(n.vst_usb_dod)+"</td>";
			table += "<td class='"+uabWowColor+"'>"+toString(n.vst_usb_wow)+"</td>";
			table += "<td class='"+uabMomColor+"'>"+toString(n.vst_usb_mom)+"</td>";
