	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 活跃用户(渠道)统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userActiveChannel/findUserActiveChannel.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 产品统计 > 活跃用户(渠道)统计
...
<script type="text/javascript">
	...
	function ajaxGetReport(){
		...
		// 参数特殊化处理
		params += "&" + paramFormat("vst_uac_channel", $("#vst_uac_channel").val());

		var ajaxUrl;
		if($("#vst_uac_channel").val() == ''){
			ajaxUrl = "${ctx}/userActiveChannel/ajaxGetReportByChannel.action"
		}else{
			ajaxUrl = "${ctx}/report/json?code=user_active_channel_line";
		}
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_uac_week[i] = n.vst_uac_week;
			vst_uac_month[i] = n.vst_uac_month;
		});
	...
	function lineByChannel(msg){
		var data = msg.data;
		var dataLength  = Object.keys(data.dateMap).length;
		var channels = data.channels;	// Top10渠道
		...
		var series = [];
		for(var i=0; i<channels.length; i++){
			var json = {name : channels[i], data : amountArr[i]};
			series.push(json);
		}
	...
	function ajaxGetReportByDim(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_active_channel_dim",
			dataType:"json",
			success:function(msg){
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_active_channel_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_uac_amount' id='vst_uac_amount2' order='desc'>日活跃</a> | <a onclick='changeOrder(this)' orderBy='vst_uac_dod' id='vst_uac_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uac_wow' id='vst_uac_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uac_mom' id='vst_uac_mom2' order='desc'>月环比</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_uac_week' id='vst_uac_week2' order='desc'>周活跃</a> | <a onclick='changeOrder(this)' orderBy='vst_uac_week_dod' id='vst_uac_week_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uac_week_wow' id='vst_uac_week_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uac_week_mom' id='vst_uac_week_mom2' order='desc'>月环比</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_uac_month' id='vst_uac_month2' order='desc'>月活跃</a> | <a onclick='changeOrder(this)' orderBy='vst_uac_month_dod' id='vst_uac_month_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uac_month_wow' id='vst_uac_month_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_uac_month_mom' id='vst_uac_month_mom2' order='desc'>月环比</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_uac_week_dod)<0){
				uaWeekDodColor = "red";
			}
			if(parseFloat(n.vst_uac_week_wow)<0){
				uaWeekWowColor = "red";
			}
			if(parseFloat(n.vst_uac_week_mom)<0){
				uaWeekMomColor = "red";
			}
			if(parseFloat(n.vst_uac_month_dod)<0){
				uaMonthDodColor = "red";
			}
			if(parseFloat(n.vst_uac_month_wow)<0){
				uaMonthWowColor = "red";
			}
			if(parseFloat(n.vst_uac_month_mom)<0){
				uaMonthMomColor = "red";
			}
			...
			table += "<td data-title='新增时间："+formatDate(n.vst_uac_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_uac_creator)
						+"<br>修改时间："+formatDate(n.vst_uac_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_uac_updator)
						+"<br>备注："+toString(n.vst_uac_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_uac_channel)+"</td>";
			//日活跃
			table += "<td>"+formatNumber(n.vst_uac_amount)+" | ";
			table += "<span class='"+uaDodColor+"'>"+toString(n.vst_uac_dod)+"</span> | ";
			table += "<span class='"+uaWowColor+"'>"+toString(n.vst_uac_wow)+"</span> | ";
			table += "<span class='"+uaMomColor+"'>"+toString(n.vst_uac_mom)+"</span></td>";
			//周活跃
			table += "<td>"+formatNumber(n.vst_uac_week)+" | ";
			table += "<span class='"+uaWeekDodColor+"'>"+toString(n.vst_uac_week_dod)+"</span> | ";
			table += "<span class='"+uaWeekWowColor+"'>"+toString(n.vst_uac_week_wow)+"</span> | ";
			table += "<span class='"+uaWeekMomColor+"'>"+toString(n.vst_uac_week_mom)+"</span></td>";
			//月活跃
			table += "<td>"+formatNumber(n.vst_uac_month)+" | ";
			table += "<span class='"+uaMonthDodColor+"'>"+toString(n.vst_uac_month_dod)+"</span> | ";
			table += "<span class='"+uaMonthWowColor+"'>"+toString(n.vst_uac_month_wow)+"</span> | ";
			table += "<span class='"+uaMonthMomColor+"'>"+toString(n.vst_uac_month_mom)+"</span></td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					location.href = "${ctx}/userActiveChannel/exportData.action?" + params
