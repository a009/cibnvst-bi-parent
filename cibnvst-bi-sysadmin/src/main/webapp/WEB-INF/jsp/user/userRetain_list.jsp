					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 留存用户统计 > 导出</span>
							</h4>
						</div>
...
<script type="text/javascript">
	...
	function line(msg){
		...
		var xaxis = [];
		var vst_ur_amount = [];
		var vst_ur_week_amount = [];
		var vst_ur_month_amount = [];
		var vst_ur_season_amount = [];

		$.each(data,function(i,n){
			xaxis[i] = getDateWeek(n.date+'', 'yyyyMMdd');
			vst_ur_amount[i] = n.vst_ur_amount;
			vst_ur_week_amount[i] = n.vst_ur_week_amount;
			vst_ur_month_amount[i] = n.vst_ur_month_amount;
			vst_ur_season_amount[i] = n.vst_ur_season_amount;
		});
		...
		$("#line-chart").highcharts({
			...
			{
				name : '周留存用户',
				data : vst_ur_week_amount,
				visible : false
			},
			{
				name : '月留存用户',
				data : vst_ur_month_amount,
				visible : false
			},
			{
				name : '季留存用户',
				data : vst_ur_season_amount,
				visible : false
			}
			],
			plotOptions: {
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_ur_date' id='vst_ur_date2' order='desc' style='color:orange;'>日期</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_ur_amount' id='vst_ur_amount2' order='desc'>次日留存</a> | <a onclick='changeOrder(this)' orderBy='vst_ur_dod' id='vst_ur_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_ur_wow' id='vst_ur_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_ur_rate' id='vst_ur_rate2' order='desc'>留存率</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_ur_week_amount' id='vst_ur_week_amount2' order='desc'>周留存</a> | <a onclick='changeOrder(this)' orderBy='vst_ur_week_dod' id='vst_ur_week_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_ur_week_wow' id='vst_ur_week_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_ur_week_rate' id='vst_ur_week_rate2' order='desc'>留存率</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_ur_month_amount' id='vst_ur_month_amount2' order='desc'>月留存</a> | <a onclick='changeOrder(this)' orderBy='vst_ur_month_dod' id='vst_ur_month_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_ur_month_wow' id='vst_ur_month_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_ur_month_rate' id='vst_ur_month_rate2' order='desc'>留存率</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_ur_season_amount' id='vst_ur_season_amount2' order='desc'>季留存</a> | <a onclick='changeOrder(this)' orderBy='vst_ur_season_dod' id='vst_ur_season_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_ur_season_wow' id='vst_ur_season_wow2' order='desc'>周环比</a> | <a onclick='changeOrder(this)' orderBy='vst_ur_season_rate' id='vst_ur_season_rate2' order='desc'>留存率</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_ur_dod)<0){
				urDodColor = "red";
			}
			if(parseFloat(n.vst_ur_wow)<0){
				urWowColor = "red";
			}
			if(parseFloat(n.vst_ur_week_dod)<0){
				urWeekDodColor = "red";
			}
			if(parseFloat(n.vst_ur_week_wow)<0){
				urWeekWowColor = "red";
			}
			if(parseFloat(n.vst_ur_month_dod)<0){
				urMonthDodColor = "red";
			}
			if(parseFloat(n.vst_ur_month_wow)<0){
				urMonthWowColor = "red";
			}
			if(parseFloat(n.vst_ur_season_dod)<0){
				urSeasonDodColor = "red";
			}
			if(parseFloat(n.vst_ur_season_wow)<0){
			...
			table += "<td data-title='新增时间："+formatDate(n.vst_ur_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_ur_creator)
						+"<br>修改时间："+formatDate(n.vst_ur_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_ur_updator)
						+"<br>备注："+toString(n.vst_ur_summary)+"' class='enlargeTextTitle'>"
					+getDateWeek(n.vst_ur_date+'', 'yyyyMMdd')+"</td>";
			//次日留存
			table += "<td>"+formatNumber(n.vst_ur_amount)+" | ";
			table += "<span class='"+urDodColor+"'>"+toString(n.vst_ur_dod)+"</span> | ";
			table += "<span class='"+urWowColor+"'>"+toString(n.vst_ur_wow)+"</span> | ";
			table += toString(n.vst_ur_rate)+"</td>";
			//周留存
			table += "<td>"+formatNumber(n.vst_ur_week_amount)+" | ";
			table += "<span class='"+urWeekDodColor+"'>"+toString(n.vst_ur_week_dod)+"</span> | ";
			table += "<span class='"+urWeekWowColor+"'>"+toString(n.vst_ur_week_wow)+"</span> | ";
			table += toString(n.vst_ur_week_rate)+"</td>";
			//月留存
			table += "<td>"+formatNumber(n.vst_ur_month_amount)+" | ";
			table += "<span class='"+urMonthDodColor+"'>"+toString(n.vst_ur_month_dod)+"</span> | ";
			table += "<span class='"+urMonthWowColor+"'>"+toString(n.vst_ur_month_wow)+"</span> | ";
			table += toString(n.vst_ur_month_rate)+"</td>";
			//季留存
			table += "<td>"+formatNumber(n.vst_ur_season_amount)+" | ";
			table += "<span class='"+urSeasonDodColor+"'>"+toString(n.vst_ur_season_dod)+"</span> | ";
			table += "<span class='"+urSeasonWowColor+"'>"+toString(n.vst_ur_season_wow)+"</span> | ";
			table += toString(n.vst_ur_season_rate)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					location.href = "${ctx}/userRetain/exportData.action?" + params
