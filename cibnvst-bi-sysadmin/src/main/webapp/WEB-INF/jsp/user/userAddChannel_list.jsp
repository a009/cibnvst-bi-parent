	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 新增用户(渠道)统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/userAddChannel/findUserAddChannel.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 产品统计 > 新增用户(渠道)统计
...
<script type="text/javascript">
	...
	function ajaxGetReport(){
		...
		if($("#vst_uac_channel").val() == ''){
			ajaxUrl = "${ctx}/userAddChannel/ajaxGetReportByChannel.action"
		}else{
			ajaxUrl = "${ctx}/report/json?code=user_add_channel_line";
		}
	...
	function ajaxGetReportByDim(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_add_channel_dim",
			dataType:"json",
			success:function(msg){
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_add_channel_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uac_channel' id='vst_uac_channel2' order='desc' style='color:orange;'>渠道</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uac_amount' id='vst_uac_amount2' order='desc'>新增用户</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uac_dod' id='vst_uac_dod2' order='desc'>天环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uac_wow' id='vst_uac_wow2' order='desc'>周环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_uac_mom' id='vst_uac_mom2' order='desc'>月环比</a></th>";
		...
		$.each(data,function(i,n){
			...
			table += "<td class='"+uaDodColor+"'>"+toString(n.vst_uac_dod)+"</td>";
			table += "<td class='"+uaWowColor+"'>"+toString(n.vst_uac_wow)+"</td>";
			table += "<td class='"+uaMomColor+"'>"+toString(n.vst_uac_mom)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					location.href = "${ctx}/userAddChannel/exportData.action?" + params
