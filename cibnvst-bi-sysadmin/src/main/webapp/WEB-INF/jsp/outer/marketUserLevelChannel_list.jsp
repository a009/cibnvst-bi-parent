	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
			<div class="content-roll">
				<form action="${ctx}/marketUserLevelChannel/findMarketUserLevelChannel.action" id="listForm" method="post">
...
<script type="text/javascript">
	...
	function ajaxGetReport(){
		...
		// 参数特殊化处理
		params += "&" + paramFormat("vst_ulc_channel", $("#vst_ulc_channel").val());
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_level_channel_line",
			dataType:"json",
	...
	function line(msg){
		...
		var vst_ulc_season_dod = [];
		var vst_ulc_season_wow = [];
		var vst_ulc_season_radio = [];
		// 调整系数
		...
		$.each(data,function(i,n){
			...
			vst_ulc_add_amount[i] = Math.round(n.vst_ulc_add_amount * vst_coefficient);
			vst_ulc_season_amount[i] = Math.round(n.vst_ulc_season_amount * vst_coefficient);
			vst_ulc_season_dod[i] = n.vst_ulc_season_dod;
			vst_ulc_season_wow[i] = n.vst_ulc_season_wow;
			vst_ulc_season_radio[i] = n.vst_ulc_season_radio;
		});
		...
		$("#line-chart").highcharts({
			...
			{
				name : '天环比(%)',
				data : vst_ulc_season_dod,
				visible : false
			},
			{
				name : '周环比(%)',
				data : vst_ulc_season_wow,
				visible : false
			},
			{
				name : '占比(%)',
				data : vst_ulc_season_radio,
				visible : false
			}
			],
			plotOptions: {
	...
	function ajaxGetReportByDim(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			//url:"${ctx}/report/json?code=outer_user_level_channel_dim",
			url:"${ctx}/report/json?code=user_level_channel_dim",
			dataType:"json",
			success:function(msg){
	...
	function bar(msg){
		...
		$.each(top10data,function(i,n){
			...
			series[i] = Math.round(n.vst_ulc_season_amount * vst_coefficient);
		});
	...
	function pie(msg){
		...
		if(data.length > 20){
   		...
   		$.each(top20data,function(i,n){
   			var vst_ulc_season_amount = Math.round(n.vst_ulc_season_amount * vst_coefficient);
   			series +="{name:\""+getValue(n["value"])+"："+vst_ulc_season_amount+"\",y:"+vst_ulc_season_amount+"},";
   		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=outer_user_level_channel_table",
			url:"${ctx}/report/json?code=user_level_channel_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_ulc_add_amount' id='vst_ulc_add_amount2' order='desc'>季新增用户</a></th>";
		...
		$.each(data,function(i,n){
			...
			table += "<td data-title='新增时间："+formatDate(n.vst_ulc_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_ulc_creator)
						+"<br>修改时间："+formatDate(n.vst_ulc_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_ulc_updator)
						+"<br>备注："+toString(n.vst_ulc_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_ulc_channel)+"</td>";
			//季新增用户
			var vst_ulc_add_amount = Math.round(n.vst_ulc_add_amount * vst_coefficient);
			table += "<td>"+formatNumber(vst_ulc_add_amount)+"</td>";
			//季质量用户
			var vst_ulc_season_amount = Math.round(n.vst_ulc_season_amount * vst_coefficient);
			table += "<td>"+formatNumber(vst_ulc_season_amount)+" | ";
	...
	function button_export(){
		...
		if(!isNotEmpty(startDay) || !isNotEmpty(endDay) || getDateDiff(startDay, endDay, 'day') > 92){
			...
			location.href = "${ctx}/marketUserLevelChannel/exportData.action?" + params
