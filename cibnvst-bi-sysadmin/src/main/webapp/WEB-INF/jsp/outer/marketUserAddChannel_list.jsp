	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
			<div class="content-roll">
				<form action="${ctx}/marketUserAddChannel/findMarketUserAddChannel.action" id="listForm" method="post">
...
<script type="text/javascript">
	...
	function ajaxGetReport(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_add_channel_line",
			dataType:"json",
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_uac_amount[i] = Math.round(n.vst_uac_amount * vst_coefficient);
		});
	...
	function bar(msg){
		...
		$.each(top10data,function(i,n){
			...
			series[i] = Math.round(n.vst_uac_amount * vst_coefficient);
		});
	...
	function pie(msg){
		...
		if(data.length > 20){
   		...
   		$.each(top20data,function(i,n){
   	   		var vst_uac_amount = Math.round(n.vst_uac_amount * vst_coefficient);
   			series +="{name:\""+getValue(n["value"])+"："+vst_uac_amount+"\",y:"+vst_uac_amount+"},";
   		});
	...
	function showTable(msg){
		...
		$.each(data,function(i,n){
			...
			var vst_uac_amount = Math.round(n.vst_uac_amount * vst_coefficient);
			table += "<td>"+formatNumber(vst_uac_amount)+"</td>";
	...
	function button_export(){
		...
		if(!isNotEmpty(startDay) || !isNotEmpty(endDay) || getDateDiff(startDay, endDay, 'day') > 92){
			...
			location.href = "${ctx}/marketUserAddChannel/exportData.action?" + params
