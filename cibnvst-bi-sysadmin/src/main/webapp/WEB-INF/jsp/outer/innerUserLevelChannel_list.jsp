...
<body class="hold-transition skin-blue sidebar-mini">
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
				...
				<div class="modal-dialog modalGrap">
					<div class="modal-content">
						...
						<form id="editBoxForm" method="post" enctype="multipart/form-data">
							<table class="line03">
								<tr>
									<td class="tright" width="20%">季新增用户：</td>
									<td width="50%">
										<input class="list-input" id="vst_ulc_add_amount4" name="vst_ulc_add_amount" />
										<span class="red">*</span>
										<span id="newAddAmount" style="font-weight: bold; color: red;"></span>
									</td>
									<td width="28%"><span id="vst_ulc_add_amount4Tip"></span></td>
								</tr>
								<tr>
									<td class="tright" width="20%">季质量用户：</td>
									<td width="50%">
										<input class="list-input" id="vst_ulc_season_amount4" name="vst_ulc_season_amount" />
										<span class="red">*</span>
										<span id="newSeasonAmount" style="font-weight: bold; color: red;"></span>
									</td>
									<td width="28%"><span id="vst_ulc_season_amount4Tip"></span></td>
								</tr>
								<tr>
									<td class="tright" width="20%">调整系数：</td>
									<td width="50%">
										<input class="list-input" id="vst_ulc_modulus4" name="vst_ulc_modulus" value="1" />
										<span class="red">*</span>
									</td>
									<td width="28%"><span id="vst_ulc_modulus4Tip"></span></td>
								</tr>
							</table>
		                ...
		                <form id="batchUpdateBoxForm" method="post" enctype="multipart/form-data">
							<table class="line03">
								<tr>
							 		<td class="tright" width="20%">调整系数：</td>
							 		<td width="50%">
							 			<input class="list-input" id="vst_ulc_modulus1" name="vst_ulc_modulus" value="0.35" />
							 			<span class="red">*</span>
							 		</td>
							 		<td width="28%"><span id="vst_ulc_modulus1Tip"></span></td>
							 	</tr>
							</table>
							<div style="padding-bottom:10px; margin-right:10px; text-align:right;">
		                ...
		                <form id="importBoxForm" method="post" enctype="multipart/form-data">
							<table class="line03">
								<tr>
							 		...
							 		<td width="50%">
							 			<input type="text" class="list-input" id="vst_ulc_date" name="vst_ulc_date" 
											placeholder="yyyy-MM-dd" readonly="readonly" />
							 		</td>
							 		<td width="28%"><span id="vst_ulc_dateTip"></span></td>
							 	</tr>
							</table>
							<div style="padding-bottom:10px; margin-right:10px; text-align:right;">
			...
			<div class="content-roll">
				<form action="${ctx}/innerUserLevelChannel/findInnerUserLevelChannel.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 市场数据 > 季质量渠道用户(对内)统计
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_ulc_channel" name="vst_ulc_channel" />
											</li>
											<li>
												<span class="sertitle">审核状态：</span>
												<select class="list-input1" id="vst_ulc_state" name="vst_ulc_state">
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_ulc_updator" name="vst_ulc_updator" />
											</li>
										</ul>
									</div>
	...
	<script type="text/javascript">
		...
		laydate.render({
	  	...
	  	$("#vst_ulc_date").val(getNDateTime("yyyy-MM-dd",0));
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//文本框，按回车触发查询
		$("#vst_ulc_channel").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_ulc_updator").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		...
		if(startDay == '' || endDay == ''){
			...
			if(confirm("确认要删除当前选定条件的数据吗？")){
		        var params = "startDay="+startDay+"&endDay="+endDay
						+"&vst_ulc_channel="+$("#vst_ulc_channel").val()
						+"&vst_ulc_state="+$("#vst_ulc_state").val()
						+"&vst_ulc_updator="+$("#vst_ulc_updator").val()
		        ...
		        $.ajax({
					...
					async : true,
					cache : false,
					url:"${ctx}/innerUserLevelChannel/ajaxDeleteData.action",
					dataType:"json",
	...
	function button_audit(){
		...
		if(confirm("您确定要审核所选的"+modifyChecked.split(',').length+"条记录吗？")){
			...
			$.ajax({
				...
				async : true,
				cache : false,
				url:"${ctx}/innerUserLevelChannel/ajaxAuditData.action",
				dataType:"json",
				success:function(msg){
					if(msg.code == 100){
		...
		if(startDay == '' || endDay == ''){
			...
			if(confirm("确认要审核当前选定条件的数据吗？")){
		        ...
		        $.ajax({
					...
					async : true,
					cache : false,
					url:"${ctx}/innerUserLevelChannel/ajaxBatchAuditData.action",
					dataType:"json",
					success:function(msg){
						if(msg.code == 100){
	...
	$(document).ready(function(){
		...
		$("#vst_ulc_modulus1").formValidator({
			validatorgroup : "1",
	  		onshow : "请填写调整系数！",
			...
			fun :function(){
				var patrn = /^\d+(\.\d{1,2})?$/;
	            if(!patrn.exec($("#vst_ulc_modulus1").val())){
		            return false;
	            }
	            return true;
			},
		...
		$("#batchUpdateSubmit").click(function(){
			...
			if(flag){
				var modulus = $("#vst_ulc_modulus1").val();
				var recordId = $("#recordId").val();
				var moduleId = $("#moduleId").val();
				$.ajax({
					url:"${ctx}/innerUserLevelChannel/ajaxBatchUpdate.action",
					type:"POST",
					dataType:"json",
					cache:false,
	...
	$(document).ready(function(){
		...
		$("#importSubmit").click(function(){
			...
			if(flag){
				var date = $("#vst_ulc_date").val();
                var $btn = $(this).button('loading');
				$.ajax({
					url:"${ctx}/innerUserLevelChannel/ajaxImportData.action",
					type:"POST",
					dataType:"json",
					cache:false,
	...
	$(document).ready(function(){
		...
		$("#syncSubmit").click(function(){
			...
			if(flag){
                ...
                var $btn = $(this).button('loading');
				$.ajax({
					url:"${ctx}/innerUserLevelChannel/ajaxSyncData.action",
					type:"POST",
					dataType:"json",
					cache:false,
	...
	$(document).ready(function(){
		...
		$("#vst_ulc_add_amount4").formValidator({
			validatorgroup : "4",
			onshow : "请填写季新增用户！",
			onfocus : "季新增用户不能为空，必须为数字！",
			oncorrect : "验证通过！"
		}).inputValidator({
			min : 1,
			max : 13,
			onerror : "季新增用户长度在1~13位！"
		}).regexValidator({
			regexp :"^[0-9]{1,}$",
			datatype :"string",
		...
		$("#vst_ulc_season_amount4").formValidator({
			validatorgroup : "4",
			onshow : "请填写季质量用户！",
			onfocus : "季质量用户不能为空，必须为数字！",
			oncorrect : "验证通过！"
		}).inputValidator({
			min : 1,
			max : 13,
			onerror : "季质量用户长度在1~13位！"
		}).regexValidator({
			regexp :"^[0-9]{1,}$",
			datatype :"string",
			onerror :"格式错误，必须为纯数字，不能带有小数！"
		...
		$("#vst_ulc_modulus4").formValidator({
			validatorgroup : "4",
			onshow : "请填写调整系数！",
			onfocus : "调整系数不能为空，必须为数字！",
			oncorrect : "验证通过！"
			...
			fun :function(){
				var patrn = /^\d+(\.\d{1,2})?$/;
				if(!patrn.exec($("#vst_ulc_modulus4").val())){
					return false;
				}
				return true;
			},
		...
		// 失去焦点，计算新的季新增用户
		$("#vst_ulc_add_amount4").blur(function(){
			var addAmount = $("#vst_ulc_add_amount4").val();
			var modulus = $("#vst_ulc_modulus4").val();
			if(addAmount != '' && modulus != ''){
				var result = Math.round(addAmount * modulus);
				$("#newAddAmount").html(result);
			}
		});

		// 失去焦点，计算新的季质量用户
		$("#vst_ulc_season_amount4").blur(function(){
			var seasonAmount = $("#vst_ulc_season_amount4").val();
			var modulus = $("#vst_ulc_modulus4").val();
			if(seasonAmount != '' && modulus != ''){
				var result = Math.round(seasonAmount * modulus);
				$("#newSeasonAmount").html(result);
			}
		});
		...
		$("#editSubmit").click(function(){
			...
			if(flag){
				...
				var moduleId = $("#moduleId").val();
				$.ajax({
					url:"${ctx}/innerUserLevelChannel/ajaxEditData.action",
					type:"POST",
					dataType:"json",
					cache:false,
					data:"recordId="+recordId+"&addAmount="+addAmount+"&seasonAmount="+seasonAmount+"&modulus="+modulus+"&moduleId="+moduleId,
					success:function(data){
						if(data.code == 100){
							// 关闭弹窗
							$("#edit_myModal").hide();
	...
	function ajaxGetReport(){
		...
		params += "&vst_ulc_channel=" + $("#vst_ulc_channel").val();
		params += "&vst_ulc_state=" + $("#vst_ulc_state").val();
		params += "&vst_ulc_updator=" + $("#vst_ulc_updator").val();
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/innerUserLevelChannel/ajaxGetReportByDate.action",
			dataType:"json",
			success:function(msg){
	...
	function line(msg){
		var data = msg.data;
		var xaxis = [];
		var vst_ulc_add_amount = [];
		var vst_ulc_season_amount = [];

		$.each(data,function(i,n){
			xaxis[i] = getDateWeek(n.date+'', 'yyyyMMdd');
			vst_ulc_add_amount[i] = n.vst_ulc_add_amount;
			vst_ulc_season_amount[i] = n.vst_ulc_season_amount;
		});
		...
		$("#line-chart").highcharts({
			...
			{
				name : '季新增用户数',
				data : vst_ulc_add_amount,
				visible : false
			},
			{
				name : '季质量用户数',
				data : vst_ulc_season_amount,
			}
			],
			plotOptions: {
	        	series: {
	...
	function ajaxGetReportByDim(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/innerUserLevelChannel/ajaxGetReportByDim.action",
			dataType:"json",
			success:function(msg){
	...
	function bar(msg){
		...
		$.each(top10data,function(i,n){
			...
			series[i] = n.vst_ulc_season_amount;
		});
	...
	function pie(msg){
		...
		$.each(top20data,function(i,n){
			series +="{name:\""+getValue(n["value"])+"："+n.vst_ulc_season_amount+"\",y:"+n.vst_ulc_season_amount+"},";
		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/innerUserLevelChannel/ajaxGetCutPage.action",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_ulc_date' id='vst_ulc_date2' order='desc'>日期</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_ulc_channel' id='vst_ulc_channel2' order='desc' style='color:orange;'>渠道</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_ulc_add_amount' id='vst_ulc_add_amount2' order='desc'>季新增用户</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_ulc_season_amount' id='vst_ulc_season_amount2' order='desc'>季质量用户</a> | <a onclick='changeOrder(this)' orderBy='vst_ulc_season_dod' id='vst_ulc_season_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_ulc_season_wow' id='vst_ulc_season_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_ulc_season_radio' id='vst_ulc_season_radio2' order='desc'>占比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_ulc_modulus' id='vst_ulc_modulus2' order='desc'>调整系数</a></th>";
		table += "<th width='15%'>新增用户*调整系数</th>";
		table += "<th width='15%'>质量用户*调整系数</th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_ulc_state' id='vst_ulc_state2' order='desc'>审核状态</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_ulc_season_dod)<0){
				dodColor = "red";
			}
			if(parseFloat(n.vst_ulc_season_wow)<0){
				wowColor = "red";
			}
			// 审核状态
			var vst_ulc_state = n.vst_ulc_state;
			if(n.vst_ulc_state == 1){
				vst_ulc_state = "<span class='red'>未审核<span>";
			}else if(n.vst_ulc_state == 2){
				vst_ulc_state = "<span class='green'>已审核<span>";
			}
			...
			table += "<td><input type='checkbox' id='recordIds' name='recordIds' "
							+ "value='" + n.vst_ulc_id + "' "
							+ "state='" + n.vst_ulc_state + "' "
							+ "modulus='" + n.vst_ulc_modulus + "' "
							+ " /></td>";
			table += "<td>"+getDateWeek(n.vst_ulc_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='同步时间："+formatDate(n.vst_ulc_synctime, "yyyy-MM-dd HH:mm:ss")
						+"<br>新增时间："+toString(n.vst_ulc_addtime)
						+"<br>创建人："+toString(n.vst_ulc_creator)
						+"<br>修改时间："+toString(n.vst_ulc_uptime)
						+"<br>修改人："+toString(n.vst_ulc_updator)
						+"<br>备注："+toString(n.vst_ulc_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_ulc_channel)+"</td>";
			//季新增用户
			table += "<td>"+formatNumber(n.vst_ulc_add_amount)+"</td>";
			//季质量用户
			table += "<td>"+formatNumber(n.vst_ulc_season_amount)+" | ";
			table += "<span class='"+dodColor+"'>"+toString(n.vst_ulc_season_dod)+"</span> | ";
			table += "<span class='"+wowColor+"'>"+toString(n.vst_ulc_season_wow)+"</span></td>";
			//占比
			table += "<td>"+toString(n.vst_ulc_season_radio)+"</td>";
			
			table += "<td class='myClick'>"+toString(n.vst_ulc_modulus)+"</td>";
			table += "<td>"+formatNumber(Math.round(n.vst_ulc_add_amount * n.vst_ulc_modulus))+"</td>";
			table += "<td>"+formatNumber(Math.round(n.vst_ulc_season_amount * n.vst_ulc_modulus))+"</td>";
			table += "<td>"+toString(vst_ulc_state)+"</td>";
	...
	function button_export(){
		...
		if(!isNotEmpty(startDay) || !isNotEmpty(endDay) || getDateDiff(startDay, endDay, 'day') > 92){
			...
			location.href = "${ctx}/innerUserLevelChannel/exportData.action?" + params
					+ "&moduleId=" + $("#moduleId").val();
		}
	}
</script>
