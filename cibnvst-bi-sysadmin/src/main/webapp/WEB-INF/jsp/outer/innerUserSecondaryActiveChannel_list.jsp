...
<body class="hold-transition skin-blue sidebar-mini">
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
				...
				<div class="modal-dialog modalGrap">
					<div class="modal-content">
						...
						<form id="editBoxForm" method="post" enctype="multipart/form-data">
							<table class="line03">
								<tr>
									<td class="tright" width="20%">二次活跃用户：</td>
									<td width="50%">
										<input class="list-input" id="vst_usac_amount4" name="vst_usac_amount" />
										<span class="red">*</span>
										<span id="newAmount" style="font-weight: bold; color: red;"></span>
									</td>
									<td width="28%"><span id="vst_usac_amount4Tip"></span></td>
								</tr>
								<tr>
									<td class="tright" width="20%">调整系数：</td>
									<td width="50%">
										<input class="list-input" id="vst_usac_modulus4" name="vst_usac_modulus" value="1" />
										<span class="red">*</span>
									</td>
									<td width="28%"><span id="vst_usac_modulus4Tip"></span></td>
								</tr>
							</table>
		                ...
		                <form id="batchUpdateBoxForm" method="post" enctype="multipart/form-data">
							<table class="line03">
								<tr>
							 		<td class="tright" width="20%">调整系数：</td>
							 		<td width="50%">
							 			<input class="list-input" id="vst_usac_modulus1" name="vst_usac_modulus" value="0.35" />
							 			<span class="red">*</span>
							 		</td>
							 		<td width="28%"><span id="vst_usac_modulus1Tip"></span></td>
							 	</tr>
							</table>
							<div style="padding-bottom:10px; margin-right:10px; text-align:right;">
		                ...
		                <form id="importBoxForm" method="post" enctype="multipart/form-data">
							<table class="line03">
								<tr>
							 		...
							 		<td width="50%">
							 			<input type="text" class="list-input" id="vst_usac_date" name="vst_usac_date"
											placeholder="yyyy-MM-dd" readonly="readonly" />
							 		</td>
							 		<td width="28%"><span id="vst_usac_dateTip"></span></td>
							 	</tr>
							</table>
							<div style="padding-bottom:10px; margin-right:10px; text-align:right;">
			...
			<div class="content-roll">
				<form action="${ctx}/innerUserSecondaryActiveChannel/findInnerUserSecondaryActiveChannel.action" id="listForm" method="post">
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<select class="list-input1" id="vst_usac_state" name="vst_usac_state">
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_usac_updator" name="vst_usac_updator" />
											</li>
										</ul>
									</div>
	...
	<script type="text/javascript">
		...
		laydate.render({
	  	...
	  	$("#vst_usac_date").val(getNDateTime("yyyy-MM-dd",0));
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#vst_usac_updator").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		...
		if(startDay == '' || endDay == ''){
			...
			if(confirm("确认要删除当前选定条件的数据吗？")){
		        var params = "startDay="+startDay+"&endDay="+endDay
						+"&vst_usac_channel="+$("#vst_usac_channel").val()
						+"&vst_usac_state="+$("#vst_usac_state").val()
						+"&vst_usac_updator="+$("#vst_usac_updator").val()
		        ...
		        $.ajax({
					...
					async : true,
					cache : false,
					url:"${ctx}/innerUserSecondaryActiveChannel/ajaxDeleteData.action",
					dataType:"json",
	...
	function button_audit(){
		...
		if(confirm("您确定要审核所选的"+modifyChecked.split(',').length+"条记录吗？")){
			...
			$.ajax({
				...
				async : true,
				cache : false,
				url:"${ctx}/innerUserSecondaryActiveChannel/ajaxAuditData.action",
				dataType:"json",
				success:function(msg){
					if(msg.code == 100){
		...
		if(startDay == '' || endDay == ''){
			...
			if(confirm("确认要审核当前选定条件的数据吗？")){
		        ...
		        $.ajax({
					...
					async : true,
					cache : false,
					url:"${ctx}/innerUserSecondaryActiveChannel/ajaxBatchAuditData.action",
					dataType:"json",
					success:function(msg){
						if(msg.code == 100){
	...
	$(document).ready(function(){
		...
		$("#vst_usac_modulus1").formValidator({
			validatorgroup : "1",
	  		onshow : "请填写调整系数！",
			...
			fun :function(){
				var patrn = /^\d+(\.\d{1,2})?$/;
	            if(!patrn.exec($("#vst_usac_modulus1").val())){
		            return false;
	            }
	            return true;
			},
		...
		$("#batchUpdateSubmit").click(function(){
			...
			if(flag){
				var modulus = $("#vst_usac_modulus1").val();
				var recordId = $("#recordId").val();
				var moduleId = $("#moduleId").val();
				$.ajax({
					url:"${ctx}/innerUserSecondaryActiveChannel/ajaxBatchUpdate.action",
					type:"POST",
					dataType:"json",
					cache:false,
	...
	$(document).ready(function(){
		...
		$("#importSubmit").click(function(){
			...
			if(flag){
				var date = $("#vst_usac_date").val();
                var $btn = $(this).button('loading');
				$.ajax({
					url:"${ctx}/innerUserSecondaryActiveChannel/ajaxImportData.action",
					type:"POST",
					dataType:"json",
					cache:false,
	...
	$(document).ready(function(){
		...
		$("#syncSubmit").click(function(){
			...
			if(flag){
                ...
                var $btn = $(this).button('loading');
				$.ajax({
					url:"${ctx}/innerUserSecondaryActiveChannel/ajaxSyncData.action",
					type:"POST",
					dataType:"json",
					cache:false,
	...
	$(document).ready(function(){
		...
		$("#vst_usac_amount4").formValidator({
			validatorgroup : "4",
			onshow : "请填写二次活跃用户！",
			onfocus : "二次活跃用户不能为空，必须为数字！",
			oncorrect : "验证通过！"
		}).inputValidator({
			min : 1,
			max : 13,
			onerror : "二次活跃用户长度在1~13位！"
		}).regexValidator({
			regexp :"^[0-9]{1,}$",
			datatype :"string",
		...
		$("#vst_usac_modulus4").formValidator({
			validatorgroup : "4",
			onshow : "请填写调整系数！",
			onfocus : "调整系数不能为空，必须为数字！",
			oncorrect : "验证通过！"
			...
			fun :function(){
				var patrn = /^\d+(\.\d{1,2})?$/;
				if(!patrn.exec($("#vst_usac_modulus4").val())){
					return false;
				}
				return true;
			},
		...
		// 失去焦点，计算新的二次活跃用户
		$("#vst_usac_amount4").blur(function(){
			var amount = $("#vst_usac_amount4").val();
			var modulus = $("#vst_usac_modulus4").val();
			if(amount != '' && modulus != ''){
		...
		$("#editSubmit").click(function(){
			...
			if(flag){
				...
				var moduleId = $("#moduleId").val();
				$.ajax({
					url:"${ctx}/innerUserSecondaryActiveChannel/ajaxEditData.action",
					type:"POST",
					dataType:"json",
					cache:false,
	...
	function ajaxGetReport(){
		...
		params += "&vst_usac_state=" + $("#vst_usac_state").val();
		params += "&vst_usac_updator=" + $("#vst_usac_updator").val();
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/innerUserSecondaryActiveChannel/ajaxGetReportByDate.action",
			dataType:"json",
			success:function(msg){
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_usac_amount[i] = n.vst_usac_amount;
		});
	...
	function ajaxGetReportByDim(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/innerUserSecondaryActiveChannel/ajaxGetReportByDim.action",
			dataType:"json",
			success:function(msg){
	...
	function pie(msg){
		...
		$.each(top20data,function(i,n){
			series +="{name:\""+getValue(n["value"])+"："+n.vst_usac_amount+"\",y:"+n.vst_usac_amount+"},";
		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/innerUserSecondaryActiveChannel/ajaxGetCutPage.action",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_usac_channel' id='vst_usac_channel2' order='desc' style='color:orange;'>渠道</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_usac_amount' id='vst_usac_amount2' order='desc'>二次活跃用户</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_usac_dod' id='vst_usac_dod2' order='desc'>天环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_usac_wow' id='vst_usac_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_usac_mom' id='vst_usac_mom2' order='desc'>月环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_usac_modulus' id='vst_usac_modulus2' order='desc'>调整系数</a></th>";
		table += "<th width='15%'>二次活跃用户*调整系数</th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_usac_state' id='vst_usac_state2' order='desc'>审核状态</a></th>";
		...
		$.each(data,function(i,n){
			...
			// 审核状态
			var vst_usac_state = n.vst_usac_state;
			if(n.vst_usac_state == 1){
				vst_usac_state = "<span class='red'>未审核<span>";
			}else if(n.vst_usac_state == 2){
				vst_usac_state = "<span class='green'>已审核<span>";
			}
			...
			table += "<td><input type='checkbox' id='recordIds' name='recordIds' "
							+ "value='" + n.vst_usac_id + "' "
							+ "state='" + n.vst_usac_state + "' "
							+ "modulus='" + n.vst_usac_modulus + "' "
							+ " /></td>";
			table += "<td>"+getDateWeek(n.vst_usac_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='同步时间："+formatDate(n.vst_usac_synctime, "yyyy-MM-dd HH:mm:ss")
						+"<br>新增时间："+toString(n.vst_usac_addtime)
						+"<br>创建人："+toString(n.vst_usac_creator)
						+"<br>修改时间："+toString(n.vst_usac_uptime)
						+"<br>修改人："+toString(n.vst_usac_updator)
						+"<br>备注："+toString(n.vst_usac_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_usac_channel)+"</td>";
			table += "<td>"+formatNumber(n.vst_usac_amount)+"</td>";
			...
			table += "<td class='myClick'>"+toString(n.vst_usac_modulus)+"</td>";
			table += "<td>"+formatNumber(Math.round(n.vst_usac_amount * n.vst_usac_modulus))+"</td>";
			table += "<td>"+toString(vst_usac_state)+"</td>";
	...
	function button_export(){
		...
		if(!isNotEmpty(startDay) || !isNotEmpty(endDay) || getDateDiff(startDay, endDay, 'day') > 92){
			...
			location.href = "${ctx}/innerUserSecondaryActiveChannel/exportData.action?" + params
					+ "&moduleId=" + $("#moduleId").val();
		}
	}
</script>
