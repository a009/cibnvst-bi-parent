...
<div class="wrapper">
	...
	<div class="content-wrapper" style="background-color: #ecf0f5;">
		<div class="content-roll">
			<form action="${ctx}/outerUserSecondaryActiveChannel/findOuterUserSecondaryActiveChannel.action" id="listForm" method="post">
...
<script type="text/javascript">
	...
	function ajaxGetReport(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/outerUserSecondaryActiveChannel/ajaxGetReportByDate.action",
			dataType:"json",
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/outerUserSecondaryActiveChannel/ajaxGetCutPage.action",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_usac_channel' id='vst_usac_channel2' order='desc'>渠道</a></th>";
		...
		$.each(data,function(i,n){
			...
			table += "<td>"+toString(n.vst_usac_channel)+"</td>";
	...
	function button_export(){
		...
		if(!isNotEmpty(startDay) || !isNotEmpty(endDay) || getDateDiff(startDay, endDay, 'day') > 92){
			...
			location.href = "${ctx}/outerUserSecondaryActiveChannel/exportData.action?" + params
