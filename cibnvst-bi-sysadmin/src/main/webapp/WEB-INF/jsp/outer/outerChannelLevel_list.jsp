	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
			<div class="content-roll">
				<form action="${ctx}/outerChannelLevel/findOuterChannelLevel.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 市场数据 > 渠道订单(对外)统计
...
<script type="text/javascript">
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/outerChannelLevel/ajaxGetCutPage.action",
			dataType:"json",
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_cl_date' id='vst_cl_date2' order='desc'>日期</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_cl_channel' id='vst_cl_channel2' order='desc'>渠道</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_cl_orders' id='vst_cl_orders2' order='desc'>订单数</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_cl_orders_amount' id='vst_cl_orders_amount2' order='desc'>订单金额(分)</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_cl_orders_price' id='vst_cl_orders_price2' order='desc'>客单价(分)</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_cl_orders_price_avg' id='vst_cl_orders_price_avg2' order='desc'>均价(分)</a></th>";
		...
		$.each(data,function(i,n){
			...
			table += "<td>"+toString(n.vst_cl_channel)+"</td>";
	...
	function button_export(){
		...
		if(!isNotEmpty(startDay) || !isNotEmpty(endDay) || getDateDiff(startDay, endDay, 'day') > 92){
			...
			location.href = "${ctx}/outerChannelLevel/exportData.action?" + params
