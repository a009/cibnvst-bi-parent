...
<script type="text/javascript">
	$(document).ready(function(){
		...
		button_getdata();
		//文本框，按回车触发查询
		$("#vst_user_total_dim_value1").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_user_total_dim_value2").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
    	...
    	//维度1为渠道的时候，维度2显示
		$("#vst_user_total_dim_name1").change(function(){
			var dim = $("#vst_user_total_dim_name1").val();
			if(dim == "渠道"){
				$(".dim2Condition").show();
			}else{
				$(".dim2Condition").hide();
				$("#vst_user_total_dim_name2").val("");
				$("#vst_user_total_dim_value2").val("");
			}
		});
	});
	...
	function button_getdata(){
		if($("#startDay").val() == '' || $("#endDay").val() == ''){
			...
			if($("#vst_user_total_dim_name1").val() == "全部"){
				$("#barChartBlock").hide();
				$("#pieChartBlock").hide();
				$("#mapChartBlock").hide();
			}else if($("#vst_user_total_dim_name1").val() == "省份"){
				$("#barChartBlock").hide();
				$("#pieChartBlock").show();
				$("#mapChartBlock").show();
	...
	function ajaxGetReport(){
		...
		params += "&vst_user_total_dim_name1=" + $("#vst_user_total_dim_name1").val();
		params += "&vst_user_total_dim_value1=" + $("#vst_user_total_dim_value1").val();
		params += "&vst_user_total_dim_name2=" + $("#vst_user_total_dim_name2").val();
		params += "&vst_user_total_dim_value2=" + $("#vst_user_total_dim_value2").val();
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_total_line",
			dataType:"json",
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_user_total[i] = n.vst_user_total;
		});
	...
	function ajaxGetReportByDimValue(){
		...
		if($("#vst_user_total_dim_name1").val() == "省份"){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_total_dimValue",
			dataType:"json",
			success:function(msg){
				if(msg.code == 100){
	...
	function pie(msg){
		...
		if(data.length>20){
   		...
   		$.each(top20data,function(i,n){
   			series +="{name:\""+getValue(n["value"])+"："+n.vst_user_total+"\",y:"+n.vst_user_total+"},";
   		});
	...
	function chinamap(msg){
        ...
        var data_province = "[";
   		$.each(data,function(i,n){
   			data_province+="{value:"+n.vst_user_total+",text:\""+n.vst_user_total+"\","+"name:\""+n.value+"\",code:\""+n.code+"\",enname:\""+n.enname+"\"},";
   			if(n.vst_user_total > maxTotal){
   	   			maxTotal = n.vst_user_total;
   	   		}
   		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=user_total_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_user_total_date' id='vst_user_total_date2' order='desc'>日期</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_user_total_dim_name1' id='vst_user_total_dim_name12' order='desc'>维度名</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_user_total_dim_value1' id='vst_user_total_dim_value12' order='desc'>维度值</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_user_total_dim_name2' id='vst_user_total_dim_name22' order='desc'>维度名2</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_user_total_dim_value2' id='vst_user_total_dim_value22' order='desc'>维度值2</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_user_total' id='vst_user_total2' order='desc'>累计用户数</a> | <a onclick='changeOrder(this)' orderBy='vst_user_total_dod' id='vst_user_total_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_user_total_wow' id='vst_user_total_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_user_total_uptime' id='vst_user_total_uptime2' order='desc'>更新时间</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_user_total_operator' id='vst_user_total_operator2' order='desc'>操作人</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_user_total_dod)<0){
				totalDodColor = "red";
			}
			if(parseFloat(n.vst_user_total_wow)<0){
				totalWowColor = "red";
			}
			...
			if(typeof(n.vst_user_total_date)=="undefined")	table += "<td></td>";
			else	table += "<td>"+getDateWeek(n.vst_user_total_date+'', 'yyyyMMdd')+"</td>";
			if(typeof(n.vst_user_total_dim_name1)=="undefined")	table += "<td></td>";
			else	table += "<td>"+n.vst_user_total_dim_name1+"</td>";
			if(typeof(n.vst_user_total_dim_value1)=="undefined")	table += "<td></td>";
			else	table += "<td>"+n.vst_user_total_dim_value1+"</td>";
			if(typeof(n.vst_user_total_dim_name2)=="undefined")	table += "<td></td>";
			else	table += "<td>"+n.vst_user_total_dim_name2+"</td>";
			if(typeof(n.vst_user_total_dim_value2)=="undefined")	table += "<td></td>";
			else	table += "<td>"+n.vst_user_total_dim_value2+"</td>";
			
			if(typeof(n.vst_user_total)=="undefined")	table += "<td> | ";
			else	table += "<td>"+formatNumber(n.vst_user_total)+" | ";
			if(typeof(n.vst_user_total_dod)=="undefined")	table += "| ";
			else	table += "<span class='"+totalDodColor+"'>"+n.vst_user_total_dod+"</span> | ";
			if(typeof(n.vst_user_total_wow)=="undefined")	table += "</td>";
			else	table += "<span class='"+totalWowColor+"'>"+n.vst_user_total_wow+"</span></td>";
			
			if(typeof(n.vst_user_total_uptime)=="undefined")	table += "<td></td>";
			else	table += "<td>"+formatDate(n.vst_user_total_uptime, "yyyy-MM-dd HH:mm:ss")+"</td>";
			if(typeof(n.vst_user_total_operator)=="undefined")	table += "<td></td>";
			else	table += "<td>"+n.vst_user_total_operator+"</td>";
	...
	function button_export(){
		...
		if(!isNotEmpty(startDay) || !isNotEmpty(endDay) || getDateDiff(startDay, endDay, 'day') > 92){
			...
			$.ajax({
				...
				async : true,
				cache : false,
				url:"${ctx}/report/json?code=user_total_export",
				dataType:"json",
				success:function(msg){
					if(msg.code == 100){
						location.href= "${ctx}/userTotal/exportData.action?data="+JSON.stringify(msg.data)
	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
			<div class="content-roll">
				<form action="${ctx}/userTotal/findUserTotal.action" id="listForm" method="post">
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<select class="list-input1" id="vst_user_total_dim_name1" name="vst_user_total_dim_name1">
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_user_total_dim_value1" name="vst_user_total_dim_value1" />
											</li>
											<li class="dim2Condition" style="display: none;">
												<span class="sertitle">维度2：</span>
												<select class="list-input1" id="vst_user_total_dim_name2" name="vst_user_total_dim_name2">
											...
											<li class="dim2Condition" style="display: none;">
												...
												<input class="list-input1" type="text" id="vst_user_total_dim_value2" name="vst_user_total_dim_value2" />
											</li>
										</ul>
									</div>
