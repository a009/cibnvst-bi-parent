	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">运营统计 > 汇总播放统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/movieTotalPlay/findMovieTotalPlay.action" id="listForm" method="post">
...
<script type="text/javascript">
	...
	function ajaxGetReport(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=movie_total_play_line",
			dataType:"json",
	...
	function line(msg){
		...
		var vst_mtp_vv = [];
		var vst_mtp_uv = [];
		var vst_mtp_playtime = [];
		var vst_mtp_playtime_avg = [];
		var vst_mtp_rate = [];

		$.each(data,function(i,n){
			xaxis[i] = getDateWeek(n.date+'', 'yyyyMMdd');
			vst_mtp_vv[i] = n.vst_mtp_vv;
			vst_mtp_uv[i] = n.vst_mtp_uv;
			vst_mtp_playtime[i] = toHour(n.vst_mtp_playtime*1000);
			vst_mtp_playtime_avg[i] = toMinute(n.vst_mtp_playtime_avg*1000);
			vst_mtp_rate[i] = n.vst_mtp_rate;
		});
		...
		$("#line-chart").highcharts({
			...
			series : [
			{
				...
				name : '总播放时长(小时)',
				data : vst_mtp_playtime,
				visible : false
			},{
				name : '人均播放时长(分钟)',
				data : vst_mtp_playtime_avg,
				visible : false
			},{
				name : '播放活跃比(%)',
				data : vst_mtp_rate,
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=movie_total_play_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_mtp_date' id='vst_mtp_date2' order='desc' style='color:orange'>日期</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_mtp_vv' id='vst_mtp_vv2' order='desc'>播放量</a> | <a onclick='changeOrder(this)' orderBy='vst_mtp_dod' id='vst_mtp_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_mtp_wow' id='vst_mtp_wow2' order='desc'>周环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_mtp_uv' id='vst_mtp_uv2' order='desc'>独立用户</a> | <a onclick='changeOrder(this)' orderBy='vst_mtp_uv_dod' id='vst_mtp_uv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_mtp_uv_wow' id='vst_mtp_uv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_mtp_avg' id='vst_mtp_avg2' order='desc'>人均播放量</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_mtp_playtime' id='vst_mtp_playtime2' order='desc'>播放时长(小时)</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_mtp_playtime_avg' id='vst_mtp_playtime_avg2' order='desc'>人均播放时长(分钟)</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_mtp_rate' id='vst_mtp_rate2' order='desc'>播放活跃比</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_mtp_dod)<0){
				vvDodColor = "red";
			}
			if(parseFloat(n.vst_mtp_wow)<0){
				vvWowColor = "red";
			}
			if(parseFloat(n.vst_mtp_uv_dod)<0){
				uvDodColor = "red";
			}
			if(parseFloat(n.vst_mtp_uv_wow)<0){
				uvWowColor = "red";
			}
			...
			table += "<td data-title='新增时间："+formatDate(n.vst_mtp_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_mtp_creator)
						+"<br>修改时间："+formatDate(n.vst_mtp_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_mtp_updator)
						+"<br>备注："+toString(n.vst_mtp_summary)+"' class='enlargeTextTitle'>"
					+getDateWeek(n.vst_mtp_date+'', 'yyyyMMdd')+"</td>";
			//播放量
			table += "<td>"+formatNumber(n.vst_mtp_vv)+" | ";
			table += "<span class='"+vvDodColor+"'>"+toString(n.vst_mtp_dod)+"</span> | ";
			table += "<span class='"+vvWowColor+"'>"+toString(n.vst_mtp_wow)+"</span></td>";
			//播放独立用户
			table += "<td>"+formatNumber(n.vst_mtp_uv)+" | ";
			table += "<span class='"+uvDodColor+"'>"+toString(n.vst_mtp_uv_dod)+"</span> | ";
			table += "<span class='"+uvWowColor+"'>"+toString(n.vst_mtp_uv_wow)+"</span></td>";
			//人均播放量
			table += "<td>"+formatNumber(n.vst_mtp_avg)+"</td>";
			//播放时长
			table += "<td>"+toHour(n.vst_mtp_playtime*1000)+"</td>";
			table += "<td>"+toMinute(n.vst_mtp_playtime_avg*1000)+"</td>";
			//播放活跃比
			table += "<td>"+toString(n.vst_mtp_rate)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					location.href = "${ctx}/movieTotalPlay/exportData.action?" + params
