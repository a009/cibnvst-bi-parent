	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">运营统计 > 影片曝光汇总统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/movieExposureSum/findMovieExposureSum.action" id="listForm" method="post">
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<select class="list-input1" id="vst_mes_special_type" name="vst_mes_special_type">
											...
											<li>
												...
												<select class="list-input1" id="vst_mes_cid" name="vst_mes_cid">
													<option value="">请选择</option>
...
<script type="text/javascript">
	...
	function ajaxGetReport(){
		...
		params += "&vst_mes_special_type=" + $("#vst_mes_special_type").val();
		params += "&vst_mes_cid=" + $("#vst_mes_cid").val();
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=movie_exposure_sum_line",
			dataType:"json",
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_mes_amount[i] = n.vst_mes_amount;
		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=movie_exposure_sum_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_mes_date' id='vst_mes_date2' order='desc' style='color:orange;'>日期</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_mes_special_type' id='vst_mes_special_type2' order='desc'>专区类型</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_mes_cid' id='vst_mes_cid2' order='desc'>分类</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_mes_amount' id='vst_mes_amount2' order='desc'>曝光量</a> | <a onclick='changeOrder(this)' orderBy='vst_mes_amount_dod' id='vst_mes_amount_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_mes_amount_wow' id='vst_mes_amount_wow2' order='desc'>周环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_mes_uv' id='vst_mes_uv2' order='desc'>独立用户</a> | <a onclick='changeOrder(this)' orderBy='vst_mes_uv_dod' id='vst_mes_uv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_mes_uv_wow' id='vst_mes_uv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_mes_avg' id='vst_mes_avg2' order='desc'>人均曝光量</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_mes_amount_dod)<0){
				amountDodColor = "red";
			}
			if(parseFloat(n.vst_mes_amount_wow)<0){
				amountWowColor = "red";
			}
			if(parseFloat(n.vst_mes_uv_dod)<0){
				uvDodColor = "red";
			}
			if(parseFloat(n.vst_mes_uv_wow)<0){
				uvWowColor = "red";
			}
			...
			table += "<td data-title='新增时间："+formatDate(n.vst_mes_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_mes_creator)
						+"<br>修改时间："+formatDate(n.vst_mes_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_mes_updator)
						+"<br>备注："+toString(n.vst_mes_summary)+"' class='enlargeTextTitle'>"
					+getDateWeek(n.vst_mes_date+'', 'yyyyMMdd')+"</td>";
			table += "<td>"+getMapValue('${specialTypes}', n.vst_mes_special_type)+"</td>";
			table += "<td>"+getMapValue('${classifys}', n.vst_mes_cid)+"</td>";
			//曝光量
			table += "<td>"+formatNumber(n.vst_mes_amount)+" | ";
			table += "<span class='"+amountDodColor+"'>"+toString(n.vst_mes_amount_dod)+"</span> | ";
			table += "<span class='"+amountWowColor+"'>"+toString(n.vst_mes_amount_wow)+"</span></td>";
			//独立用户
			table += "<td>"+formatNumber(n.vst_mes_uv)+" | ";
			table += "<span class='"+uvDodColor+"'>"+toString(n.vst_mes_uv_dod)+"</span> | ";
			table += "<span class='"+uvWowColor+"'>"+toString(n.vst_mes_uv_wow)+"</span></td>";
			//人均曝光量
			table += "<td>"+formatNumber(n.vst_mes_avg)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					location.href = "${ctx}/movieExposureSum/exportData.action?" + params
