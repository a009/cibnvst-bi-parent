	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">运营统计 > 影片搜索统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/movieSearch/findMovieSearch.action" id="listForm" method="post">
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<select class="list-input1" id="vst_mos_special_type" name="vst_mos_special_type">
											...
											<li>
												...
												<select class="list-input1" id="vst_mos_cid" name="vst_mos_cid">
													<option value="">请选择</option>
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_mos_uuid" name="vst_mos_uuid" />
											</li>
											<li>
												<span class="sertitle">影片名称：</span>
												<input class="list-input1" type="text" id="vst_mos_name" name="vst_mos_name" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(function(){
		...
		//文本框，按回车触发查询
		$("#vst_mos_uuid").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_mos_name").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		params += "&vst_mos_special_type=" + $("#vst_mos_special_type").val();
		params += "&vst_mos_cid=" + $("#vst_mos_cid").val();
		params += "&vst_mos_uuid=" + $("#vst_mos_uuid").val();
		// 参数特殊化处理
		params += "&" + paramFormat("vst_mos_name", $("#vst_mos_name").val());
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=movie_search_line",
			dataType:"json",
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_mos_amount[i] = n.vst_mos_amount;
		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=movie_search_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_mos_date' id='vst_mos_date2' order='desc'>日期</a></th>";
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_mos_special_type' id='vst_mos_special_type2' order='desc'>专区类型</a></th>";
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_mos_cid' id='vst_mos_cid2' order='desc'>分类</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_mos_uuid' id='vst_mos_uuid2' order='desc' style='color:orange;'>影片ID</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_mos_name' id='vst_mos_name2' order='desc'>影片名称</a></th>";
		
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_mos_amount' id='vst_mos_amount2' order='desc'>搜索量</a> | <a onclick='changeOrder(this)' orderBy='vst_mos_amount_dod' id='vst_mos_amount_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_mos_amount_wow' id='vst_mos_amount_wow2' order='desc'>周环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_mos_uv' id='vst_mos_uv2' order='desc'>独立用户</a> | <a onclick='changeOrder(this)' orderBy='vst_mos_uv_dod' id='vst_mos_uv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_mos_uv_wow' id='vst_mos_uv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_mos_avg' id='vst_mos_avg2' order='desc'>人均搜索量</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_mos_amount_dod)<0){
				amountDodColor = "red";
			}
			if(parseFloat(n.vst_mos_amount_wow)<0){
				amountWowColor = "red";
			}
			if(parseFloat(n.vst_mos_uv_dod)<0){
				uvDodColor = "red";
			}
			if(parseFloat(n.vst_mos_uv_wow)<0){
				uvWowColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_mos_date+'', 'yyyyMMdd')+"</td>";
			table += "<td>"+getMapValue('${specialTypes}', n.vst_mos_special_type)+"</td>";
			table += "<td>"+getMapValue('${classifys}', n.vst_mos_cid)+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_mos_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_mos_creator)
						+"<br>修改时间："+formatDate(n.vst_mos_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_mos_updator)
						+"<br>备注："+toString(n.vst_mos_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_mos_uuid)+"</td>";
			table += "<td title='"+n.vst_mos_name+"'>"+formatString(n.vst_mos_name)+"</td>";
			//搜索量
			table += "<td>"+formatNumber(n.vst_mos_amount)+" | ";
			table += "<span class='"+amountDodColor+"'>"+toString(n.vst_mos_amount_dod)+"</span> | ";
			table += "<span class='"+amountWowColor+"'>"+toString(n.vst_mos_amount_wow)+"</span></td>";
			//独立用户
			table += "<td>"+formatNumber(n.vst_mos_uv)+" | ";
			table += "<span class='"+uvDodColor+"'>"+toString(n.vst_mos_uv_dod)+"</span> | ";
			table += "<span class='"+uvWowColor+"'>"+toString(n.vst_mos_uv_wow)+"</span></td>";
			//人均搜索量
			table += "<td>"+formatNumber(n.vst_mos_avg)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&vst_mos_cid=" + $("#vst_mos_cid").val();
					params += "&vst_mos_uuid=" + $("#vst_mos_uuid").val();
					params += "&vst_mos_name=" + encodeURI(encodeURI($("#vst_mos_name").val()));
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					
					location.href = "${ctx}/movieSearch/exportData.action?" + params
