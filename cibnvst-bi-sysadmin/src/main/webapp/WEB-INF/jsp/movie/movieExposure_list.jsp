	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">运营统计 > 影片曝光统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/movieExposure/findMovieExposure.action" id="listForm" method="post">
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<select class="list-input1" id="vst_me_special_type" name="vst_me_special_type">
											...
											<li>
												...
												<select class="list-input1" id="vst_me_cid" name="vst_me_cid">
													<option value="">请选择</option>
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_me_uuid" name="vst_me_uuid" />
											</li>
											<li>
												<span class="sertitle">影片名称：</span>
												<input class="list-input1" type="text" id="vst_me_name" name="vst_me_name" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(function(){
		...
		//文本框，按回车触发查询
		$("#vst_me_uuid").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_me_name").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		params += "&vst_me_special_type=" + $("#vst_me_special_type").val();
		params += "&vst_me_cid=" + $("#vst_me_cid").val();
		params += "&vst_me_uuid=" + $("#vst_me_uuid").val();
		// 参数特殊化处理
		params += "&" + paramFormat("vst_me_name", $("#vst_me_name").val());
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=movie_exposure_line",
			dataType:"json",
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_me_amount[i] = n.vst_me_amount;
		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=movie_exposure_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_me_date' id='vst_me_date2' order='desc'>日期</a></th>";
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_me_special_type' id='vst_me_special_type2' order='desc'>专区类型</a></th>";
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_me_cid' id='vst_me_cid2' order='desc'>分类</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_me_uuid' id='vst_me_uuid2' order='desc' style='color:orange;'>影片ID</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_me_name' id='vst_me_name2' order='desc'>影片名称</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_me_amount' id='vst_me_amount2' order='desc'>曝光量</a> | <a onclick='changeOrder(this)' orderBy='vst_me_amount_dod' id='vst_me_amount_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_me_amount_wow' id='vst_me_amount_wow2' order='desc'>周环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_me_uv' id='vst_me_uv2' order='desc'>独立用户</a> | <a onclick='changeOrder(this)' orderBy='vst_me_uv_dod' id='vst_me_uv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_me_uv_wow' id='vst_me_uv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_me_avg' id='vst_me_avg2' order='desc'>人均曝光量</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_me_amount_dod)<0){
				amountDodColor = "red";
			}
			if(parseFloat(n.vst_me_amount_wow)<0){
				amountWowColor = "red";
			}
			if(parseFloat(n.vst_me_uv_dod)<0){
				uvDodColor = "red";
			}
			if(parseFloat(n.vst_me_uv_wow)<0){
				uvWowColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_me_date+'', 'yyyyMMdd')+"</td>";
			table += "<td>"+getMapValue('${specialTypes}', n.vst_me_special_type)+"</td>";
			table += "<td>"+getMapValue('${classifys}', n.vst_me_cid)+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_me_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_me_creator)
						+"<br>修改时间："+formatDate(n.vst_me_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_me_updator)
						+"<br>备注："+toString(n.vst_me_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_me_uuid)+"</td>";
			table += "<td title='"+n.vst_me_name+"'>"+formatString(n.vst_me_name)+"</td>";
			//曝光量
			table += "<td>"+formatNumber(n.vst_me_amount)+" | ";
			table += "<span class='"+amountDodColor+"'>"+toString(n.vst_me_amount_dod)+"</span> | ";
			table += "<span class='"+amountWowColor+"'>"+toString(n.vst_me_amount_wow)+"</span></td>";
			//独立用户
			table += "<td>"+formatNumber(n.vst_me_uv)+" | ";
			table += "<span class='"+uvDodColor+"'>"+toString(n.vst_me_uv_dod)+"</span> | ";
			table += "<span class='"+uvWowColor+"'>"+toString(n.vst_me_uv_wow)+"</span></td>";
			//人均曝光量
			table += "<td>"+formatNumber(n.vst_me_avg)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&vst_me_cid=" + $("#vst_me_cid").val();
					params += "&vst_me_uuid=" + $("#vst_me_uuid").val();
					params += "&vst_me_name=" + encodeURI(encodeURI($("#vst_me_name").val()));
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					
					location.href = "${ctx}/movieExposure/exportData.action?" + params
