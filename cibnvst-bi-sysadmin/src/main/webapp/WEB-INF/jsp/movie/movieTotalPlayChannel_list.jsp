	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">运营统计 > 汇总播放(渠道)统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/movieTotalPlayChannel/findMovieTotalPlayChannel.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 运营统计 > 汇总播放(渠道)统计
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_mtpc_channel" name="vst_mtpc_channel" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//文本框，按回车触发查询
		$("#vst_mtpc_channel").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		// 参数特殊化处理
		params += "&" + paramFormat("vst_mtpc_channel", $("#vst_mtpc_channel").val());

		var ajaxUrl;
		if($("#vst_mtpc_channel").val() == ''){
			ajaxUrl = "${ctx}/report/json?code=movie_total_play_channel_bar";
		}else{
			ajaxUrl = "${ctx}/report/json?code=movie_total_play_channel_line";
		}
	...
	function line(msg){
		...
		var vst_mtpc_vv = [];
		var vst_mtpc_uv = [];
		var vst_mtpc_playtime = [];
		var vst_mtpc_playtime_avg = [];
		var vst_mtpc_rate = [];

		$.each(data,function(i,n){
			xaxis[i] = getDateWeek(n.date+'', 'yyyyMMdd');
			vst_mtpc_vv[i] = n.vst_mtpc_vv;
			vst_mtpc_uv[i] = n.vst_mtpc_uv;
			vst_mtpc_playtime[i] = toHour(n.vst_mtpc_playtime*1000);
			vst_mtpc_playtime_avg[i] = toMinute(n.vst_mtpc_playtime_avg*1000);
			vst_mtpc_rate[i] = n.vst_mtpc_rate;
		});
		...
		$("#line-chart").highcharts({
			...
			series : [
			{
				...
				name : '总播放时长(小时)',
				data : vst_mtpc_playtime,
				visible : false
			},{
				name : '人均播放时长(分钟)',
				data : vst_mtpc_playtime_avg,
				visible : false
			},{
				name : '播放活跃比(%)',
				data : vst_mtpc_rate,
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=movie_total_play_channel_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_mtpc_date' id='vst_mtpc_date2' order='desc'>日期</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_mtpc_channel' id='vst_mtpc_channel2' order='desc' style='color:orange'>渠道</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_mtpc_vv' id='vst_mtpc_vv2' order='desc'>播放量</a> | <a onclick='changeOrder(this)' orderBy='vst_mtpc_dod' id='vst_mtpc_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_mtpc_wow' id='vst_mtpc_wow2' order='desc'>周环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_mtpc_uv' id='vst_mtpc_uv2' order='desc'>独立用户</a> | <a onclick='changeOrder(this)' orderBy='vst_mtpc_uv_dod' id='vst_mtpc_uv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_mtpc_uv_wow' id='vst_mtpc_uv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_mtpc_avg' id='vst_mtpc_avg2' order='desc'>人均播放量</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_mtpc_playtime' id='vst_mtpc_playtime2' order='desc'>播放时长(小时)</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_mtpc_playtime_avg' id='vst_mtpc_playtime_avg2' order='desc'>人均播放时长(分钟)</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_mtpc_rate' id='vst_mtpc_rate2' order='desc'>播放活跃比</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_mtpc_dod)<0){
				vvDodColor = "red";
			}
			if(parseFloat(n.vst_mtpc_wow)<0){
				vvWowColor = "red";
			}
			if(parseFloat(n.vst_mtpc_uv_dod)<0){
				uvDodColor = "red";
			}
			if(parseFloat(n.vst_mtpc_uv_wow)<0){
				uvWowColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_mtpc_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_mtpc_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_mtpc_creator)
						+"<br>修改时间："+formatDate(n.vst_mtpc_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_mtpc_updator)
						+"<br>备注："+toString(n.vst_mtpc_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_mtpc_channel)+"</td>";
			//播放量
			table += "<td>"+formatNumber(n.vst_mtpc_vv)+" | ";
			table += "<span class='"+vvDodColor+"'>"+toString(n.vst_mtpc_dod)+"</span> | ";
			table += "<span class='"+vvWowColor+"'>"+toString(n.vst_mtpc_wow)+"</span></td>";
			//播放独立用户
			table += "<td>"+formatNumber(n.vst_mtpc_uv)+" | ";
			table += "<span class='"+uvDodColor+"'>"+toString(n.vst_mtpc_uv_dod)+"</span> | ";
			table += "<span class='"+uvWowColor+"'>"+toString(n.vst_mtpc_uv_wow)+"</span></td>";
			//人均播放量
			table += "<td>"+formatNumber(n.vst_mtpc_avg)+"</td>";
			//播放时长
			table += "<td>"+toHour(n.vst_mtpc_playtime*1000)+"</td>";
			table += "<td>"+toMinute(n.vst_mtpc_playtime_avg*1000)+"</td>";
			//播放活跃比
			table += "<td>"+toString(n.vst_mtpc_rate)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&pkgName=" + $("#pkgName").val();
					params += "&vst_mtpc_channel=" + $("#vst_mtpc_channel").val();
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					
					location.href = "${ctx}/movieTotalPlayChannel/exportData.action?" + params
