	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">运营统计 > 影片播放统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/moviePlay/findMoviePlay.action" id="listForm" method="post">
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<select class="list-input1" id="vst_mp_special_type" name="vst_mp_special_type">
											...
											<li>
												...
												<select class="list-input1" id="vst_mp_cid" name="vst_mp_cid">
													<option value="">请选择</option>
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_mp_uuid" name="vst_mp_uuid" />
											</li>
											<li>
												<span class="sertitle">影片名称：</span>
												<input class="list-input1" type="text" id="vst_mp_name" name="vst_mp_name" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//文本框，按回车触发查询
		$("#vst_mp_uuid").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_mp_name").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		params += "&vst_mp_special_type=" + $("#vst_mp_special_type").val();
		params += "&vst_mp_cid=" + $("#vst_mp_cid").val();
		params += "&vst_mp_uuid=" + $("#vst_mp_uuid").val();
		// 参数特殊化处理
		params += "&" + paramFormat("vst_mp_name", $("#vst_mp_name").val());
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=movie_play_line",
			dataType:"json",
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_mp_vv[i] = n.vst_mp_vv;
			vst_mp_playtime[i] = toHour(n.vst_mp_playtime*1000);
		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=movie_play_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_mp_date' id='vst_mp_date2' order='desc'>日期</a></th>";
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_mp_special_type' id='vst_mp_special_type2' order='desc'>专区类型</a></th>";
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_mp_cid' id='vst_mp_cid2' order='desc'>分类</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_mp_uuid' id='vst_mp_uuid2' order='desc' style='color:orange;'>影片ID</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_mp_name' id='vst_mp_name2' order='desc'>影片名称</a></th>";
		
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_mp_vv' id='vst_mp_vv2' order='desc'>播放量</a> | <a onclick='changeOrder(this)' orderBy='vst_mp_vv_dod' id='vst_mp_vv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_mp_vv_wow' id='vst_mp_vv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_mp_uv' id='vst_mp_uv2' order='desc'>独立用户</a> | <a onclick='changeOrder(this)' orderBy='vst_mp_uv_dod' id='vst_mp_uv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_mp_uv_wow' id='vst_mp_uv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_mp_avg' id='vst_mp_avg2' order='desc'>人均播放量</a></th>";
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_mp_playtime' id='vst_mp_playtime2' order='desc'>播放时长(小时)</a></th>";
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_mp_playtime_avg' id='vst_mp_playtime_avg2' order='desc'>人均播放时长(分钟)</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_mp_vv_wow)<0){
				vvWowColor = "red";
			}
			if(parseFloat(n.vst_mp_uv_dod)<0){
				uvDodColor = "red";
			}
			if(parseFloat(n.vst_mp_uv_wow)<0){
				uvWowColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_mp_date+'', 'yyyyMMdd')+"</td>";
			table += "<td>"+getMapValue('${specialTypes}', n.vst_mp_special_type)+"</td>";
			table += "<td>"+getMapValue('${classifys}', n.vst_mp_cid)+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_mp_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_mp_creator)
						+"<br>修改时间："+formatDate(n.vst_mp_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_mp_updator)
						+"<br>备注："+toString(n.vst_mp_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_mp_uuid)+"</td>";
			table += "<td title='"+n.vst_mp_name+"'>"+formatString(n.vst_mp_name)+"</td>";
			//播放量
			table += "<td>"+formatNumber(n.vst_mp_vv)+" | ";
			table += "<span class='"+vvDodColor+"'>"+toString(n.vst_mp_vv_dod)+"</span> | ";
			table += "<span class='"+vvWowColor+"'>"+toString(n.vst_mp_vv_wow)+"</span></td>";
			//独立用户
			table += "<td>"+formatNumber(n.vst_mp_uv)+" | ";
			table += "<span class='"+uvDodColor+"'>"+toString(n.vst_mp_uv_dod)+"</span> | ";
			table += "<span class='"+uvWowColor+"'>"+toString(n.vst_mp_uv_wow)+"</span></td>";
			//人均播放量
			table += "<td>"+formatNumber(n.vst_mp_avg)+"</td>";
			//播放时长
			table += "<td>"+toHour(n.vst_mp_playtime*1000)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&vst_mp_cid=" + $("#vst_mp_cid").val();
					params += "&vst_mp_uuid=" + $("#vst_mp_uuid").val();
					params += "&vst_mp_name=" + encodeURI(encodeURI($("#vst_mp_name").val()));
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					
					location.href = "${ctx}/moviePlay/exportData.action?" + params
