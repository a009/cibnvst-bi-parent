	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">运营统计 > 平台播放统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/movieSite/findMovieSite.action" id="listForm" method="post">
									...
									<div class="box-search">
										<c:choose>
											...
											<%--<li>
												...
												<select class="list-input1" id="vst_ms_special_type" name="vst_ms_special_type">
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_ms_site" name="vst_ms_site" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//文本框，按回车触发查询
		$("#vst_ms_site").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		// 参数特殊化处理
		params += "&" + paramFormat("vst_ms_site", $("#vst_ms_site").val());

		var ajaxUrl;
		if($("#vst_ms_site").val() == ''){
			if(startDay == endDay){
				ajaxUrl = "${ctx}/report/json?code=movie_site_pie";
			}else{
				ajaxUrl = "${ctx}/movieSite/ajaxGetReportBySite.action";
			}
		}else{
			ajaxUrl = "${ctx}/report/json?code=movie_site_line";
		}
	...
	function line(msg){
		...
		$.each(data,function(i,n){
			...
			vst_ms_vv[i] = n.vst_ms_vv;
			vst_ms_uv[i] = n.vst_ms_uv;
			vst_ms_playtime[i] = toHour(n.vst_ms_playtime*1000);
		});
	...
	function pie(msg){
		...
		if(data.length > 20){
   		...
   		$.each(top20data, function(i,n){
   			series +="{name:\""+n["value"]+"："+n.vst_ms_vv+"\",y:"+n.vst_ms_vv+"},";
   		});
	...
	function lineBySite(msg){
		var data = msg.data;
		var dataLength  = Object.keys(data.dateMap).length;
		var sites = data.sites;	// Top10平台
		...
		for(var i=0; i<sites.length; i++){
			...
			var json = {name : siteName, data : vvArr[i]};
			series.push(json);
		}
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=movie_site_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_ms_date' id='vst_ms_date2' order='desc'>日期</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_ms_site' id='vst_ms_site2' order='desc' style='color:orange'>平台名称</a></th>";
		
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_ms_vv' id='vst_ms_vv2' order='desc'>播放量</a> | <a onclick='changeOrder(this)' orderBy='vst_ms_vv_dod' id='vst_ms_vv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_ms_vv_wow' id='vst_ms_vv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_ms_uv' id='vst_ms_uv2' order='desc'>独立用户</a> | <a onclick='changeOrder(this)' orderBy='vst_ms_uv_dod' id='vst_ms_uv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_ms_uv_wow' id='vst_ms_uv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_ms_avg' id='vst_ms_avg2' order='desc'>人均播放量</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_ms_playtime' id='vst_ms_playtime2' order='desc'>播放时长(小时)</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_ms_playtime_avg' id='vst_ms_playtime_avg2' order='desc'>人均播放时长(分钟)</a></th>";
		...
		$.each(data,function(i,n){
			...
			if(parseFloat(n.vst_ms_vv_wow)<0){
				vvWowColor = "red";
			}
			if(parseFloat(n.vst_ms_uv_dod)<0){
				uvDodColor = "red";
			}
			if(parseFloat(n.vst_ms_uv_wow)<0){
				uvWowColor = "red";
			}
			...
			var vst_ms_pkg = n.vst_ms_pkg;
			var vst_ms_site = n.vst_ms_site;
			if(vst_ms_pkg == 'com.xiaowei.media' || vst_ms_pkg == 'com.xiaowei.media.ios'){
				if(vst_ms_site == ''){
					vst_ms_site = "直播平台";
				}
			}
			
			table += "<td>"+getDateWeek(n.vst_ms_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_ms_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_ms_creator)
						+"<br>修改时间："+formatDate(n.vst_ms_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_ms_updator)
						+"<br>备注："+toString(n.vst_ms_summary)+"' class='enlargeTextTitle'>"
					//+toString(n.vst_ms_site)+"</td>";
					+toString(vst_ms_site)+"</td>";
			//播放量
			table += "<td>"+formatNumber(n.vst_ms_vv)+" | ";
			table += "<span class='"+vvDodColor+"'>"+toString(n.vst_ms_vv_dod)+"</span> | ";
			table += "<span class='"+vvWowColor+"'>"+toString(n.vst_ms_vv_wow)+"</span></td>";
			//独立用户
			table += "<td>"+formatNumber(n.vst_ms_uv)+" | ";
			table += "<span class='"+uvDodColor+"'>"+toString(n.vst_ms_uv_dod)+"</span> | ";
			table += "<span class='"+uvWowColor+"'>"+toString(n.vst_ms_uv_wow)+"</span></td>";
			//人均播放量
			table += "<td>"+formatNumber(n.vst_ms_avg)+"</td>";
			//播放时长
			table += "<td>"+toHour(n.vst_ms_playtime*1000)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&pkgName=" + $("#pkgName").val();
					params += "&vst_ms_site=" + encodeURI(encodeURI($("#vst_ms_site").val()));
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					
					location.href = "${ctx}/movieSite/exportData.action?" + params
