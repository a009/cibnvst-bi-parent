...
<script type="text/javascript">
  	$(function(){
  		...
  		$("#vst_group_state").val($("#hidden_group_state").val());
  	...
  	// 新增
	function button_add(){
		$("#listForm").attr("action", "${ctx}/sqlGroup/toAdd.action");
		$("#listForm").submit();
	}
	...
	function button_edit(){
	    ...
	    if(modifyChecked==""){
	        ...
	        $("#listForm").attr("action", "${ctx}/sqlGroup/toEdit.action");
			$("#listForm").submit();
	    }
	}
	...
	function button_delete(){
	    ...
	    if(modifyChecked==""){
	    	...
	    	if(window.confirm("您确定要删除所选的"+modifyChecked.split(',').length+"条记录吗?")){
		    	$("#recordId").val(modifyChecked);
		        $("#listForm").attr("action", "${ctx}/sqlGroup/deleteSqlGroup.action");
				$("#listForm").submit();
			}
	    }
	}
	...
	function button_stop(){
		...
		if(confirm("您确定要禁用所选的"+modifyChecked.split(',').length+"条记录吗？")){
			$("#recordId").val(modifyChecked);
			$("#recordState").val(2);
			$("#listForm").attr("action", "${ctx}/sqlGroup/modifySqlGroupState.action");
			$("#listForm").submit();
		}
	}
	...
	function button_copyAdd(){
	    ...
	    if(modifyChecked==""){
	    	...
	    	$("#recordId").val(modifyChecked);
	        $("#listForm").attr("action", "${ctx}/sqlGroup/toCopyAdd.action");
			$("#listForm").submit();
	    }
	}
	...
	function makeIndex(){
		if($("#doIndex").attr("disabled") == undefined || $("#doIndex").attr("disabled") == ""){
			if(!checkIndexs()){
				...
				$.ajax({
					url:"${ctx}/sqlGroup/modifySqlGroupIndexs.action",
					type:"POST",
	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
			<div class="content-roll">
				<form action="${ctx}/sqlGroup/findSqlGroups.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 配置管理 > sql分组配置查询</span>
					</section>
								...
								<div class="box-body">
									<div class="box-search">
										<c:choose>
											...
											<li>
												<span class="sertitle">分组ID：</span>
												<input class="list-input1" type="text" name="vst_group_id" value="${cutPage._queryParam.vst_group_id }" />
											</li>
											<li>
											...
											<li>
												<span class="sertitle">分组字段名称：</span>
												<input class="list-input1" type="text" name="vst_group_name" value="${cutPage._queryParam.vst_group_name }" />
											</li>
											<li>
												<span class="sertitle">分组字段别名：</span>
												<input class="list-input1" type="text" name="vst_group_alias" value="${cutPage._queryParam.vst_group_alias }" />
											</li>
											<li>
												<span class="sertitle">状态：</span>
												<input type="hidden" id="hidden_group_state" value="${cutPage._queryParam.vst_group_state }" />
												<select class="list-input1" id="vst_group_state" name="vst_group_state">
													<option value="">请选择</option>
											...
											<li>
												<span class="sertitle">分组描述：</span>
												<input class="list-input1" type="text" name="vst_group_desc" value="${cutPage._queryParam.vst_group_desc }" />
											</li>
										</ul>
										<c:choose>
						    				<c:when test="${hidden_search=='1' || hidden_search==1}">
													...
													<div class="table-thead" style="width: 1820px;">
														<ul id="rowHead" style="background-color:#d2d6de;">
															<li style="width: 100px;">
																...
																<c:url value="../share/order.jsp" var="vst_group_id_order">
																    <c:param name="orderBy" value="vst_group_id"/>
																    <c:param name="fieldName" value="全选"/>
															    </c:url>
															    <c:import url="${vst_group_id_order}"/>
															</li>
															...
															<li style="width: 150px;">
																<c:url value="../share/order.jsp" var="vst_group_name_order">
																    <c:param name="orderBy" value="vst_group_name"/>
																    <c:param name="fieldName" value="分组字段名称"/>
															    </c:url>
															    <c:import url="${vst_group_name_order}"/>
															</li>
															<li style="width: 150px;">
																<c:url value="../share/order.jsp" var="vst_group_alias_order">
																    <c:param name="orderBy" value="vst_group_alias"/>
																    <c:param name="fieldName" value="分组字段别名"/>
															    </c:url>
															    <c:import url="${vst_group_alias_order}"/>
															</li>
															<li style="width: 250px;">
																<c:url value="../share/order.jsp" var="vst_group_desc_order">
																    <c:param name="orderBy" value="vst_group_desc"/>
																    <c:param name="fieldName" value="分组描述"/>
															    </c:url>
															    <c:import url="${vst_group_desc_order}"/>
															</li>
															<li style="width: 100px;">
																<c:url value="../share/order.jsp" var="vst_group_index_order">
																    <c:param name="orderBy" value="vst_group_index"/>
																    <c:param name="fieldName" value="排序"/>
															    </c:url>
															    <c:import url="${vst_group_index_order}"/>
															</li>
															<li style="width: 100px;">
																<c:url value="../share/order.jsp" var="vst_group_state_order">
																    <c:param name="orderBy" value="vst_group_state"/>
																    <c:param name="fieldName" value="状态"/>
															    </c:url>
															    <c:import url="${vst_group_state_order}"/>
															</li>
															<li style="width: 150px;">
																<c:url value="../share/order.jsp" var="vst_group_addtime_order">
																    <c:param name="orderBy" value="vst_group_addtime"/>
																    <c:param name="fieldName" value="新增时间"/>
															    </c:url>
															    <c:import url="${vst_group_addtime_order}"/>
															</li>
															<li style="width: 100px;">
																<c:url value="../share/order.jsp" var="vst_group_creator_order">
																    <c:param name="orderBy" value="vst_group_creator"/>
																    <c:param name="fieldName" value="创建人"/>
															    </c:url>
															    <c:import url="${vst_group_creator_order}"/>
															</li>
															<li style="width: 150px;">
																<c:url value="../share/order.jsp" var="vst_group_uptime_order">
																    <c:param name="orderBy" value="vst_group_uptime"/>
																    <c:param name="fieldName" value="修改时间"/>
															    </c:url>
															    <c:import url="${vst_group_uptime_order}"/>
															</li>
															<li style="width: 100px;">
																<c:url value="../share/order.jsp" var="vst_group_updator_order">
																    <c:param name="orderBy" value="vst_group_updator"/>
																    <c:param name="fieldName" value="修改人"/>
															    </c:url>
															    <c:import url="${vst_group_updator_order}"/>
															</li>
															<li style="width: 200px;">
																<c:url value="../share/order.jsp" var="vst_group_summary_order">
																    <c:param name="orderBy" value="vst_group_summary"/>
																    <c:param name="fieldName" value="备注"/>
															    </c:url>
															    <c:import url="${vst_group_summary_order}"/>
															</li>
														</ul>
													</div>
														...
														<ul style="width: 1820px;" <c:if test="${(stuts.index+1)%2!=0}">class="row1"</c:if><c:if test="${(stuts.index+1)%2==0}">class="row2"</c:if>>
															<li style="width: 100px;">
																<input type="checkbox" id="recordIds" name="recordIds"
																	value="${bean.vst_group_id}"
																	state="${bean.vst_group_state}"
																	index="${bean.vst_group_index}"
																	style="width:auto; vertical-align: middle; margin:0 0 0 10px;" />
																<span style="display:inline-block;line-height:32px;">${bean.vst_group_id}</span>
															</li>
															<li style="width: 270px;">
																[${bean.vst_sql_id}]
																<c:forEach items="${sqls}" var="sql">
															...
															<li style="width: 150px;" title="${bean.vst_group_name}">
																${bean.vst_group_name}
															</li>
															<li style="width: 150px;" title="${bean.vst_group_alias}">
																${bean.vst_group_alias}
															</li>
															<li style="width: 250px;" title="${bean.vst_group_desc}">
																${bean.vst_group_desc}
															</li>
															<li class="myClick" style="width: 100px;">${bean.vst_group_index}</li>
															<li style="width: 100px;">
												    			<c:choose>
												    				<c:when test="${bean.vst_group_state == 1}">
												    					<span style="color: green;">正常</span>
												    				</c:when>
												    				<c:when test="${bean.vst_group_state == 2}">
												    					<span style="color: red;">禁用</span>
												    				</c:when>
												    				<c:otherwise>
												    					${bean.vst_group_state}
												    				</c:otherwise>
												    			</c:choose>
												    		</li>
															<li style="width: 150px;">${bean.vst_group_addtime}</li>
															<li style="width: 100px;">${bean.vst_group_creator}</li>
												    		<li style="width: 150px;">${bean.vst_group_uptime}</li>
												    		<li style="width: 100px;">${bean.vst_group_updator}</li>
												    		<li style="width: 200px;" title="${bean.vst_group_summary}">
																${bean.vst_group_summary}
															</li>
														</ul>
													</c:forEach>
													</div>
