...
<script type="text/javascript">
  	$(function(){
  		//下拉列表框，赋值
  		$("#vst_topic_state").val($("#hidden_topic_state").val());
  	...
  	// 新增
	function button_add(){
		$("#listForm").attr("action", "${ctx}/sysTopic/toAdd.action");
		$("#listForm").submit();
	}
	...
	function button_edit(){
	    ...
	    if(modifyChecked==""){
	        ...
	        $("#listForm").attr("action", "${ctx}/sysTopic/toEdit.action");
			$("#listForm").submit();
	    }
	}
	...
	function button_delete(){
	    ...
	    if(modifyChecked==""){
	    	...
	    	if(window.confirm("您确定要删除所选的"+modifyChecked.split(',').length+"条记录吗?")){
		    	$("#recordId").val(modifyChecked);
		        $("#listForm").attr("action", "${ctx}/sysTopic/deleteSysTopic.action");
				$("#listForm").submit();
			}
	    }
	}
	...
	function button_stop(){
		...
		if(confirm("您确定要禁用所选的"+modifyChecked.split(',').length+"条记录吗？")){
			$("#recordId").val(modifyChecked);
			$("#recordState").val(2);
			$("#listForm").attr("action", "${ctx}/sysTopic/modifySysTopicState.action");
			$("#listForm").submit();
		}
	}
	...
	function makeIndex(){
		if($("#doIndex").attr("disabled") == undefined || $("#doIndex").attr("disabled") == ""){
			if(!checkIndexs()){
				...
				$.ajax({
					url:"${ctx}/sysTopic/modifySysTopicIndexs.action",
					type:"POST",
	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
			<div class="content-roll">
				<form action="${ctx}/sysTopic/findSysTopics.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 配置管理 > topic配置查询</span>
					</section>
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<input class="list-input1" type="text" name="vst_topic_id" value="${cutPage._queryParam.vst_topic_id }" />
											</li>
											<li>
												<span class="sertitle">topic名称：</span>
												<input class="list-input1" type="text" name="vst_topic_name" value="${cutPage._queryParam.vst_topic_name }" />
											</li>
											<li>
												<span class="sertitle">topic值：</span>
												<input class="list-input1" type="text" name="vst_topic_value" value="${cutPage._queryParam.vst_topic_value }" />
											</li>
											<li>
												<span class="sertitle">状态：</span>
												<input type="hidden" id="hidden_topic_state" value="${cutPage._queryParam.vst_topic_state }" />
												<select class="list-input1" id="vst_topic_state" name="vst_topic_state">
													...
													<div class="table-thead" style="width: 1820px;">
														<ul id="rowHead" style="background-color:#d2d6de;">
															...
															<li style="width: 300px;">
																<c:url value="../share/order.jsp" var="vst_topic_name_order">
																    <c:param name="orderBy" value="vst_topic_name"/>
																    <c:param name="fieldName" value="topic名称"/>
															    </c:url>
															    <c:import url="${vst_topic_name_order}"/>
															</li>
															<li style="width: 300px;">
																<c:url value="../share/order.jsp" var="vst_topic_value_order">
																    <c:param name="orderBy" value="vst_topic_value"/>
																    <c:param name="fieldName" value="topic值"/>
															    </c:url>
															    <c:import url="${vst_topic_value_order}"/>
															</li>
															<li style="width: 120px;">
																<c:url value="../share/order.jsp" var="vst_topic_index_order">
																    <c:param name="orderBy" value="vst_topic_index"/>
																    <c:param name="fieldName" value="排序"/>
															    </c:url>
															    <c:import url="${vst_topic_index_order}"/>
															</li>
															<li style="width: 120px;">
																<c:url value="../share/order.jsp" var="vst_topic_state_order">
																    <c:param name="orderBy" value="vst_topic_state"/>
																    <c:param name="fieldName" value="状态"/>
															    </c:url>
															    <c:import url="${vst_topic_state_order}"/>
															</li>
															<li style="width: 150px;">
																<c:url value="../share/order.jsp" var="vst_topic_addtime_order">
																    <c:param name="orderBy" value="vst_topic_addtime"/>
																    <c:param name="fieldName" value="新增时间"/>
															    </c:url>
															    <c:import url="${vst_topic_addtime_order}"/>
															</li>
															<li style="width: 150px;">
																<c:url value="../share/order.jsp" var="vst_topic_creator_order">
																    <c:param name="orderBy" value="vst_topic_creator"/>
																    <c:param name="fieldName" value="创建人"/>
															    </c:url>
															    <c:import url="${vst_topic_creator_order}"/>
															</li>
															<li style="width: 150px;">
																<c:url value="../share/order.jsp" var="vst_topic_uptime_order">
																    <c:param name="orderBy" value="vst_topic_uptime"/>
																    <c:param name="fieldName" value="修改时间"/>
															    </c:url>
															    <c:import url="${vst_topic_uptime_order}"/>
															</li>
															<li style="width: 150px;">
																<c:url value="../share/order.jsp" var="vst_topic_updator_order">
																    <c:param name="orderBy" value="vst_topic_updator"/>
																    <c:param name="fieldName" value="修改人"/>
															    </c:url>
															    <c:import url="${vst_topic_updator_order}"/>
															</li>
															<li style="width: 210px;">
																<c:url value="../share/order.jsp" var="vst_topic_summary_order">
																    <c:param name="orderBy" value="vst_topic_summary"/>
																    <c:param name="fieldName" value="备注"/>
															    </c:url>
															    <c:import url="${vst_topic_summary_order}"/>
															</li>
														</ul>
													</div>
														...
														<ul style="width: 1820px;" <c:if test="${(stuts.index+1)%2!=0}">class="row1"</c:if><c:if test="${(stuts.index+1)%2==0}">class="row2"</c:if>>
															<li style="width: 70px;">
																<input type="checkbox" id="recordIds" name="recordIds"
																	value="${bean.vst_topic_id}"
																	state="${bean.vst_topic_state}"
																	index="${bean.vst_topic_index}" />
															</li>
															<li style="width: 100px;">${bean.vst_topic_id}</li>
															<li style="width: 300px;" title="${bean.vst_topic_name}">
																${bean.vst_topic_name}
															</li>
															<li style="width: 300px;" title="${bean.vst_topic_value}">
																${bean.vst_topic_value}
															</li>
															<li class="myClick" style="width: 120px;">${bean.vst_topic_index}</li>
															<li style="width: 120px;">
												    			<c:choose>
												    				<c:when test="${bean.vst_topic_state == 1}">
												    					<span style="color: green;">正常</span>
												    				</c:when>
												    				<c:when test="${bean.vst_topic_state == 2}">
												    					<span style="color: red;">禁用</span>
												    				</c:when>
												    				<c:otherwise>
												    					${bean.vst_topic_state}
												    				</c:otherwise>
												    			</c:choose>
												    		</li>
															<li style="width: 150px;">${bean.vst_topic_addtime}</li>
															<li style="width: 150px;">${bean.vst_topic_creator}</li>
												    		<li style="width: 150px;">${bean.vst_topic_uptime}</li>
												    		<li style="width: 150px;">${bean.vst_topic_updator}</li>
												    		<li style="width: 210px;" title="${bean.vst_topic_summary}">
																${bean.vst_topic_summary}
															</li>
														</ul>
													</c:forEach>
													</div>
