...
<script type="text/javascript">
  	$(function(){
  		...
  		$("#vst_td_state").val($("#hidden_td_state").val());
  		$("#vst_td_desc").val($("#hidden_td_operateType").val());
  		$("#vst_td_dataType").val($("#hidden_td_dataType").val());
  	...
  	// 新增
	function button_add(){
		$("#listForm").attr("action", "${ctx}/tableDesc/toAdd.action");
		$("#listForm").submit();
	}
	...
	function button_edit(){
	    ...
	    if(modifyChecked==""){
	        ...
	        $("#listForm").attr("action", "${ctx}/tableDesc/toEdit.action");
			$("#listForm").submit();
	    }
	}
	...
	function button_delete(){
	    ...
	    if(modifyChecked==""){
	    	...
	    	if(window.confirm("您确定要删除所选的"+modifyChecked.split(',').length+"条记录吗?")){
		    	$("#recordId").val(modifyChecked);
		        $("#listForm").attr("action", "${ctx}/tableDesc/deleteTableDesc.action");
				$("#listForm").submit();
			}
	    }
	}
	...
	function button_stop(){
		...
		if(confirm("您确定要禁用所选的"+modifyChecked.split(',').length+"条记录吗？")){
			$("#recordId").val(modifyChecked);
			$("#recordState").val(2);
			$("#listForm").attr("action", "${ctx}/tableDesc/modifyTableDescState.action");
			$("#listForm").submit();
		}
	}
	...
	function button_batchAdd(){
		$("#listForm").attr("action", "${ctx}/tableDesc/toBatchAdd.action");
		$("#listForm").submit();
	}
	...
	function button_copyAdd(){
	    ...
	    if(modifyChecked==""){
	    	...
	    	$("#recordId").val(modifyChecked);
	        $("#listForm").attr("action", "${ctx}/tableDesc/toCopyAdd.action");
			$("#listForm").submit();
	    }
	}
	...
	function makeIndex(){
		if($("#doIndex").attr("disabled") == undefined || $("#doIndex").attr("disabled") == ""){
			if(!checkIndexs()){
				...
				$.ajax({
					url:"${ctx}/tableDesc/modifyTableDescIndexs.action",
					type:"POST",
	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
			<div class="content-roll">
				<form action="${ctx}/tableDesc/findTableDescs.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 配置管理 > 表算法描述查询</span>
					</section>
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<input class="list-input1" type="text" name="vst_td_id" value="${cutPage._queryParam.vst_td_id }" />
											</li>
											<li>
												<span class="sertitle">表名：</span>
												<input class="list-input1" type="text" id="vst_td_table" name="vst_td_table" value="${cutPage._queryParam.vst_td_table }"
													autocomplete="off" placeholder="默认" />
											</li>
											<li>
												<span class="sertitle">描述属性：</span>
												<input class="list-input1" type="text" name="vst_td_title" value="${cutPage._queryParam.vst_td_title }" />
											</li>
											<li>
												<span class="sertitle">描述内容：</span>
												<input class="list-input1" type="text" name="vst_td_desc" value="${cutPage._queryParam.vst_td_desc }" />
											</li>
											<li>
												<span class="sertitle">状态：</span>
												<input type="hidden" id="hidden_td_state" value="${cutPage._queryParam.vst_td_state }" />
												<select class="list-input1" id="vst_td_state" name="vst_td_state">
													...
													<div class="table-thead" style="width: 1820px;">
														<ul id="rowHead" style="background-color:#d2d6de;">
															<li style="width: 100px;">
																...
																<c:url value="../share/order.jsp" var="vst_td_id_order">
																    <c:param name="orderBy" value="vst_td_id"/>
																    <c:param name="fieldName" value="全选"/>
															    </c:url>
															    <c:import url="${vst_td_id_order}"/>
															</li>
															<li style="width: 150px;">
																<c:url value="../share/order.jsp" var="vst_td_table_order">
																    <c:param name="orderBy" value="vst_td_table"/>
																    <c:param name="fieldName" value="表名"/>
															    </c:url>
															    <c:import url="${vst_td_table_order}"/>
															</li>
															<li style="width: 200px;">
																<c:url value="../share/order.jsp" var="vst_td_title_order">
																    <c:param name="orderBy" value="vst_td_title"/>
																    <c:param name="fieldName" value="描述属性"/>
															    </c:url>
															    <c:import url="${vst_td_title_order}"/>
															</li>
															<li style="width: 470px;">
																<c:url value="../share/order.jsp" var="vst_td_desc_order">
																    <c:param name="orderBy" value="vst_td_desc"/>
																    <c:param name="fieldName" value="描述内容"/>
															    </c:url>
															    <c:import url="${vst_td_desc_order}"/>
															</li>
															<li style="width: 100px;">
																<c:url value="../share/order.jsp" var="vst_td_index_order">
																    <c:param name="orderBy" value="vst_td_index"/>
																    <c:param name="fieldName" value="排序"/>
															    </c:url>
															    <c:import url="${vst_td_index_order}"/>
															</li>
															<li style="width: 100px;">
																<c:url value="../share/order.jsp" var="vst_td_state_order">
																    <c:param name="orderBy" value="vst_td_state"/>
																    <c:param name="fieldName" value="状态"/>
															    </c:url>
															    <c:import url="${vst_td_state_order}"/>
															</li>
															<li style="width: 150px;">
																<c:url value="../share/order.jsp" var="vst_td_addtime_order">
																    <c:param name="orderBy" value="vst_td_addtime"/>
																    <c:param name="fieldName" value="新增时间"/>
															    </c:url>
															    <c:import url="${vst_td_addtime_order}"/>
															</li>
															<li style="width: 100px;">
																<c:url value="../share/order.jsp" var="vst_td_creator_order">
																    <c:param name="orderBy" value="vst_td_creator"/>
																    <c:param name="fieldName" value="创建人"/>
															    </c:url>
															    <c:import url="${vst_td_creator_order}"/>
															</li>
															<li style="width: 150px;">
																<c:url value="../share/order.jsp" var="vst_td_uptime_order">
																    <c:param name="orderBy" value="vst_td_uptime"/>
																    <c:param name="fieldName" value="修改时间"/>
															    </c:url>
															    <c:import url="${vst_td_uptime_order}"/>
															</li>
															<li style="width: 100px;">
																<c:url value="../share/order.jsp" var="vst_td_updator_order">
																    <c:param name="orderBy" value="vst_td_updator"/>
																    <c:param name="fieldName" value="修改人"/>
															    </c:url>
															    <c:import url="${vst_td_updator_order}"/>
															</li>
															<li style="width: 200px;">
																<c:url value="../share/order.jsp" var="vst_td_summary_order">
																    <c:param name="orderBy" value="vst_td_summary"/>
																    <c:param name="fieldName" value="备注"/>
															    </c:url>
															    <c:import url="${vst_td_summary_order}"/>
															</li>
														</ul>
													</div>
														...
														<ul style="width: 1820px;" <c:if test="${(stuts.index+1)%2!=0}">class="row1"</c:if><c:if test="${(stuts.index+1)%2==0}">class="row2"</c:if>>
															<li style="width: 100px;">
																<input type="checkbox" id="recordIds" name="recordIds"
																	value="${bean.vst_td_id}"
																	state="${bean.vst_td_state}"
																	index="${bean.vst_td_index}"
																	style="width:auto; vertical-align: middle; margin:0 0 0 10px;" />
																<span style="display:inline-block;line-height:32px;">${bean.vst_td_id}</span>
															</li>
															<li style="width: 150px;" title="${bean.vst_td_table}">
																${bean.vst_td_table}
															</li>
															<li style="width: 200px;" title="${bean.vst_td_title}">
																${bean.vst_td_title}
															</li>
															<li style="width: 470px;" title="${bean.vst_td_desc}">
																${bean.vst_td_desc}
															</li>
															<li class="myClick" style="width: 100px;">${bean.vst_td_index}</li>
															<li style="width: 100px;">
												    			<c:choose>
												    				<c:when test="${bean.vst_td_state == 1}">
												    					<span style="color: green;">正常</span>
												    				</c:when>
												    				<c:when test="${bean.vst_td_state == 2}">
															...
															<li style="width: 150px;">${bean.vst_td_addtime}</li>
															<li style="width: 100px;">${bean.vst_td_creator}</li>
												    		<li style="width: 150px;">${bean.vst_td_uptime}</li>
												    		<li style="width: 100px;">${bean.vst_td_updator}</li>
												    		<li style="width: 200px;" title="${bean.vst_td_summary}">
																${bean.vst_td_summary}
															</li>
														</ul>
													</c:forEach>
...
<script type="text/javascript">
	...
	var tableDatas = new Array(tableArr.length);
	...
	for(var i=0; i<tableArr.length; i++){
		...
		tableDatas[i] = {"id":key, "text":value};
	}
	
	// 文本选择框点击事件
	var tableSelFun = function(){
		$("#vst_td_table").val(this.id);
	};
	
	$.selectSuggest('vst_td_table', tableDatas, tableSelFun);
</script>
