	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">全球购统计 > 直播订单统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/shoppingLiveOrder/findShoppingLiveOrder.action" id="listForm" method="post">
...
<script type="text/javascript">
	...
	function ajaxGetData(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url : "${ctx}/shoppingLiveOrder/ajaxGetReportByDate.action",
			dataType : "json",
	...
	function ajaxGetDataByChannel(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url : "${ctx}/shoppingLiveOrder/ajaxGetReportByChannel.action",
			dataType : "json",
			success : function(msg){
	...
	function ajaxGetCutPage2(){
		...
		$.ajax({
			...
			async : true,
			cache : false,	
			url:"${ctx}/shoppingLiveOrder/ajaxGetReportDataByGood.action",
			dataType:"json",
			success:function(msg){
	...
	function button_exportSum(){
		...
		if(!isNotEmpty(startDate) || !isNotEmpty(endDate) || getDateDiff(startDate, endDate, 'day') > 92){
			...
			location.href = "${ctx}/shoppingLiveOrder/exportSumDataByGood.action?" + params
	...
	function button_grasp(){
		$("#listForm").attr("action", "${ctx}/shoppingLiveOrder/toGrasp.action");
		$("#listForm").submit();
	}
	...
	function button_delete(){
		if(confirm("是否删除查询出来的数据？")){
			...
			$("#listForm").attr("action", "${ctx}/shoppingLiveOrder/deleteLiveOrder.action?"+params);
			$("#listForm").submit();
		}
	}
