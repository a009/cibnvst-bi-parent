	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">用户行为 > 页面统计详情 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/pageJump/findDetails.action" id="listForm" method="post">
					<input type="hidden" id="vst_pj_date" name="vst_pj_date" value="${date }" />
					<input type="hidden" id="vst_pj_curr_page" name="vst_pj_curr_page" value="${currPage }" />
								...
								<div class="box-body">
									<div class="box-search">
										<c:choose>
											...
											<li>
												<span class="sertitle">跳转页面：</span>
												<input class="list-input1" type="text" id="vst_pj_jump_page" name="vst_pj_jump_page" />
											</li>
										</ul>
									</div>
									<div style="text-align: center;">
										<input type="button" class="queryBtn" value="查询" onclick="javascript:button_getdata();" />
										<input type="button" class="resetBtn" value="返回" onclick="javascript:window.history.go(-1);" />
									</div>
								</div>
							</div>
						</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		// 将接收到的时间转化成时间戳
        var timeStampt = moment(parseDate('${date}')).valueOf();
        $.initDaterangepicker({startTime:timeStampt, endTime:timeStampt});
		// 表描述赋值
		...
		//文本框，按回车触发查询
		$("#vst_pj_jump_page").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		params += "&vst_pj_curr_page_eq=" + $("#vst_pj_curr_page").val();
		// 参数特殊化处理
		params += "&" + paramFormat("vst_pj_jump_page", $("#vst_pj_jump_page").val());

		var ajaxUrl;
		if($("#vst_pj_jump_page").val() == ''){
			if(startDay == endDay){
				ajaxUrl = "${ctx}/report/json?code=page_jump_pie";
			}else{
				ajaxUrl = "${ctx}/pageJump/ajaxGetReportByJumpPage.action";
			}
		}else{
			ajaxUrl = "${ctx}/report/json?code=page_jump_line";
		}
		$.ajax({
			type : "POST",
			data : params,
	...
	function line(msg){
		var data = msg.data;
		var xaxis = [];
		var vst_pj_jump_amount = [];

		$.each(data,function(i,n){
			xaxis[i] = getDateWeek(n.date+'', 'yyyyMMdd');
			vst_pj_jump_amount[i] = n.vst_pj_jump_amount;
		});
	...
	function pie(msg){
		...
		if(data.length > 20){
   		...
   		$.each(top20data, function(i,n){
   			series +="{name:\""+getMapValue('${pageMap}', n.value)+"："+n.vst_pj_jump_amount+"\",y:"+n.vst_pj_jump_amount+"},";
   		});
	...
	function linePercent(msg){
		...
		var series = [];
		for(var i=0; i<values.length; i++){
			var value = getMapValue('${pageMap}', values[i]);
			if(values[i] == ''){
				value = "未知";
			}
			var json = {name : value, data : amountArr[i]};
			series.push(json);
		}
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=page_jump_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_pj_date' id='vst_pj_date2' order='desc'>日期</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_pj_curr_page' id='vst_pj_curr_page2' order='desc' style='color:orange;'>当前页面</a></th>";
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_pj_jump_page' id='vst_pj_jump_page2' order='desc'>跳转页面</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_pj_jump_amount' id='vst_pj_jump_amount2' order='desc'>页面跳转数</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_pj_jump_radio' id='vst_pj_jump_radio2' order='desc'>页面跳转占比</a></th>";
		...
		$.each(data,function(i,n){
			...
			table += "<td>"+getDateWeek(n.vst_pj_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_pj_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_pj_creator)
						+"<br>修改时间："+formatDate(n.vst_pj_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_pj_updator)
						+"<br>备注："+toString(n.vst_pj_summary)+"' class='enlargeTextTitle' title='"+n.vst_pj_curr_page+"'>"
					+getMapValue('${pageMap}', n.vst_pj_curr_page)+"</td>";
			table += "<td title='"+n.vst_pj_jump_page+"'>"
					+getMapValue('${pageMap}', n.vst_pj_jump_page)+"</td>";
			//页面跳转
			table += "<td>"+formatNumber(n.vst_pj_jump_amount)+"</td>";
			table += "<td>"+toString(n.vst_pj_jump_radio)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&pkgName=" + $("#pkgName").val();
					params += "&vst_pj_curr_page=" + encodeURI(encodeURI($("#vst_pj_curr_page").val()));
					params += "&vst_pj_jump_page=" + encodeURI(encodeURI($("#vst_pj_jump_page").val()));
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					
					location.href = "${ctx}/pageJump/exportData.action?" + params
