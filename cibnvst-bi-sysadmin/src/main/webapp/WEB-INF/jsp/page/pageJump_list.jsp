	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">用户行为 > 页面统计 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/pageJump/findPageJump.action" id="listForm" method="post">
								...
								<div class="box-body">
									<div class="box-search">
										<c:choose>
											...
											<li>
												<span class="sertitle">当前页面：</span>
												<input class="list-input1" type="text" id="vst_pj_curr_page" name="vst_pj_curr_page" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//文本框，按回车触发查询
		$("#vst_pj_curr_page").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		// 参数特殊化处理
		params += "&" + paramFormat("vst_pj_curr_page", $("#vst_pj_curr_page").val());
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=page_jump_barByCurr",
			dataType:"json",
	...
	function bar(msg){
		...
		$.each(top10data, function(i,n){
			xaxis[i] = getMapValue('${pageMap}', n.vst_pj_curr_page);
			series[i] = n.vst_pj_curr_amount;
		});
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=page_jump_tableByCurr",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='20%'><a onclick='changeOrder(this)' orderBy='vst_pj_curr_page' id='vst_pj_curr_page2' order='desc'>当前页面</a></th>";
		table += "<th width='10%'><a onclick='changeOrder(this)' orderBy='vst_pj_curr_amount' id='vst_pj_curr_amount2' order='desc'>页面点击</a></th>";
		...
		$.each(data,function(i,n){
			...
			table += "<td title='"+n.vst_pj_curr_page+"'>"
					+getMapValue('${pageMap}', n.vst_pj_curr_page)+"</td>";
			//页面点击
			table += "<td>"+formatNumber(n.vst_pj_curr_amount)+"</td>";
			table += "<td><a onclick='getDetails("+n.vst_pj_date+", &apos;"+n.vst_pj_curr_page+"&apos;)'>查看详情</a></td>"
	...
	// 查看详情
	function getDetails(date, currPage){
		window.location.href = "${ctx}/pageJump/findDetails.action?date="+date+"&currPage="+currPage+"&moduleId="+$("#moduleId").val();
	}
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					location.href = "${ctx}/pageJump/exportDataByCurrPage.action?" + params
