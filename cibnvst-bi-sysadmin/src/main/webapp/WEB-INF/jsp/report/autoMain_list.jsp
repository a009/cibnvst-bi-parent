...
<script type="text/javascript">
  	$(function(){
  		...
  		//下拉列表框，赋值
  		if($("#hidden_main_state").val() == ''){
  			$("#vst_main_state").val(1);
  		}else{
  			$("#vst_main_state").val($("#hidden_main_state").val());
  		}
  		$("#vst_main_isPaging").val($("#hidden_main_isPaging").val());
  	...
  	// 新增
	function button_add(){
		$("#listForm").attr("action", "${ctx}/autoMain/toAdd.action");
		$("#listForm").submit();
	}
	...
	function button_edit(){
	    ...
	    if(modifyChecked==""){
	        ...
	        $("#listForm").attr("action", "${ctx}/autoMain/toEdit.action");
			$("#listForm").submit();
	    }
	}
	...
	function button_delete(){
	    ...
	    if(modifyChecked==""){
	    	...
	    	if(window.confirm("您确定要删除所选的"+modifyChecked.split(',').length+"条记录吗?")){
		    	$("#recordId").val(modifyChecked);
		        $("#listForm").attr("action", "${ctx}/autoMain/deleteAutoMain.action");
				$("#listForm").submit();
			}
	    }
	}
	...
	function button_stop(){
		...
		if(confirm("您确定要禁用所选的"+modifyChecked.split(',').length+"条记录吗？")){
			$("#recordId").val(modifyChecked);
			$("#recordState").val(2);
			$("#listForm").attr("action", "${ctx}/autoMain/modifyAutoMainState.action");
			$("#listForm").submit();
		}
	}
	...
	function button_push(){
    	if(window.confirm("您确定要刷新自动化缓存吗?")){
	        $("#listForm").attr("action", "${ctx}/autoMain/flushAutoCache.action");
			$("#listForm").submit();
		}
	}

	// 彻底删除
	function button_realDelete(){
		var modifyChecked="";			
		$("input:checked[type=checkbox][name=recordIds]").each(function(){
			modifyChecked = modifyChecked + $(this).attr("code") + ",";
		});
	    modifyChecked = modifyChecked.substring(0,modifyChecked.length-1);
	    if(modifyChecked==""){
	       	alert("您还没有选择记录!");
	    	...
	    	if(window.confirm("删除相关联的代码编号数据，是否确认?")){
		    	$("#recordId").val(modifyChecked);
		        $("#listForm").attr("action", "${ctx}/autoMain/realDeleteByCode.action");
				$("#listForm").submit();
			}
	    }
	}
	...
	function button_look(){
	    ...
	    if(modifyChecked==""){
	    	...
	    	$("#recordId").val(modifyChecked);
	        $("#listForm").attr("action", "${ctx}/autoMain/toLook.action");
			$("#listForm").submit();
	    }
	}
	...
	function makeIndex(){
		if($("#doIndex").attr("disabled") == undefined || $("#doIndex").attr("disabled") == ""){
			if(!checkIndexs()){
				...
				$.ajax({
					url:"${ctx}/autoMain/modifyAutoMainIndexs.action",
					type:"POST",
	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
			<div class="content-roll">
				<form action="${ctx}/autoMain/findAutoMains.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 自动化管理 > 自动化(主表)查询</span>
					</section>
								...
								<div class="box-body">
									<div class="box-search">
										<c:choose>
											...
											<li>
												<span class="sertitle">主表ID：</span>
												<input class="list-input1" type="text" name="vst_main_id" value="${cutPage._queryParam.vst_main_id }" />
											</li>
											<li>
											...
											<li>
												...
												<input type="hidden" id="hidden_main_state" value="${cutPage._queryParam.vst_main_state }" />
												<select class="list-input1" id="vst_main_state" name="vst_main_state">
											...
											<li>
												...
												<input class="list-input1" type="text" name="vst_main_script" value="${cutPage._queryParam.vst_main_script }" />
											</li>
											<li>
												<span class="sertitle">是否分页：</span>
												<input type="hidden" id="hidden_main_isPaging" value="${cutPage._queryParam.vst_main_isPaging }" />
												<select class="list-input1" id="vst_main_isPaging" name="vst_main_isPaging">
											...
											<li>
												...
												<input class="list-input1" type="text" name="vst_main_summary" value="${cutPage._queryParam.vst_main_summary }" />
											</li>
										</ul>
										<c:choose>
						    				<c:when test="${hidden_search=='1' || hidden_search==1}">
												...
												<div class="table table-hover text-center" style="width: 1820px;max-width: 1820px;">
													<div class="table-thead" style="width: 1820px;">
														<ul id="rowHead" style="background-color:#d2d6de;">
															...
															<li style="width: 70px;">
																<c:url value="../share/order.jsp" var="vst_main_id_order">
																    <c:param name="orderBy" value="vst_main_id"/>
																    <c:param name="fieldName" value="主表ID"/>
															    </c:url>
															    <c:import url="${vst_main_id_order}"/>
															</li>
															...
															<li style="width: 600px;">
																<c:url value="../share/order.jsp" var="vst_main_script_order">
																    <c:param name="orderBy" value="vst_main_script"/>
																    <c:param name="fieldName" value="脚本"/>
															    </c:url>
															    <c:import url="${vst_main_script_order}"/>
															</li>
															<li style="width: 70px;">
																<c:url value="../share/order.jsp" var="vst_main_isPaging_order">
																    <c:param name="orderBy" value="vst_main_isPaging"/>
																    <c:param name="fieldName" value="是否分页"/>
															    </c:url>
															    <c:import url="${vst_main_isPaging_order}"/>
															</li>
															<li style="width: 70px;">
																<c:url value="../share/order.jsp" var="vst_main_index_order">
																    <c:param name="orderBy" value="vst_main_index"/>
																    <c:param name="fieldName" value="排序"/>
															    </c:url>
															    <c:import url="${vst_main_index_order}"/>
															</li>
															<li style="width: 70px;">
																<c:url value="../share/order.jsp" var="vst_main_state_order">
																    <c:param name="orderBy" value="vst_main_state"/>
																    <c:param name="fieldName" value="状态"/>
															    </c:url>
															    <c:import url="${vst_main_state_order}"/>
															</li>
															<li style="width: 150px;">
																<c:url value="../share/order.jsp" var="vst_main_addtime_order">
																    <c:param name="orderBy" value="vst_main_addtime"/>
																    <c:param name="fieldName" value="新增时间"/>
															    </c:url>
															    <c:import url="${vst_main_addtime_order}"/>
															</li>
															<li style="width: 100px;">
																<c:url value="../share/order.jsp" var="vst_main_creator_order">
																    <c:param name="orderBy" value="vst_main_creator"/>
																    <c:param name="fieldName" value="创建人"/>
															    </c:url>
															    <c:import url="${vst_main_creator_order}"/>
															</li>
															<li style="width: 150px;">
																<c:url value="../share/order.jsp" var="vst_main_uptime_order">
																    <c:param name="orderBy" value="vst_main_uptime"/>
																    <c:param name="fieldName" value="修改时间"/>
															    </c:url>
															    <c:import url="${vst_main_uptime_order}"/>
															</li>
															<li style="width: 100px;">
																<c:url value="../share/order.jsp" var="vst_main_updator_order">
																    <c:param name="orderBy" value="vst_main_updator"/>
																    <c:param name="fieldName" value="修改人"/>
															    </c:url>
															    <c:import url="${vst_main_updator_order}"/>
															</li>
															<li style="width: 210px;">
																<c:url value="../share/order.jsp" var="vst_main_summary_order">
																    <c:param name="orderBy" value="vst_main_summary"/>
																    <c:param name="fieldName" value="描述"/>
															    </c:url>
															    <c:import url="${vst_main_summary_order}"/>
															</li>
														</ul>
													</div>
														...
														<ul style="width: 1820px;" <c:if test="${(stuts.index+1)%2!=0}">class="row1"</c:if><c:if test="${(stuts.index+1)%2==0}">class="row2"</c:if>>
															<li style="width: 70px;">
																<input type="checkbox" id="recordIds" name="recordIds"
																	value="${bean.vst_main_id}"
																	state="${bean.vst_main_state}"
																	index="${bean.vst_main_index}"
																	code="${bean.vst_code}" />
															</li>
															<li style="width: 70px;">${bean.vst_main_id}</li>
															<li style="width: 160px;" title="${bean.vst_code}">
																${bean.vst_code}
															</li>
															<li style="width: 600px; text-align: left;" class="enlargeTextCon">
																${bean.vst_main_script}
															</li>
															<li style="width: 70px;">
																<c:choose>
												    				<c:when test="${bean.vst_main_isPaging == 0}">
												    					<span style="color: red;">否</span>
												    				</c:when>
												    				<c:when test="${bean.vst_main_isPaging == 1}">
												    					<span style="color: green;">是</span>
												    				</c:when>
												    				<c:otherwise>
												    					${bean.vst_main_isPaging}
												    				</c:otherwise>
												    			</c:choose>
															</li>
															<li class="myClick" style="width: 70px;">${bean.vst_main_index}</li>
															<li style="width: 70px;">
												    			<c:choose>
												    				<c:when test="${bean.vst_main_state == 1}">
												    					<span style="color: green;">正常</span>
												    				</c:when>
												    				<c:when test="${bean.vst_main_state == 2}">
															...
															<li style="width: 150px;">${bean.vst_main_addtime}</li>
															<li style="width: 100px;">${bean.vst_main_creator}</li>
												    		<li style="width: 150px;">${bean.vst_main_uptime}</li>
												    		<li style="width: 100px;">${bean.vst_main_updator}</li>
												    		<li style="width: 210px;" title="${bean.vst_main_summary}">
																${bean.vst_main_summary}
															</li>
														</ul>
													</c:forEach>
													</div>
