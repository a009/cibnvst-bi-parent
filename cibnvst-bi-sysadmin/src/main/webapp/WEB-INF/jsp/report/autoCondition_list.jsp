...
<script type="text/javascript">
  	$(function(){
  		...
  		//下拉列表框，赋值
  		$("#vst_condition_state").val($("#hidden_condition_state").val());
  	...
  	// 新增
	function button_add(){
		$("#listForm").attr("action", "${ctx}/autoCondition/toAdd.action");
		$("#listForm").submit();
	}
	...
	function button_edit(){
	    ...
	    if(modifyChecked==""){
	        ...
	        $("#listForm").attr("action", "${ctx}/autoCondition/toEdit.action");
			$("#listForm").submit();
	    }
	}
	...
	function button_delete(){
	    ...
	    if(modifyChecked==""){
	    	...
	    	if(window.confirm("您确定要删除所选的"+modifyChecked.split(',').length+"条记录吗?")){
		    	$("#recordId").val(modifyChecked);
		        $("#listForm").attr("action", "${ctx}/autoCondition/deleteAutoCondition.action");
				$("#listForm").submit();
			}
	    }
	}
	...
	function button_stop(){
		...
		if(confirm("您确定要禁用所选的"+modifyChecked.split(',').length+"条记录吗？")){
			$("#recordId").val(modifyChecked);
			$("#recordState").val(2);
			$("#listForm").attr("action", "${ctx}/autoCondition/modifyAutoConditionState.action");
			$("#listForm").submit();
		}
	}
	...
	function button_copy(){
	    ...
	    if(modifyChecked==""){
	    	...
	    	$("#recordId").val(modifyChecked);
	        $("#listForm").attr("action", "${ctx}/autoCondition/toCopy.action");
			$("#listForm").submit();
	    }
	}
	...
	$(document).ready(function(){
		...
		$("#replaceSubmit").click(function(){
			...
			if(flag){
				...
				$.ajax({
					url:"${ctx}/autoCondition/ajaxReplaceCondition.action",
					type:"POST",
					dataType:"json",
					cache:false,
	...
	function makeIndex(){
		if($("#doIndex").attr("disabled") == undefined || $("#doIndex").attr("disabled") == ""){
			if(!checkIndexs()){
				...
				$.ajax({
					url:"${ctx}/autoCondition/modifyAutoConditionIndexs.action",
					type:"POST",
	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
			...
			<div class="content-roll">
				<form action="${ctx}/autoCondition/findAutoConditions.action" id="listForm" method="post">
					<%@include file="../share/sharForm.jsp"%>
					<section class="content-header">
						<i class="fa fa-home"></i>
						<a href="${ctx}/sysMain/index.action">首页</a>
						<span> > 自动化管理 > 自动化(条件)查询</span>
					</section>
								...
								<div class="box-body">
									<div class="box-search">
										<c:choose>
											...
											<li>
												<span class="sertitle">条件ID：</span>
												<input class="list-input1" type="text" name="vst_condition_id" value="${cutPage._queryParam.vst_condition_id }" />
											</li>
											<li>
											...
											<li>
												...
												<input type="hidden" id="hidden_condition_state" value="${cutPage._queryParam.vst_condition_state }" />
												<select class="list-input1" id="vst_condition_state" name="vst_condition_state">
													<option value="">请选择</option>
											...
											<li>
												<span class="sertitle">参数：</span>
												<input class="list-input1" type="text" name="vst_condition_arg" value="${cutPage._queryParam.vst_condition_arg }" />
											</li>
											<li>
												<span class="sertitle">脚本：</span>
												<input class="list-input1" type="text" name="vst_condition_script" value="${cutPage._queryParam.vst_condition_script }" />
											</li>
											<li>
												<span class="sertitle">是否必填：</span>
												<input type="hidden" id="hidden_condition_need" value="${cutPage._queryParam.vst_condition_need }" />
												<select class="list-input1" id="vst_condition_need" name="vst_condition_need">
													<option value="">请选择</option>
											...
											<li>
												...
												<input class="list-input1" type="text" name="vst_condition_type" value="${cutPage._queryParam.vst_condition_type }" />
											</li>
											<li>
												<span class="sertitle">描述：</span>
												<input class="list-input1" type="text" name="vst_condition_summary" value="${cutPage._queryParam.vst_condition_summary }" />
											</li>
										</ul>
										<c:choose>
						    				<c:when test="${hidden_search=='1' || hidden_search==1}">
													...
													<div class="table-thead" style="width: 1820px;">
														<ul id="rowHead" style="background-color:#d2d6de;">
															...
															<li style="width: 100px;">
																<c:url value="../share/order.jsp" var="vst_condition_id_order">
																    <c:param name="orderBy" value="vst_condition_id"/>
																    <c:param name="fieldName" value="条件ID"/>
															    </c:url>
															    <c:import url="${vst_condition_id_order}"/>
															</li>
															...
															<li style="width: 150px;">
																<c:url value="../share/order.jsp" var="vst_condition_arg_order">
																    <c:param name="orderBy" value="vst_condition_arg"/>
																    <c:param name="fieldName" value="参数"/>
															    </c:url>
															    <c:import url="${vst_condition_arg_order}"/>
															</li>
															<li style="width: 300px;">
																<c:url value="../share/order.jsp" var="vst_condition_script_order">
																    <c:param name="orderBy" value="vst_condition_script"/>
																    <c:param name="fieldName" value="脚本"/>
															    </c:url>
															    <c:import url="${vst_condition_script_order}"/>
															</li>
															<li style="width: 100px;">
																<c:url value="../share/order.jsp" var="vst_condition_need_order">
																    <c:param name="orderBy" value="vst_condition_need"/>
																    <c:param name="fieldName" value="是否必填"/>
															    </c:url>
															    <c:import url="${vst_condition_need_order}"/>
															</li>
															<li style="width: 100px;">
																<c:url value="../share/order.jsp" var="vst_condition_type_order">
																    <c:param name="orderBy" value="vst_condition_type"/>
																    <c:param name="fieldName" value="参数类型"/>
															    </c:url>
															    <c:import url="${vst_condition_type_order}"/>
															</li>
															
															<li style="width: 70px;">
																<c:url value="../share/order.jsp" var="vst_condition_index_order">
																    <c:param name="orderBy" value="vst_condition_index"/>
																    <c:param name="fieldName" value="排序"/>
															    </c:url>
															    <c:import url="${vst_condition_index_order}"/>
															</li>
															<li style="width: 70px;">
																<c:url value="../share/order.jsp" var="vst_condition_state_order">
																    <c:param name="orderBy" value="vst_condition_state"/>
																    <c:param name="fieldName" value="状态"/>
															    </c:url>
															    <c:import url="${vst_condition_state_order}"/>
															</li>
															<li style="width: 150px;">
																<c:url value="../share/order.jsp" var="vst_condition_addtime_order">
																    <c:param name="orderBy" value="vst_condition_addtime"/>
																    <c:param name="fieldName" value="新增时间"/>
															    </c:url>
															    <c:import url="${vst_condition_addtime_order}"/>
															</li>
															<li style="width: 100px;">
																<c:url value="../share/order.jsp" var="vst_condition_creator_order">
																    <c:param name="orderBy" value="vst_condition_creator"/>
																    <c:param name="fieldName" value="创建人"/>
															    </c:url>
															    <c:import url="${vst_condition_creator_order}"/>
															</li>
															<li style="width: 150px;">
																<c:url value="../share/order.jsp" var="vst_condition_uptime_order">
																    <c:param name="orderBy" value="vst_condition_uptime"/>
																    <c:param name="fieldName" value="修改时间"/>
															    </c:url>
															    <c:import url="${vst_condition_uptime_order}"/>
															</li>
															<li style="width: 100px;">
																<c:url value="../share/order.jsp" var="vst_condition_updator_order">
																    <c:param name="orderBy" value="vst_condition_updator"/>
																    <c:param name="fieldName" value="修改人"/>
															    </c:url>
															    <c:import url="${vst_condition_updator_order}"/>
															</li>
															<li style="width: 210px;">
																<c:url value="../share/order.jsp" var="vst_condition_summary_order">
																    <c:param name="orderBy" value="vst_condition_summary"/>
																    <c:param name="fieldName" value="描述"/>
															    </c:url>
															    <c:import url="${vst_condition_summary_order}"/>
															</li>
														</ul>
													</div>
														...
														<ul style="width: 1820px;" <c:if test="${(stuts.index+1)%2!=0}">class="row1"</c:if><c:if test="${(stuts.index+1)%2==0}">class="row2"</c:if>>
															<li style="width: 70px;">
																<input type="checkbox" id="recordIds" name="recordIds"
																	value="${bean.vst_condition_id}"
																	state="${bean.vst_condition_state}"
																	index="${bean.vst_condition_index}" />
															</li>
															<li style="width: 100px;">${bean.vst_condition_id}</li>
															<li style="width: 150px;" title="${bean.vst_code}">
																${bean.vst_code}
															</li>
															<li style="width: 150px;" title="${bean.vst_condition_arg}">
																${bean.vst_condition_arg}
															</li>
															<li style="width: 300px;" class="enlargeTextCon">
																${bean.vst_condition_script}
															</li>
															<li style="width: 100px;">
												    			<c:choose>
												    				<c:when test="${bean.vst_condition_need == 1}">
												    					<span style="color: red;">是</span>
												    				</c:when>
												    				<c:when test="${bean.vst_condition_need == 2}">
												    					否
												    				</c:when>
												    				<c:otherwise>
												    					${bean.vst_condition_need}
												    				</c:otherwise>
												    			</c:choose>
												    		</li>
															<li style="width: 100px;">${bean.vst_condition_type}</li>
															<li class="myClick" style="width: 70px;">${bean.vst_condition_index}</li>
															<li style="width: 70px;">
												    			<c:choose>
												    				<c:when test="${bean.vst_condition_state == 1}">
												    					<span style="color: green;">正常</span>
												    				</c:when>
												    				<c:when test="${bean.vst_condition_state == 2}">
												    					<span style="color: red;">禁用</span>
												    				</c:when>
												    				<c:otherwise>
												    					${bean.vst_condition_state}
												    				</c:otherwise>
												    			</c:choose>
												    		</li>
															<li style="width: 150px;">${bean.vst_condition_addtime}</li>
															<li style="width: 100px;">${bean.vst_condition_creator}</li>
												    		<li style="width: 150px;">${bean.vst_condition_uptime}</li>
												    		<li style="width: 100px;">${bean.vst_condition_updator}</li>
												    		<li style="width: 210px;" title="${bean.vst_condition_summary}">
																${bean.vst_condition_summary}
															</li>
														</ul>
													</c:forEach>
													</div>
