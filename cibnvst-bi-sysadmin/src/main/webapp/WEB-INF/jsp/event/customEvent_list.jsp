	...
	<div class="wrapper">
		...
		<div class="content-wrapper" style="background-color: #ecf0f5;">
					...
					<div class="modal-content">
						<div class="modal-header">
							...
							<h4 class="modal-title">
								<i class="fa fa-home"></i><span style="font-size:14px;">产品统计 > 自定义事件 > 导出</span>
							</h4>
						</div>
			...
			<div class="content-roll">
				<form action="${ctx}/customEvent/findCustomEvent.action" id="listForm" method="post">
									...
									<div class="box-search">
										<c:choose>
											...
											<li>
												...
												<input class="list-input1" type="text" id="vst_ce_version" name="vst_ce_version" />
											</li>
											<li>
												<span class="sertitle">事件名称：</span>
												<input class="list-input1" type="text" id="vst_ce_event_name" name="vst_ce_event_name" />
											</li>
											<li>
												<span class="sertitle">事件类型：</span>
												<input class="list-input1" type="text" id="vst_ce_event_type" name="vst_ce_event_type" />
											</li>
											<li>
												<span class="sertitle">事件Key：</span>
												<input class="list-input1" type="text" id="vst_ce_event_key" name="vst_ce_event_key" />
											</li>
											<li>
												<span class="sertitle">事件Value：</span>
												<input class="list-input1" type="text" id="vst_ce_event_value" name="vst_ce_event_value" />
											</li>
										</ul>
									</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		// 文本框，按回车触发查询
		$("#vst_ce_version").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_ce_event_type").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_ce_event_name").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_ce_event_key").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
		$("#vst_ce_event_value").keydown(function(event){
        	if(event.keyCode == "13"){
        		button_getdata();
        	}
    	});
	...
	function ajaxGetReport(){
		...
		// 参数特殊化处理
		params += "&" + paramFormat("vst_ce_version", $("#vst_ce_version").val());
		params += "&" + paramFormat("vst_ce_event_type", $("#vst_ce_event_type").val());
		params += "&" + paramFormat("vst_ce_event_name", $("#vst_ce_event_name").val());
		params += "&" + paramFormat("vst_ce_event_key", $("#vst_ce_event_key").val());
		params += "&" + paramFormat("vst_ce_event_value", $("#vst_ce_event_value").val());
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url : "${ctx}/report/json?code=custom_event_line",
			dataType : "json",
	...
	function ajaxGetCutPage(){
		...
		$.ajax({
			...
			async : true,
			cache : false,
			url:"${ctx}/report/json?code=custom_event_table",
			dataType:"json",
			success:function(msg){
	...
	function showTable(msg){
		...
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_ce_date' id='vst_ce_date2' order='desc'>日期</a></th>";
		table += "<th width='5%'><a onclick='changeOrder(this)' orderBy='vst_ce_version' id='vst_ce_version2' order='desc' style='color:orange;'>版本</a></th>";
		table += "<th width='8%'><a onclick='changeOrder(this)' orderBy='vst_ce_event_name' id='vst_ce_event_name2' order='desc'>事件名称</a></th>";
		table += "<th width='8%'><a onclick='changeOrder(this)' orderBy='vst_ce_event_type' id='vst_ce_event_type2' order='desc'>事件类型</a></th>";
		table += "<th width='8%'><a onclick='changeOrder(this)' orderBy='vst_ce_event_key' id='vst_ce_event_key2' order='desc'>事件Key</a></th>";
		table += "<th width='8%'><a onclick='changeOrder(this)' orderBy='vst_ce_event_value' id='vst_ce_event_value2' order='desc'>事件Value</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_ce_vv' id='vst_ce_vv2' order='desc'>消息数</a> | <a onclick='changeOrder(this)' orderBy='vst_ce_vv_dod' id='vst_ce_vv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_ce_vv_wow' id='vst_ce_vv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='15%'><a onclick='changeOrder(this)' orderBy='vst_ce_uv' id='vst_ce_uv2' order='desc'>独立用户</a> | <a onclick='changeOrder(this)' orderBy='vst_ce_uv_dod' id='vst_ce_uv_dod2' order='desc'>天环比</a> | <a onclick='changeOrder(this)' orderBy='vst_ce_uv_wow' id='vst_ce_uv_wow2' order='desc'>周环比</a></th>";
		table += "<th width='8%'><a onclick='changeOrder(this)' orderBy='vst_ce_avg' id='vst_ce_avg2' order='desc'>人均消息数</a></th>";
		
		table += "</tr></thead><tbody>";
		
		$.each(data,function(i,n){
			var row = i%2+1;
			var ceVvDodColor = "green";
			var ceVvWowColor = "green";
			var ceUvDodColor = "green";
			var ceUvWowColor = "green";
			
			if(parseFloat(n.vst_ce_vv_dod)<0){
				ceVvDodColor = "red";
			}
			if(parseFloat(n.vst_ce_vv_wow)<0){
				ceVvWowColor = "red";
			}
			if(parseFloat(n.vst_ce_uv_dod)<0){
				ceUvDodColor = "red";
			}
			if(parseFloat(n.vst_ce_uv_wow)<0){
				ceUvWowColor = "red";
			}
			...
			table += "<td>"+getDateWeek(n.vst_ce_date+'', 'yyyyMMdd')+"</td>";
			table += "<td data-title='新增时间："+formatDate(n.vst_ce_addtime, "yyyy-MM-dd HH:mm:ss")
						+"<br>创建人："+toString(n.vst_ce_creator)
						+"<br>修改时间："+formatDate(n.vst_ce_uptime, "yyyy-MM-dd HH:mm:ss")
						+"<br>修改人："+toString(n.vst_ce_updator)
						+"<br>备注："+toString(n.vst_ce_summary)+"' class='enlargeTextTitle'>"
					+toString(n.vst_ce_version)+"</td>";
			table += "<td>"+toString(n.vst_ce_event_name)+"</td>";
			table += "<td>"+toString(n.vst_ce_event_type)+"</td>";
			table += "<td>"+toString(n.vst_ce_event_key)+"</td>";
			table += "<td>"+toString(n.vst_ce_event_value)+"</td>";
			//消息数
			table += "<td>"+formatNumber(n.vst_ce_vv)+" | ";
			table += "<span class='"+ceVvDodColor+"'>"+toString(n.vst_ce_vv_dod)+"</span> | ";
			table += "<span class='"+ceVvWowColor+"'>"+toString(n.vst_ce_vv_wow)+"</span></td>";
			//独立用户
			table += "<td>"+formatNumber(n.vst_ce_uv)+" | ";
			table += "<span class='"+ceUvDodColor+"'>"+toString(n.vst_ce_uv_dod)+"</span> | ";
			table += "<span class='"+ceUvWowColor+"'>"+toString(n.vst_ce_uv_wow)+"</span></td>";
			//人均消息数
			table += "<td>"+toString(n.vst_ce_avg)+"</td>";
		...
		$("#exportSubmit").click(function(){
			...
			if(flag){
				...
				if(confirm("导出速度，由数据大小决定，请耐心等待。。")){
					...
					params += "&pkgName=" + $("#pkgName").val();
					params += "&vst_ce_version=" + $("#vst_ce_version").val();
					params += "&vst_ce_event_type=" + encodeURI(encodeURI($("#vst_ce_event_type").val()));
					params += "&vst_ce_event_name=" + encodeURI(encodeURI($("#vst_ce_event_name").val()));
					params += "&vst_ce_event_key=" + encodeURI(encodeURI($("#vst_ce_event_key").val()));
					params += "&vst_ce_event_value=" + encodeURI(encodeURI($("#vst_ce_event_value").val()));
					params += "&export_column=" + encodeURI(encodeURI(export_column));
					
					location.href = "${ctx}/customEvent/exportData.action?" + params
