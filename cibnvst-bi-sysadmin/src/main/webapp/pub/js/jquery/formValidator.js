...
$.formValidator = 
{
	...
	sustainType : function(id,setting)
	{
		...
		switch(setting.validatetype)
		{
			...
			case "RegexValidator":
				return ((srcTag == "INPUT" || srcTag == "TEXTAREA" || stype == "password") ? (stype != "checkbox" && stype != "radio") : false);
	...
	showAjaxMessage : function(returnObj)
	{
		...
		var sendFlag = true;
		if(setting.validatetype == "AjaxValidator") {
			sendFlag = setting.isSend();
		}
		if(elem.validoldvalue!=$(elem).val() || (("submit" == event.type || "blur" == event.type) && 13 == curEventObj.which ))
		{
			$.formValidator.ajaxValid(returnObj);
		}
		else
		{	
			if(!sendFlag) {
				...
				elem.lastshowmsg = setting.oncorrect;
    ...
    getLength : function(id)
    {
        ...
        switch(sType)
		{
			...
			case "checkbox":
			case "radio": 
				var tempName = srcjo.attr("name").replace(/\./g, "\\.").replace(/\[/g,"\\[").replace(/\]/g,"\\]").replace(/\'/g,"\\'");
				len = $("input[type='"+sType+"'][name='"+tempName+"']:checked").length;
				break;
		    case "select-one":
	...
	ajaxValid : function(returnObj)
	{
		...
		var isSend = setting.isSend();
	    if (srcjo.size() == 0 && settings[0].empty || !isSend) {
			returnObj.setting = settings[0];
			returnObj.isvalid = true;
			$.formValidator.showMessage(returnObj);
		...
		if(setting.data.length >0){
			ls_url = ls_url + "&" + setting.data +"="+ srcjo.val();
		}
		//发送ajax请求
		$.ajax(
		{	
	...
	regexValid : function(returnObj)
	{
		...
		else 
		{
			...
			$.each(regexpresslist, function() {
			    ...
			    else if(setting.comparetype=="&&" && !isvalid) 
			    {
			        setting.isvalid = false;
			        return true
			    } else {
...
$.fn.formValidator = function(cs) 
{
	...
	return this.each(function(e)
	{
		...
		if(srcTag == "input" || srcTag=="textarea")
		{
			...
			jqobj.bind(setting.triggerevent, function(e){
				var settings = this.settings;
