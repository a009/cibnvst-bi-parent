...
import com.vst.defend.service.home.VstHomeContentClickService;
import com.vst.defend.service.task.VstExportTaskService;

/**
 * 首页内容点击
 * @author lhp
 * @date 2018-4-10 下午04:55:21
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/homeContentClick")
public class VstHomeContentClickController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstHomeContentClickController.class.getName();
	
	/**
	 * 表名
	 */
	private static final String _tableName = "vst_home_content_click";
	 ...
	 */
	@Resource
	private VstHomeContentClickService _vstHomeContentClickService;
	 ...
	 * @return
	 */
	@RequestMapping("/findHomeContentClick.action")
	public String findHomeContentClick(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_homeContentClick");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			params.put("vst_table_name", "vst_home_content_click");
			params.put("vst_column_name", "vst_hcc_type");
			params.put("vst_state", VstConstants.STATE_AVALIABLE);
			request.setAttribute("types", _baseService.getDictionaryForMap(params));
			// 获取影片分类
			params.put("vst_table_name", "vst_home_content_click");
			params.put("vst_column_name", "vst_hcc_cid");
			...
			columns.put("vst_hcc_type", "类型");
			columns.put("vst_hcc_cid", "影片分类");
			columns.put("vst_hcc_uuid", "影片ID");
			columns.put("vst_hcc_name", "名称");
			columns.put("vst_hcc_vv", "点击量");
			columns.put("vst_hcc_vv_dod", "点击量天环比");
			columns.put("vst_hcc_vv_wow", "点击量周环比");
			columns.put("vst_hcc_uv", "独立用户");
			columns.put("vst_hcc_uv_dod", "独立用户天环比");
			columns.put("vst_hcc_uv_wow", "独立用户周环比");
			columns.put("vst_hcc_avg", "人均点击量");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc(_tableName));
		} catch (Exception e) {
			logger.error("findHomeContentClick error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_homeContentClick");
		}
		
		return "home/homeContentClick_list";
	}
...
//			queryParam.put("vst_hcc_type", ToolsString.checkEmpty(request.getParameter("vst_hcc_type")));
//			queryParam.put("vst_hcc_cid", ToolsString.checkEmpty(request.getParameter("vst_hcc_cid")));
//			queryParam.put("vst_hcc_uuid", ToolsString.checkEmpty(request.getParameter("vst_hcc_uuid")));
//			queryParam.put("export_column", ToolsString.checkEmpty(request.getParameter("export_column")));
//			// 参数&符还原
//			String vst_hcc_name = ToolsString.checkEmpty(request.getParameter("vst_hcc_name"));
//			vst_hcc_name = vst_hcc_name.replace("<@_@>", "&");
//			queryParam.put("vst_hcc_name", vst_hcc_name);
//			// 编码转码
//			VstTools.decodeToUTF_8(queryParam);
//			// 获取模块ID
//			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
//			ReportBean bean = _vstHomeContentClickService.getExportData(queryParam, getUserSession(moduleId));
...
//	}
	...
	public String exportData(){
		...
		try{
			...
			final String vst_hcc_type = ToolsString.checkEmpty(request.getParameter("vst_hcc_type"));
			final String vst_hcc_cid = ToolsString.checkEmpty(request.getParameter("vst_hcc_cid"));
			final String vst_hcc_uuid = ToolsString.checkEmpty(request.getParameter("vst_hcc_uuid"));
			final String vst_hcc_name = URLDecoder.decode(ToolsString.checkEmpty(request.getParameter("vst_hcc_name")), "UTF-8");
				...
				Runnable runnable = new Runnable() {
					...
					public void run() {
						...
						try{
							...
							queryParam.put("vst_hcc_uuid", vst_hcc_uuid);
							// 参数&符还原
							String new_hcc_name = vst_hcc_name.replace("<@_@>", "&");
							queryParam.put("vst_hcc_name", new_hcc_name);
							// 1、插入导出任务表
