...
@Scope("prototype")
@Controller
@RequestMapping("/outerUserSecondaryActiveChannelDiff")
public class OuterUserSecondaryActiveChannelDiffController extends BaseController {
    /**
     * 类名
     */
    private static final String _className = OuterUserSecondaryActiveChannelDiffController.class.getName();
    ...
    /**
     * 二次活跃渠道用户(对外)Service接口
     */
    @Resource
    private OuterUserSecondaryActiveChannelDiffService _outerUserSecondaryActiveChannelDiffService;
     ...
     * @return
     */
    @RequestMapping("/findOuterUserSecondaryActiveChannelDiff.action")
    public String findOuterUserSecondaryActiveChannelDiff(){
        try {
            // 初始化
            this.initializeAction(_className);

            Object obj = session.getAttribute("cutPage_outerUserSecondaryActiveChannelDiff");
            if(obj != null){
                cutPage = (CutPage) obj;
            }else{
                cutPage = new CutPage();
            ...
            logger.error("findOuterUserSecondaryActiveChannelDiff error." + SystemException.getExceptionInfo(e));
            // 转向失败页面
            return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
                    .getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
        } finally {
            // 记得移除session中的分页信息
            session.removeAttribute("cutPage_outerUserSecondaryActiveChannelDiff");
        }

        return "outer/outerUserSecondaryActiveChannelDiff_list";
    }
    ...
    public String ajaxGetCutPage(){
        try{
            ...
            queryParam.put("vst_usacd_channel", vst_sys_channel);
            // 排序信息
            ...
            ReportBean bean = _outerUserSecondaryActiveChannelDiffService.getCutPageData(cutPage);
    ...
    public String exportData(){
        try{
            ...
            // 获取模块ID
            String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
            ReportBean bean = _outerUserSecondaryActiveChannelDiffService.getExportData(queryParam, getUserSession(moduleId));
    ...
    public String ajaxGetReportByDate(){
        try{
            ...
            ReportBean result = _outerUserSecondaryActiveChannelDiffService.getReportByDate(queryParam);
