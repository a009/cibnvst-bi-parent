 ...
 * 活跃用户(渠道、设备)
 * @author lhp
 * @date 2019-3-6 上午11:55:38
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/userActiveChannelBdmodel")
public class VstUserActiveChannelBdmodelController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstUserActiveChannelBdmodelController.class.getName();
	...
	/**
	 * 活跃用户(渠道、设备)Service接口
	 */
	@Resource
	private VstUserActiveChannelBdmodelService _vstUserActiveChannelBdmodelService;
	 ...
	 * @return
	 */
	@RequestMapping("/findUserActiveChannelBdmodel.action")
	public String findUserActiveChannelBdmodel(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_userActiveChannelBdmodel");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			columns.put("vst_uacb_week", "周活跃");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc("vst_user_active_channel_bdmodel"));
		} catch (Exception e) {
			logger.error("findUserActiveChannelBdmodel error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_userActiveChannelBdmodel");
		}
		
		return "user/userActiveChannelBdmodel_list";
	}
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _vstUserActiveChannelBdmodelService.getExportData(queryParam, getUserSession(moduleId));
	...
	public String ajaxGetReportByBdmodel(){
		try{
			...
			JSONObject data = _vstUserActiveChannelBdmodelService.getReportByBdmodel(queryParam);
