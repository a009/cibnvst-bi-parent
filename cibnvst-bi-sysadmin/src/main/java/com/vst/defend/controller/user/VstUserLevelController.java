...
import com.vst.defend.service.user.VstUserLevelService;

/**
 * 季质量用户
 * @author lhp
 * @date 2018-1-4 下午02:31:40
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/userLevel")
public class VstUserLevelController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstUserLevelController.class.getName();
	 ...
	 */
	@Resource
	private VstUserLevelService _vstUserLevelService;
	 ...
	 * @return
	 */
	@RequestMapping("/findUserLevel.action")
	public String findUserLevel(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_userLevel");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			columns.put("vst_ul_date", "日期");
			columns.put("vst_ul_pkg", "包名");
			columns.put("vst_ul_add_amount", "季新增用户数");
			columns.put("vst_ul_season_amount", "季质量用户数");
			columns.put("vst_ul_season_dod", "天环比");
			columns.put("vst_ul_season_wow", "周环比");
			columns.put("vst_ul_season_radio", "占比");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc("vst_user_level"));
		} catch (Exception e) {
			logger.error("findUserLevel error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_userLevel");
		}
		
		return "user/userLevel_list";
	}
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _vstUserLevelService.getExportData(queryParam, getUserSession(moduleId));
