...
import com.vst.defend.service.user.VstUserSecondaryActiveService;
...
@Scope("session")
@Controller
@RequestMapping("/userSecondaryActive")
public class VstUserSecondaryActiveController extends BaseController {
    /**
     * 类名
     */
    private static final String _className = VstUserSecondaryActiveController.class.getName();
     ...
     */
    @Resource
    private VstUserSecondaryActiveService _vstUserSecondaryActiveService;
     ...
     * @return
     */
    @RequestMapping("/findUserSecondaryActive.action")
    public String findUserSecondaryActive() {
        try {
            // 初始化
            this.initializeAction(_className);

            Object obj = session.getAttribute("cutPage_userSecondaryActive");
            if (obj != null) {
                cutPage = (CutPage) obj;
            } else {
                cutPage = new CutPage();
            ...
            columns.put("vst_usa_date", "日期");
            columns.put("vst_usa_pkg", "包名");
            columns.put("vst_usa_amount", "二次活跃用户数");
            columns.put("vst_usa_dod", "天环比");
            columns.put("vst_usa_wow", "周环比");
            columns.put("vst_usa_mom", "月环比");
            request.setAttribute("columns", columns);
            // 获取表描述
            request.setAttribute("tableDesc", _baseService.getTableDesc("vst_user_secondary_active"));
        } catch (Exception e) {
            logger.error("findUserSecondaryActive error." + SystemException.getExceptionInfo(e));
            // 转向失败页面
            return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
                    .getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
        } finally {
            // 记得移除session中的分页信息
            session.removeAttribute("cutPage_userSecondaryActive");
        }

        return "user/userSecondaryActive_list";
    }
    ...
    public String exportData() {
        try {
            ...
            // 获取模块ID
            String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
            ReportBean bean = _vstUserSecondaryActiveService.getExportData(queryParam, getUserSession(moduleId));
