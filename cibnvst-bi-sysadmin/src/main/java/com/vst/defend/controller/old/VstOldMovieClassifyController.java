 ...
 * 影片分类统计
 * @author lhp
 * @date 2017-11-17 上午11:07:40
 * @version
 */
@Controller
@RequestMapping("/oldMovieClassify")
public class VstOldMovieClassifyController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstOldMovieClassifyController.class.getName();
	 ...
	 * @return
	 */
	@RequestMapping("/findMovieClassify.action")
	public String findMovieClassify(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_movieClassify");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			logger.error("findMovieClassify error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_movieClassify");
		}
		
		return "old/movieClassify_list";
	}
	...
	public String exportData(){
		try{
			...
			File file = ExportUtil.ExportExcel(dataList, PropertiesReader.getProperty("export_dir"), "vst_movie_classify_data");
			if(file.exists()){
				String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
				UserSession user = getUserSession(moduleId);
				// 写操作日志
				StringBuilder sb = new StringBuilder();
				sb.append("导出表名：vst_movie_classify_data")
