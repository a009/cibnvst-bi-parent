...
import com.vst.defend.service.user.VstUserAddChannelService;
...
@Scope("session")
@Controller
@RequestMapping("/userAddChannel")
public class VstUserAddChannelController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstUserAddChannelController.class.getName();
	 ...
	 */
	@Resource
	private VstUserAddChannelService _vstUserAddChannelService;
	 ...
	 * @return
	 */
	@RequestMapping("/findUserAddChannel.action")
	public String findUserAddChannel(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_userAddChannel");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			columns.put("vst_uac_amount", "新增用户数");
			columns.put("vst_uac_dod", "天环比");
			columns.put("vst_uac_wow", "周环比");
			columns.put("vst_uac_mom", "月环比");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc("vst_user_add_channel"));
		} catch (Exception e) {
			logger.error("findUserAddChannel error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_userAddChannel");
		}
		
		return "user/userAddChannel_list";
	}
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _vstUserAddChannelService.getExportData(queryParam, getUserSession(moduleId));
	...
	public String ajaxGetReportByChannel(){
		try{
			...
			JSONObject data = _vstUserAddChannelService.getReportByChannel(queryParam);
