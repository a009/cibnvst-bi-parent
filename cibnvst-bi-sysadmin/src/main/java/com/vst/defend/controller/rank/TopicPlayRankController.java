...
import com.vst.defend.service.rank.TopicPlayRankService;

/**
 * 专题播放排行
 * @author lhp
 * @date 2018-2-1 下午04:27:49
 * @version
 */
@Scope("prototype")
@Controller
@RequestMapping("/topicPlayRank")
public class TopicPlayRankController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = TopicPlayRankController.class.getName();
	 ...
	 */
	@Resource
	private TopicPlayRankService _topicPlayRankService;
	 ...
	 * @return
	 */
	@RequestMapping("/findTopicPlayRank.action")
	public String findTopicPlayRank(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_topicPlayRank");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			logger.error("findTopicPlayRank error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_topicPlayRank");
		}
		
		return "rank/topicPlayRank_list";
	}
	...
	public String ajaxGetCutPage(){
		try{
			...
			ReportBean bean = _topicPlayRankService.getCutPageData(cutPage);
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _topicPlayRankService.getExportData(queryParam, getUserSession(moduleId));
