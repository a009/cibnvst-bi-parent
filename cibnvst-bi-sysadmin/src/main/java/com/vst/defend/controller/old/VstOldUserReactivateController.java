 ...
 * 二次激活用户
 * @author lhp
 * @date 2017-11-21 下午02:59:34
 * @version
 */
@Controller
@RequestMapping("/oldUserReactivate")
public class VstOldUserReactivateController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstOldUserReactivateController.class.getName();
	 ...
	 * @return
	 */
	@RequestMapping("/findUserReactivate.action")
	public String findUserReactivate(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_userReactivate");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			logger.error("findUserReactivate error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_userReactivate");
		}
		
		return "old/userReactivate_list";
	}
	...
	public String exportData(){
		try{
			...
			File file = ExportUtil.ExportExcel(dataList, PropertiesReader.getProperty("export_dir"), "vst_user_reactivate_data");
			if(file.exists()){
				String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
				UserSession user = getUserSession(moduleId);
				// 写操作日志
				StringBuilder sb = new StringBuilder();
				sb.append("导出表名：vst_user_reactivate_data")
