...
import com.vst.defend.controller.user.VstUserLevelChannelController;
import com.vst.defend.service.outer.OuterUserLevelChannelService;

/**
 * 季质量渠道用户(对外)
 * @author lhp
 * @date 2018-1-16 下午06:58:00
 * @version
 */
@Scope("prototype")
@Controller
@RequestMapping("/outerUserLevelChannel")
public class OuterUserLevelChannelController extends BaseController {
	/**
	 * 类名
	 */
	 ...
	 */
	@Resource
	private OuterUserLevelChannelService _outerUserLevelChannelService;
	 ...
	 * @return
	 */
	@RequestMapping("/findOuterUserLevelChannel.action")
	public String findOuterUserLevelChannel(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_outerUserLevelChannel");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			logger.error("findOuterUserLevelChannel error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_outerUserLevelChannel");
		}
		
		return "outer/outerUserLevelChannel_list";
	}
	...
	public String ajaxGetCutPage(){
		try{
			...
			queryParam.put("vst_ulc_channel", vst_sys_channel);
			// 排序信息
			...
			ReportBean bean = _outerUserLevelChannelService.getCutPageData(cutPage);
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _outerUserLevelChannelService.getExportData(queryParam, getUserSession(moduleId));
	...
	public String ajaxGetReportByDate(){
		try{
			...
			ReportBean result = _outerUserLevelChannelService.getReportByDate(queryParam);
	...
	public String ajaxGetReportByChannel(){
		try{
			...
			ReportBean result = _outerUserLevelChannelService.getReportByChannel(queryParam);
			List<Map<String, Object>> data = result.getMapData();
