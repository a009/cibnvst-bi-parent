 ...
 * 影片搜索排行
 * @author lhp
 * @date 2018-2-1 下午03:44:01
 * @version
 */
@Scope("prototype")
@Controller
@RequestMapping("/movieSearchRank")
public class MovieSearchRankController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = MovieSearchRankController.class.getName();
	 ...
	 */
	@Resource
	private MovieSearchRankService _movieSearchRankService;
	 ...
	 * @return
	 */
	@RequestMapping("/findMovieSearchRank.action")
	public String findMovieSearchRank(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_movieSearchRank");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			logger.error("findMovieSearchRank error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_movieSearchRank");
		}
		
		return "rank/movieSearchRank_list";
	}
	...
	public String ajaxGetCutPage(){
		try{
			...
			ReportBean bean = _movieSearchRankService.getCutPageData(cutPage);
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _movieSearchRankService.getExportData(queryParam, getUserSession(moduleId));
