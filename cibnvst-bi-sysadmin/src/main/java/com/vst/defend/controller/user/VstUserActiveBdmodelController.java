...
import com.vst.defend.service.user.VstUserActiveBdmodelService;

/**
 * 活跃用户(设备)
 * @author lhp
 * @date 2019-3-6 上午10:05:35
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/userActiveBdmodel")
public class VstUserActiveBdmodelController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstUserActiveBdmodelController.class.getName();
	 ...
	 */
	@Resource
	private VstUserActiveBdmodelService _vstUserActiveBdmodelService;
	 ...
	 * @return
	 */
	@RequestMapping("/findUserActiveBdmodel.action")
	public String findUserActiveBdmodel(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_userActiveBdmodel");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			columns.put("vst_uab_week", "周活跃");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc("vst_user_active_bdmodel"));
		} catch (Exception e) {
			logger.error("findUserActiveBdmodel error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_userActiveBdmodel");
		}
		
		return "user/userActiveBdmodel_list";
	}
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _vstUserActiveBdmodelService.getExportData(queryParam, getUserSession(moduleId));
