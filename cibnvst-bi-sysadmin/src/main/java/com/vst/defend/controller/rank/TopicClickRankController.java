 ...
 * 专题点击排行
 * @author lhp
 * @date 2018-2-1 下午04:04:18
 * @version
 */
@Scope("prototype")
@Controller
@RequestMapping("/topicClickRank")
public class TopicClickRankController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = TopicClickRankController.class.getName();
	 ...
	 */
	@Resource
	private TopicClickRankService _topicClickRankService;
	 ...
	 * @return
	 */
	@RequestMapping("/findTopicClickRank.action")
	public String findTopicClickRank(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_topicClickRank");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			logger.error("findTopicClickRank error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_topicClickRank");
		}
		
		return "rank/topicClickRank_list";
	}
	...
	public String ajaxGetCutPage(){
		try{
			...
			ReportBean bean = _topicClickRankService.getCutPageData(cutPage);
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _topicClickRankService.getExportData(queryParam, getUserSession(moduleId));
