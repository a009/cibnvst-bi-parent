 ...
 * 活跃用户(版本、设备)
 * @author lhp
 * @date 2019-3-12 下午03:53:48
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/userActiveVersionBdmodel")
public class VstUserActiveVersionBdmodelController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstUserActiveVersionBdmodelController.class.getName();
	...
	/**
	 * 活跃用户(版本、设备)Service接口
	 */
	@Resource
	private VstUserActiveVersionBdmodelService _vstUserActiveVersionBdmodelService;
	 ...
	 * @return
	 */
	@RequestMapping("/findUserActiveVersionBdmodel.action")
	public String findUserActiveVersionBdmodel(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_userActiveVersionBdmodel");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			columns.put("vst_uavb_week", "周活跃");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc("vst_user_active_version_bdmodel"));
		} catch (Exception e) {
			logger.error("findUserActiveVersionBdmodel error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_userActiveVersionBdmodel");
		}
		
		return "user/userActiveVersionBdmodel_list";
	}
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _vstUserActiveVersionBdmodelService.getExportData(queryParam, getUserSession(moduleId));
	...
	public String ajaxGetReportByBdmodel(){
		try{
			...
			JSONObject data = _vstUserActiveVersionBdmodelService.getReportByBdmodel(queryParam);
