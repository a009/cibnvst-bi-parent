...
@Scope("prototype")
@Controller
@RequestMapping("/innerUserSecondaryActiveChannel")
public class InnerUserSecondaryActiveChannelController extends BaseController {
    /**
     * 类名
     */
    private static final String _className = InnerUserSecondaryActiveChannelController.class.getName();
     ...
     */
    @Resource
    private InnerUserSecondaryActiveChannelService _innerUserSecondaryActiveChannelService;
     ...
     * @return
     */
    @RequestMapping("/findInnerUserSecondaryActiveChannel.action")
    public String findInnerUserSecondaryActiveChannel(){
        try {
            // 初始化
            this.initializeAction(_className);

            Object obj = session.getAttribute("cutPage_innerUserSecondaryActiveChannel");
            if(obj != null){
                cutPage = (CutPage) obj;
            }else{
                cutPage = new CutPage();
            ...
            logger.error("findInnerUserSecondaryActiveChannel error." + SystemException.getExceptionInfo(e));
            // 转向失败页面
            return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
                    .getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
        } finally {
            // 记得移除session中的分页信息
            session.removeAttribute("cutPage_innerUserSecondaryActiveChannel");
        }

        return "outer/innerUserSecondaryActiveChannel_list";
    }
    ...
    public String ajaxGetCutPage(){
        try{
            ...
            queryParam.put("vst_usac_state", ToolsString.checkEmpty(request.getParameter("vst_usac_state")));
            queryParam.put("vst_usac_updator", ToolsString.checkEmpty(request.getParameter("vst_usac_updator")));
            // 排序信息
            ...
            ReportBean bean = _innerUserSecondaryActiveChannelService.getCutPageData(cutPage);
    ...
    public String exportData(){
        try{
            ...
            // 获取模块ID
            String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
            ReportBean bean = _innerUserSecondaryActiveChannelService.getExportData(queryParam, getUserSession(moduleId));
    ...
    public String ajaxGetReportByDate(){
        try{
            ...
            ReportBean result = _innerUserSecondaryActiveChannelService.getReportByDate(queryParam);
    ...
    public String ajaxAuditData(){
        ...
        try{
            ...
            String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
            int result = _innerUserSecondaryActiveChannelService.auditData(recordId, state, getUserSession(moduleId));
    ...
    public String ajaxBatchAuditData(){
        ...
        try {
            ...
            if(ToolsString.isEmpty(startDay) || ToolsString.isEmpty(endDay)){
                // 转向失败页面
                return this.moveToPage(VstConstants.OPERATE_INFO, "必选选择日期才能审核！", findInnerUserSecondaryActiveChannel());
            }

            Map<String, Object> params = new HashMap<String, Object>();
            params.put("startDay", startDay);
            params.put("endDay", endDay);
            params.put("vst_usac_channel", ToolsString.checkEmpty(request.getParameter("vst_usac_channel")));
            params.put("vst_usac_state", ToolsString.checkEmpty(request.getParameter("vst_usac_state")));
            params.put("vst_usac_updator", ToolsString.checkEmpty(request.getParameter("vst_usac_updator")));

            String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
            int result = _innerUserSecondaryActiveChannelService.batchAuditData(params, getUserSession(moduleId));
            if(result > 0){
                json.put("code", 100);
    ...
    public String ajaxModifyModulus(){
        ...
        try {
            ...
            String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
            _innerUserSecondaryActiveChannelService.modifyModulus(ids, modulus, getUserSession(moduleId));
            out.print(true);
            out.flush();
        } catch (Exception e) {
    ...
    public String ajaxBatchUpdate(){
        ...
        try {
            ...
            String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
            int result = _innerUserSecondaryActiveChannelService.modifyModulus(ids, modulus, getUserSession(moduleId));
            if(result > 0){
                json.put("code", 100);
            }else{
                json.put("code", 200);
    ...
    public String ajaxImportData(){
        ...
        try {
            ...
            String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
            int result = _innerUserSecondaryActiveChannelService.importData(date, getUserSession(moduleId));
            if(result > 0){
                json.put("code", 100);
                json.put("result", result);
            }else{
    ...
    public String ajaxDeleteData(){
        ...
        try {
            ...
            if(ToolsString.isEmpty(startDay) || ToolsString.isEmpty(endDay)){
                // 转向失败页面
                return this.moveToPage(VstConstants.OPERATE_INFO, "必选选择日期才能删除！", findInnerUserSecondaryActiveChannel());
            }
            ...
            String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
            int result = _innerUserSecondaryActiveChannelService.deleteData(params, getUserSession(moduleId));
            if(result > 0){
                json.put("code", 100);
                json.put("result", result);
            }else{
    ...
    public String ajaxSyncData(){
        ...
        try {
            ...
            String url = "http://bi.cibnvst.com:8081/cibnvst-bi-quartz/push/user/secondaryActive/channel?startDate={startDate}&endDate={endDate}"
    ...
    public String ajaxEditData(){
        ...
        try {
            ...
            Map<String, Object> params = new HashMap<String, Object>();
            params.put("vst_usac_id", recordId);
            params.put("vst_usac_amount", ToolsString.checkEmpty(request.getParameter("amount")));
            params.put("vst_usac_modulus", ToolsString.checkEmpty(request.getParameter("modulus")));

            String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
            int result = _innerUserSecondaryActiveChannelService.updateData(params, getUserSession(moduleId));
            if(result > 0){
                json.put("code", 100);
            }else{
                json.put("code", 200);
    ...
    public String ajaxGetReportByDim(){
        try{
            ...
            ReportBean result = _innerUserSecondaryActiveChannelService.getReportByDim(queryParam);
            List<Map<String, Object>> data = result.getMapData();
