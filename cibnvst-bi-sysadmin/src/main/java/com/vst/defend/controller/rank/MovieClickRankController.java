...
import com.vst.defend.service.rank.MovieClickRankService;

/**
 * 影片点击排行
 * @author lhp
 * @date 2018-2-1 上午11:15:43
 * @version
 */
@Scope("prototype")
@Controller
@RequestMapping("/movieClickRank")
public class MovieClickRankController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = MovieClickRankController.class.getName();
	 ...
	 */
	@Resource
	private MovieClickRankService _movieClickRankService;
	 ...
	 * @return
	 */
	@RequestMapping("/findMovieClickRank.action")
	public String findMovieClickRank(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_movieClickRank");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			logger.error("findMovieClickRank error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_movieClickRank");
		}
		
		return "rank/movieClickRank_list";
	}
	...
	public String ajaxGetCutPage(){
		try{
			...
			ReportBean bean = _movieClickRankService.getCutPageData(cutPage);
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _movieClickRankService.getExportData(queryParam, getUserSession(moduleId));
