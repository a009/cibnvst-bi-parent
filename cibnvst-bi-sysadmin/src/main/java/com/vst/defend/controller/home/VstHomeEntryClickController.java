...
import com.vst.defend.service.home.VstHomeEntryClickService;
import com.vst.defend.service.task.VstExportTaskService;

/**
 * 首页入口点击
 * @author lhp
 * @date 2018-2-28 上午11:35:44
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/homeEntryClick")
public class VstHomeEntryClickController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstHomeEntryClickController.class.getName();
	
	/**
	 * 表名
	 */
	private static final String _tableName = "vst_home_entry_click";
	 ...
	 */
	@Resource
	private VstHomeEntryClickService _vstHomeEntryClickService;
	 ...
	 * @return
	 */
	@RequestMapping("/findHomeEntryClick.action")
	public String findHomeEntryClick(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_homeEntryClick");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			params.put("vst_table_name", "vst_home_entry_click");
			params.put("vst_column_name", "vst_hec_type");
			...
			columns.put("vst_hec_date", "日期");
			columns.put("vst_hec_pkg", "包名");
			columns.put("vst_hec_type", "类型");
			columns.put("vst_hec_name", "名称");
			columns.put("vst_hec_vv", "点击量");
			columns.put("vst_hec_vv_dod", "点击量天环比");
			columns.put("vst_hec_vv_wow", "点击量周环比");
			columns.put("vst_hec_uv", "独立用户");
			columns.put("vst_hec_uv_dod", "独立用户天环比");
			columns.put("vst_hec_uv_wow", "独立用户周环比");
			columns.put("vst_hec_avg", "人均点击量");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc(_tableName));
		} catch (Exception e) {
			logger.error("findHomeEntryClick error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_homeEntryClick");
		}
		
		return "home/homeEntryClick_list";
	}
...
//			queryParam.put("vst_hec_type", ToolsString.checkEmpty(request.getParameter("vst_hec_type")));
//			queryParam.put("export_column", ToolsString.checkEmpty(request.getParameter("export_column")));
//			// 参数&符还原
//			String vst_hec_name = ToolsString.checkEmpty(request.getParameter("vst_hec_name"));
//			vst_hec_name = vst_hec_name.replace("<@_@>", "&");
//			queryParam.put("vst_hec_name", vst_hec_name);
//			// 编码转码
//			VstTools.decodeToUTF_8(queryParam);
//			// 获取模块ID
//			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
//			ReportBean bean = _vstHomeEntryClickService.getExportData(queryParam, getUserSession(moduleId));
...
//	}
	...
	public String exportData(){
		...
		try{
			...
			final String vst_hec_type = ToolsString.checkEmpty(request.getParameter("vst_hec_type"));
			final String vst_hec_name = URLDecoder.decode(ToolsString.checkEmpty(request.getParameter("vst_hec_name")), "UTF-8");
				...
				Runnable runnable = new Runnable() {
					...
					public void run() {
						...
						try{
							...
							queryParam.put("vst_hec_type", vst_hec_type);
							// 参数&符还原
							String new_hec_name = vst_hec_name.replace("<@_@>", "&");
							queryParam.put("vst_hec_name", new_hec_name);
							// 1、插入导出任务表
