 ...
 * 专题汇总
 * @author lhp
 * @date 2017-11-23 下午04:04:57
 * @version
 */
@Controller
@RequestMapping("/oldTopicTotal")
public class VstOldTopicTotalController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstOldTopicTotalController.class.getName();
	 ...
	 * @return
	 */
	@RequestMapping("/findTopicTotal.action")
	public String findTopicTotal(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_topicTotal");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			// 获取专题类型
			params.put("vst_table_name", "vst_topic");
			params.put("vst_column_name", "topicType");
			request.setAttribute("topicTypes", _baseService.getDictionaryForMap(params));
		} catch (Exception e) {
			logger.error("findTopicTotal error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_topicTotal");
		}
		
		return "old/topicTotal_list";
	}
	...
	public String exportData(){
		try{
			...
			File file = ExportUtil.ExportExcel(dataList, PropertiesReader.getProperty("export_dir"), "vst_topic_total_data");
			if(file.exists()){
				String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
				UserSession user = getUserSession(moduleId);
				// 写操作日志
				StringBuilder sb = new StringBuilder();
				sb.append("导出表名：vst_topic_total_data")
