...
@Scope("session")
@Controller
@RequestMapping("/marketUserLevelChannel")
public class MarketUserLevelChannelController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = MarketUserLevelChannelController.class.getName();
	...
	/**
	 * 季质量渠道用户(市场数据)Service接口
	 */
	@Resource
	private MarketUserLevelChannelService _marketUserLevelChannelService;
	 ...
	 * @return
	 */
	@RequestMapping("/findMarketUserLevelChannel.action")
	public String findMarketUserLevelChannel(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_marketUserLevelChannel");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			logger.error("findMarketUserLevelChannel error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_marketUserLevelChannel");
		}
		
		return "outer/marketUserLevelChannel_list";
	}
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _marketUserLevelChannelService.getExportData(queryParam, getUserSession(moduleId));
