 ...
 * 新增用户(城市)
 * @author lhp
 * @date 2018-1-3 下午05:41:38
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/userAddCity")
public class VstUserAddCityController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstUserAddCityController.class.getName();
	 ...
	 */
	@Resource
	private VstUserAddCityService _vstUserAddCityService;
	 ...
	 * @return
	 */
	@RequestMapping("/findUserAddCity.action")
	public String findUserAddCity(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_userAddCity");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			columns.put("vst_uaci_amount", "新增用户数");
			columns.put("vst_uaci_dod", "天环比");
			columns.put("vst_uaci_wow", "周环比");
			columns.put("vst_uaci_mom", "月环比");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc("vst_user_add_city"));
		} catch (Exception e) {
			logger.error("findUserAddCity error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_userAddCity");
		}
		
		return "user/userAddCity_list";
	}
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _vstUserAddCityService.getExportData(queryParam, getUserSession(moduleId));
	...
	public String ajaxGetReportByCity(){
		try{
			...
			ReportBean result = _vstUserAddCityService.getReportByCity(queryParam);
