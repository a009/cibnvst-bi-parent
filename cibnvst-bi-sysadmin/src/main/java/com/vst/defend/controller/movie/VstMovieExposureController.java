...
import com.vst.defend.service.movie.VstMovieExposureService;
...
@Scope("session")
@Controller
@RequestMapping("/movieExposure")
public class VstMovieExposureController extends BaseController {
    /**
     * 类名
     */
    private static final String _className = VstMovieExposureController.class.getName();
     ...
     */
    @Resource
    private VstMovieExposureService _vstMovieExposureService;
     ...
     * @return
     */
    @RequestMapping("/findMovieExposure.action")
    public String findMovieExposure(){
        try {
            // 初始化
            this.initializeAction(_className);

            Object obj = session.getAttribute("cutPage_movieExposure");
            if(obj != null){
                cutPage = (CutPage) obj;
            }else{
                cutPage = new CutPage();
            ...
            columns.put("vst_me_date", "日期");
            columns.put("vst_me_pkg", "包名");
            columns.put("vst_me_special_type", "专区类型");
            columns.put("vst_me_cid", "影片分类");
            columns.put("vst_me_uuid", "影片ID");
            columns.put("vst_me_name", "影片名称");
            columns.put("vst_me_amount", "曝光量");
            columns.put("vst_me_amount_dod", "曝光量天环比");
            columns.put("vst_me_amount_wow", "曝光量周环比");
            columns.put("vst_me_uv", "独立用户");
            columns.put("vst_me_uv_dod", "独立用户天环比");
            columns.put("vst_me_uv_wow", "独立用户周环比");
            columns.put("vst_me_avg", "人均曝光量");
            request.setAttribute("columns", columns);
            // 获取表描述
            request.setAttribute("tableDesc", _baseService.getTableDesc("vst_movie_exposure"));
        } catch (Exception e) {
            logger.error("findMovieExposure error." + SystemException.getExceptionInfo(e));
            // 转向失败页面
            return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
                    .getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
        } finally {
            // 记得移除session中的分页信息
            session.removeAttribute("cutPage_movieExposure");
        }

        return "movie/movieExposure_list";
    }
    ...
    public String exportData(){
        try{
            ...
            queryParam.put("vst_me_special_type", ToolsString.checkEmpty(request.getParameter("vst_me_special_type")));
            queryParam.put("vst_me_cid", ToolsString.checkEmpty(request.getParameter("vst_me_cid")));
            queryParam.put("vst_me_uuid", ToolsString.checkEmpty(request.getParameter("vst_me_uuid")));
            queryParam.put("vst_me_name", ToolsString.checkEmpty(request.getParameter("vst_me_name")));
            queryParam.put("export_column", ToolsString.checkEmpty(request.getParameter("export_column")));
            // 编码转码
            VstTools.decodeToUTF_8(queryParam);
            // 获取模块ID
            String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
            ReportBean bean = _vstMovieExposureService.getExportData(queryParam, getUserSession(moduleId));
