...
import com.vst.defend.service.user.VstUserAddService;

/**
 * 新增用户
 * @author lhp
 * @date 2017-12-29 下午03:02:05
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/userAdd")
public class VstUserAddController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstUserAddController.class.getName();
	 ...
	 */
	@Resource
	private VstUserAddService _vstUserAddService;
	 ...
	 * @return
	 */
	@RequestMapping("/findUserAdd.action")
	public String findUserAdd(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_userAdd");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			columns.put("vst_ua_amount", "新增用户数");
			columns.put("vst_ua_dod", "天环比");
			columns.put("vst_ua_wow", "周环比");
			columns.put("vst_ua_mom", "月环比");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc("vst_user_add"));
		} catch (Exception e) {
			logger.error("findUserAdd error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_userAdd");
		}
		
		return "user/userAdd_list";
	}
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _vstUserAddService.getExportData(queryParam, getUserSession(moduleId));
