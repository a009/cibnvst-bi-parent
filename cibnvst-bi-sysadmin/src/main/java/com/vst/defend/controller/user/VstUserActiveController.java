...
import com.vst.defend.service.user.VstUserActiveService;

/**
 * 活跃用户
 * @author lhp
 * @date 2018-1-3 下午06:23:41
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/userActive")
public class VstUserActiveController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstUserActiveController.class.getName();
	 ...
	 */
	@Resource
	private VstUserActiveService _vstUserActiveService;
	 ...
	 * @return
	 */
	@RequestMapping("/findUserActive.action")
	public String findUserActive(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_userActive");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			columns.put("vst_ua_date", "日期");
			columns.put("vst_ua_pkg", "包名");
			columns.put("vst_ua_amount", "日活跃用户");
			columns.put("vst_ua_dod", "日活跃天环比");
			columns.put("vst_ua_wow", "日活跃周环比");
			columns.put("vst_ua_mom", "日活跃月环比");
			columns.put("vst_ua_week", "周活跃用户");
			columns.put("vst_ua_week_dod", "周活跃天环比");
			columns.put("vst_ua_week_wow", "周活跃周环比");
			columns.put("vst_ua_week_mom", "周活跃月环比");
			columns.put("vst_ua_month", "月活跃用户");
			columns.put("vst_ua_month_dod", "月活跃天环比");
			columns.put("vst_ua_month_wow", "月活跃周环比");
			columns.put("vst_ua_month_mom", "月活跃月环比");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc("vst_user_active"));
		} catch (Exception e) {
			logger.error("findUserActive error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_userActive");
		}
		
		return "user/userActive_list";
	}
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _vstUserActiveService.getExportData(queryParam, getUserSession(moduleId));
