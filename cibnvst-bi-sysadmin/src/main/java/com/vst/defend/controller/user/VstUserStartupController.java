...
import com.vst.defend.service.user.VstUserStartupService;

/**
 * 启动次数
 * @author lhp
 * @date 2018-2-5 上午10:24:27
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/userStartup")
public class VstUserStartupController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstUserStartupController.class.getName();
	 ...
	 */
	@Resource
	private VstUserStartupService _vstUserStartupService;
	 ...
	 * @return
	 */
	@RequestMapping("/findUserStartup.action")
	public String findUserStartup(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_userStartup");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			columns.put("vst_us_date", "日期");
			columns.put("vst_us_pkg", "包名");
			columns.put("vst_us_amount", "启动次数");
			columns.put("vst_us_dod", "启动次数天环比");
			columns.put("vst_us_wow", "启动次数周环比");
			columns.put("vst_us_mom", "启动次数月环比");
			columns.put("vst_us_uv", "独立用户");
			columns.put("vst_us_uv_dod", "独立用户天环比");
			columns.put("vst_us_uv_wow", "独立用户周环比");
			columns.put("vst_us_uv_mom", "独立用户月环比");
			columns.put("vst_us_avg", "人均启动次数");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc("vst_user_startup"));
		} catch (Exception e) {
			logger.error("findUserStartup error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_userStartup");
		}
		
		return "user/userStartup_list";
	}
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _vstUserStartupService.getExportData(queryParam, getUserSession(moduleId));
