 ...
 * 历史选集统计
 * @author lhp
 * @date 2017-11-23 上午10:13:30
 * @version
 */
@Controller
@RequestMapping("/oldHistorySubmovie")
public class VstOldHistorySubmovieController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstOldHistorySubmovieController.class.getName();
	 ...
	 * @return
	 */
	@RequestMapping("/findHistorySubmovie.action")
	public String findHistorySubmovie(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_historySubmovie");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			logger.error("findHistorySubmovie error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_historySubmovie");
		}
		
		return "old/historySubmovie_list";
	}
	...
	public String exportData(){
		try{
			...
			File file = ExportUtil.ExportExcel(dataList, PropertiesReader.getProperty("export_dir"), "vst_history_submovie_data");
			if(file.exists()){
				String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
				UserSession user = getUserSession(moduleId);
				// 写操作日志
				StringBuilder sb = new StringBuilder();
				sb.append("导出表名：vst_history_submovie_data")
