 ...
 * 影片分类播放
 * @author lhp
 * @date 2017-12-1 下午03:18:29
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/movieClassifyPlay")
public class VstMovieClassifyPlayController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstMovieClassifyPlayController.class.getName();
	 ...
	 */
	@Resource
	private VstMovieClassifyPlayService _vstMovieClassifyPlayService;
	 ...
	 * @return
	 */
	@RequestMapping("/findMovieClassifyPlay.action")
	public String findMovieClassifyPlay(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_movieClassifyPlay");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			columns.put("vst_mcp_date", "日期");
			columns.put("vst_mcp_pkg", "包名");
			columns.put("vst_mcp_special_type", "专区类型");
			columns.put("vst_mcp_cid", "影片分类");
			columns.put("vst_mcp_vv", "播放量");
			columns.put("vst_mcp_vv_dod", "播放量天环比");
			columns.put("vst_mcp_vv_wow", "播放量周环比");
			columns.put("vst_mcp_uv", "独立用户");
			columns.put("vst_mcp_uv_dod", "独立用户天环比");
			columns.put("vst_mcp_uv_wow", "独立用户周环比");
			columns.put("vst_mcp_avg", "人均播放量");
			columns.put("vst_mcp_playtime", "播放时长(H)");
			columns.put("vst_mcp_playtime_avg", "人均播放时长(m)");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc("vst_movie_classify_play"));
		} catch (Exception e) {
			logger.error("findMovieClassifyPlay error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_movieClassifyPlay");
		}
		
		return "movie/movieClassifyPlay_list";
	}
	...
	public String exportData(){
		try{
			...
			queryParam.put("vst_mcp_special_type", ToolsString.checkEmpty(request.getParameter("vst_mcp_special_type")));
			queryParam.put("vst_mcp_cid", ToolsString.checkEmpty(request.getParameter("vst_mcp_cid")));
			queryParam.put("export_column", ToolsString.checkEmpty(request.getParameter("export_column")));
			// 编码转码
			VstTools.decodeToUTF_8(queryParam);
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _vstMovieClassifyPlayService.getExportData(queryParam, getUserSession(moduleId));
	...
	public String exportSumData(){
		try{
			...
			queryParam.put("startDay", ToolsString.checkEmpty(request.getParameter("startDay")));
			queryParam.put("endDay", ToolsString.checkEmpty(request.getParameter("endDay")));
			queryParam.put("dataType", ToolsString.checkEmpty(request.getParameter("dataType")));
			
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _vstMovieClassifyPlayService.getExportSumData(queryParam, getUserSession(moduleId));
			List<Map<String, Object>> data = bean.getMapData();
			File file = ExportUtil.ExportExcel(data, PropertiesReader.getProperty("export_dir"), bean.getTitle());
			if(file.exists()){
				VstTools.fileDownLoad(response, file);
	...
	public String ajaxGetReportByCid(){
		try{
			...
			// 按分类统计
			JSONObject data = _vstMovieClassifyPlayService.getReportByCid(queryParam);
