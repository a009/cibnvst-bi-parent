...
import com.vst.defend.service.user.VstUserRetainService;

/**
 * 留存用户
 * @author lhp
 * @date 2018-1-3 下午09:26:46
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/userRetain")
public class VstUserRetainController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstUserRetainController.class.getName();
	 ...
	 */
	@Resource
	private VstUserRetainService _vstUserRetainService;
	...
	public String findUserRetain(){
		try {
			...
			columns.put("vst_ur_date", "日期");
			columns.put("vst_ur_pkg", "包名");
			columns.put("vst_ur_amount", "次日留存用户");
			columns.put("vst_ur_dod", "次日留存天环比");
			columns.put("vst_ur_wow", "次日留存周环比");
			columns.put("vst_ur_rate", "次日留存率");
			columns.put("vst_ur_week_amount", "周留存用户");
			columns.put("vst_ur_week_dod", "周留存天环比");
			columns.put("vst_ur_week_wow", "周留存周环比");
			columns.put("vst_ur_week_rate", "周留存率");
			columns.put("vst_ur_month_amount", "月留存用户");
			columns.put("vst_ur_month_dod", "月留存天环比");
			columns.put("vst_ur_month_wow", "月留存周环比");
			columns.put("vst_ur_month_rate", "月留存率");
			columns.put("vst_ur_season_amount", "季留存用户");
			columns.put("vst_ur_season_dod", "季留存天环比");
			columns.put("vst_ur_season_wow", "季留存周环比");
			columns.put("vst_ur_season_rate", "季留存率");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc("vst_user_retain"));
		} catch (Exception e) {
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _vstUserRetainService.getExportData(queryParam, getUserSession(moduleId));
