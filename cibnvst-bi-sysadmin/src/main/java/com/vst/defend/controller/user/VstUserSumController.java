...
import com.vst.defend.service.user.VstUserSumService;

/**
 * 累计用户
 * @author lhp
 * @date 2018-1-3 上午10:03:31
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/userSum")
public class VstUserSumController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstUserSumController.class.getName();
	 ...
	 */
	@Resource
	private VstUserSumService _vstUserSumService;
	 ...
	 * @return
	 */
	@RequestMapping("/findUserSum.action")
	public String findUserSum(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_userSum");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			columns.put("vst_us_amount", "累计用户数");
			columns.put("vst_us_dod", "天环比");
			columns.put("vst_us_wow", "周环比");
			columns.put("vst_us_mom", "月环比");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc("vst_user_sum"));
		} catch (Exception e) {
			logger.error("findUserSum error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_userSum");
		}
		
		return "user/userSum_list";
	}
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _vstUserSumService.getExportData(queryParam, getUserSession(moduleId));
