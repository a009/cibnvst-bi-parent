 ...
 */
@Controller
@RequestMapping("/oldUserTotal")
public class VstOldUserTotalController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstOldUserTotalController.class.getName();
	 ...
	 * @return
	 */
	@RequestMapping("/findUserTotal.action")
	public String findUserTotal(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_userTotal");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			logger.error("findUserTotal error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_userTotal");
		}
		
		return "old/userTotal_list";
	}
	...
	public String exportData(){
		try{
			...
			File file = ExportUtil.ExportExcel(dataList, PropertiesReader.getProperty("export_dir"), "vst_user_total_data");
			if(file.exists()){
				String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
				UserSession user = getUserSession(moduleId);
				// 写操作日志
				StringBuilder sb = new StringBuilder();
				sb.append("导出表名：vst_user_total_data")
