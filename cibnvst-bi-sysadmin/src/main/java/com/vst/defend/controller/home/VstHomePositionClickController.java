...
import com.vst.defend.service.home.VstHomePositionClickService;
import com.vst.defend.service.task.VstExportTaskService;

/**
 * 首页位置点击
 * @author lhp
 * @date 2018-4-12 下午04:29:01
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/homePositionClick")
public class VstHomePositionClickController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstHomePositionClickController.class.getName();
	
	/**
	 * 表名
	 */
	private static final String _tableName = "vst_home_position_click";
	 ...
	 */
	@Resource
	private VstHomePositionClickService _vstHomePositionClickService;
	 ...
	 * @return
	 */
	@RequestMapping("/findHomePositionClick.action")
	public String findHomePositionClick(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_homePositionClick");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			params.put("vst_table_name", "vst_home_position_click");
			params.put("vst_column_name", "vst_hpc_type");
			...
			columns.put("vst_hpc_date", "日期");
			columns.put("vst_hpc_pkg", "包名");
			columns.put("vst_hpc_type", "类型");
			columns.put("vst_hpc_pos", "位置编号");
			columns.put("vst_hpc_vv", "点击量");
			columns.put("vst_hpc_vv_dod", "点击量天环比");
			columns.put("vst_hpc_vv_wow", "点击量周环比");
			columns.put("vst_hpc_uv", "独立用户");
			columns.put("vst_hpc_uv_dod", "独立用户天环比");
			columns.put("vst_hpc_uv_wow", "独立用户周环比");
			columns.put("vst_hpc_avg", "人均点击量");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc(_tableName));
		} catch (Exception e) {
			logger.error("findHomePositionClick error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_homePositionClick");
		}
		
		return "home/homePositionClick_list";
	}
...
//			queryParam.put("vst_hpc_type", ToolsString.checkEmpty(request.getParameter("vst_hpc_type")));
//			queryParam.put("vst_hpc_pos", ToolsString.checkEmpty(request.getParameter("vst_hpc_pos")));
//			queryParam.put("export_column", ToolsString.checkEmpty(request.getParameter("export_column")));
//			// 编码转码
//			VstTools.decodeToUTF_8(queryParam);
//			// 获取模块ID
//			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
//			ReportBean bean = _vstHomePositionClickService.getExportData(queryParam, getUserSession(moduleId));
...
//	}
	...
	public String exportData(){
		...
		try{
			...
			final String vst_hpc_type = ToolsString.checkEmpty(request.getParameter("vst_hpc_type"));
			final String vst_hpc_pos = ToolsString.checkEmpty(request.getParameter("vst_hpc_pos"));
				...
				Runnable runnable = new Runnable() {
					...
					public void run() {
						...
						try{
							...
							queryParam.put("vst_hpc_type", vst_hpc_type);
							queryParam.put("vst_hpc_pos", vst_hpc_pos);
							// 1、插入导出任务表
