...
import com.vst.defend.service.user.VstUserSumChannelService;

/**
 * 累计用户(渠道)
 * @author lhp
 * @date 2018-1-4 下午03:50:09
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/userSumChannel")
public class VstUserSumChannelController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstUserSumChannelController.class.getName();
	 ...
	 */
	@Resource
	private VstUserSumChannelService _vstUserSumChannelService;
	 ...
	 * @return
	 */
	@RequestMapping("/findUserSumChannel.action")
	public String findUserSumChannel(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_userSumChannel");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			columns.put("vst_usc_date", "日期");
			columns.put("vst_usc_pkg", "包名");
			columns.put("vst_usc_channel", "渠道");
			columns.put("vst_usc_amount", "累计用户数");
			columns.put("vst_usc_dod", "天环比");
			columns.put("vst_usc_wow", "周环比");
			columns.put("vst_usc_mom", "月环比");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc("vst_user_sum_channel"));
		} catch (Exception e) {
			logger.error("findUserSumChannel error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_userSumChannel");
		}
		
		return "user/userSumChannel_list";
	}
	...
	public String exportData(){
		try{
			...
			queryParam.put("vst_usc_channel", ToolsString.checkEmpty(request.getParameter("vst_usc_channel")));
			queryParam.put("export_column", ToolsString.checkEmpty(request.getParameter("export_column")));
			// 编码转码
			VstTools.decodeToUTF_8(queryParam);
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _vstUserSumChannelService.getExportData(queryParam, getUserSession(moduleId));
	...
	public String ajaxGetReportByChannel(){
		try{
			...
			queryParam.put("pkgName", ToolsString.checkEmpty(request.getParameter("pkgName")));
			queryParam.put("vst_usc_channel", ToolsString.checkEmpty(request.getParameter("vst_usc_channel")));
			queryParam.put("vst_usc_channel_eq", ToolsString.checkEmpty(request.getParameter("vst_usc_channel_eq")));
			queryParam.put("vst_usc_channel_ne", ToolsString.checkEmpty(request.getParameter("vst_usc_channel_ne")));
			
			JSONObject data = _vstUserSumChannelService.getReportByChannel(queryParam);
