 ...
 * 页面转化
 * @author lhp
 * @date 2017-11-23 下午02:42:47
 * @version
 */
@Controller
@RequestMapping("/oldPageConvert")
public class VstOldPageConvertController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstOldPageConvertController.class.getName();
	 ...
	 * @return
	 */
	@RequestMapping("/findPageConvert.action")
	public String findPageConvert(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_pageConvert");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			logger.error("findPageConvert error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_pageConvert");
		}
		
		return "old/pageConvert_list";
	}
	...
	public String exportData(){
		try{
			...
			File file = ExportUtil.ExportExcel(dataList, PropertiesReader.getProperty("export_dir"), "vst_page_convert_data");
			if(file.exists()){
				String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
				UserSession user = getUserSession(moduleId);
				// 写操作日志
				StringBuilder sb = new StringBuilder();
				sb.append("导出表名：vst_page_convert_data")
