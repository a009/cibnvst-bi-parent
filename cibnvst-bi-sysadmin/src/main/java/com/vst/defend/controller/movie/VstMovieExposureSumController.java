...
@Scope("session")
@Controller
@RequestMapping("/movieExposureSum")
public class VstMovieExposureSumController extends BaseController {
    /**
     * 类名
     */
    private static final String _className = VstMovieExposureSumController.class.getName();
     ...
     */
    @Resource
    private VstMovieExposureSumService _vstMovieExposureSumService;
     ...
     * @return
     */
    @RequestMapping("/findMovieExposureSum.action")
    public String findMovieExposureSum(){
        try {
            // 初始化
            this.initializeAction(_className);

            Object obj = session.getAttribute("cutPage_movieExposureSum");
            if(obj != null){
                cutPage = (CutPage) obj;
            }else{
                cutPage = new CutPage();
            ...
            columns.put("vst_mes_date", "日期");
            columns.put("vst_mes_pkg", "包名");
            columns.put("vst_mes_special_type", "专区类型");
            columns.put("vst_mes_cid", "影片分类");
            columns.put("vst_mes_amount", "曝光量");
            columns.put("vst_mes_amount_dod", "曝光量天环比");
            columns.put("vst_mes_amount_wow", "曝光量周环比");
            columns.put("vst_mes_uv", "独立用户");
            columns.put("vst_mes_uv_dod", "独立用户天环比");
            columns.put("vst_mes_uv_wow", "独立用户周环比");
            columns.put("vst_mes_avg", "人均曝光量");
            request.setAttribute("columns", columns);
            // 获取表描述
            request.setAttribute("tableDesc", _baseService.getTableDesc("vst_movie_exposure_sum"));
        } catch (Exception e) {
            logger.error("findMovieExposureSum error." + SystemException.getExceptionInfo(e));
            // 转向失败页面
            return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
                    .getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
        } finally {
            // 记得移除session中的分页信息
            session.removeAttribute("cutPage_movieExposureSum");
        }

        return "movie/movieExposureSum_list";
    }
    ...
    public String exportData(){
        try{
            ...
            queryParam.put("vst_mes_special_type", ToolsString.checkEmpty(request.getParameter("vst_mes_special_type")));
            queryParam.put("vst_mes_cid", ToolsString.checkEmpty(request.getParameter("vst_mes_cid")));
            queryParam.put("export_column", ToolsString.checkEmpty(request.getParameter("export_column")));
            // 编码转码
            VstTools.decodeToUTF_8(queryParam);
            // 获取模块ID
            String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
            ReportBean bean = _vstMovieExposureSumService.getExportData(queryParam, getUserSession(moduleId));
