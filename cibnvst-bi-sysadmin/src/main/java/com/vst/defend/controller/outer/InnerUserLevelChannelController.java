 ...
 * 季质量渠道用户(对内)
 * @author lhp
 * @date 2018-1-24 上午10:59:45
 * @version
 */
@Scope("prototype")
@Controller
@RequestMapping("/innerUserLevelChannel")
public class InnerUserLevelChannelController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = InnerUserLevelChannelController.class.getName();
	 ...
	 */
	@Resource
	private InnerUserLevelChannelService _innerUserLevelChannelService;
	 ...
	 * @return
	 */
	@RequestMapping("/findInnerUserLevelChannel.action")
	public String findInnerUserLevelChannel(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_innerUserLevelChannel");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc("outer_vst_user_level_channel"));
		} catch (Exception e) {
			logger.error("findInnerUserLevelChannel error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_innerUserLevelChannel");
		}
		
		return "outer/innerUserLevelChannel_list";
	}
	...
	public String ajaxGetCutPage(){
		try{
			...
			queryParam.put("vst_ulc_state", ToolsString.checkEmpty(request.getParameter("vst_ulc_state")));
			queryParam.put("vst_ulc_updator", ToolsString.checkEmpty(request.getParameter("vst_ulc_updator")));
			// 排序信息
			...
			ReportBean bean = _innerUserLevelChannelService.getCutPageData(cutPage);
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _innerUserLevelChannelService.getExportData(queryParam, getUserSession(moduleId));
	...
	public String ajaxGetReportByDate(){
		try{
			...
			ReportBean result = _innerUserLevelChannelService.getReportByDate(queryParam);
	...
	public String ajaxAuditData(){
		...
		try{
			...
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			int result = _innerUserLevelChannelService.auditData(recordId, state, getUserSession(moduleId));
	...
	public String ajaxBatchAuditData(){
		...
		try {
			...
			if(ToolsString.isEmpty(startDay) || ToolsString.isEmpty(endDay)){
				// 转向失败页面
				return this.moveToPage(VstConstants.OPERATE_INFO, "必选选择日期才能审核！", findInnerUserLevelChannel());
			}
			
			Map<String, Object> params = new HashMap<String, Object>();
			params.put("startDay", startDay);
			params.put("endDay", endDay);
			params.put("vst_ulc_channel", ToolsString.checkEmpty(request.getParameter("vst_ulc_channel")));
			params.put("vst_ulc_state", ToolsString.checkEmpty(request.getParameter("vst_ulc_state")));
			params.put("vst_ulc_updator", ToolsString.checkEmpty(request.getParameter("vst_ulc_updator")));
			
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			int result = _innerUserLevelChannelService.batchAuditData(params, getUserSession(moduleId));
			if(result > 0){
				json.put("code", 100);
	...
	public String ajaxModifyModulus(){
		...
		try {
			...
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			_innerUserLevelChannelService.modifyModulus(ids, modulus, getUserSession(moduleId));
			out.print(true);
			out.flush();
		} catch (Exception e) {
	...
	public String ajaxBatchUpdate(){
		...
		try {
			...
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			int result = _innerUserLevelChannelService.modifyModulus(ids, modulus, getUserSession(moduleId));
			if(result > 0){
				json.put("code", 100);
			}else{
				json.put("code", 200);
	...
	public String ajaxImportData(){
		...
		try {
			...
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			int result = _innerUserLevelChannelService.importData(date, getUserSession(moduleId));
			if(result > 0){
				json.put("code", 100);
				json.put("result", result);
			}else{
	...
	public String ajaxDeleteData(){
		...
		try {
			...
			if(ToolsString.isEmpty(startDay) || ToolsString.isEmpty(endDay)){
				// 转向失败页面
				return this.moveToPage(VstConstants.OPERATE_INFO, "必选选择日期才能删除！", findInnerUserLevelChannel());
			}
			...
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			int result = _innerUserLevelChannelService.deleteData(params, getUserSession(moduleId));
			if(result > 0){
				json.put("code", 100);
				json.put("result", result);
			}else{
	...
	public String ajaxSyncData(){
		...
		try {
			...
			String url = "http://bi.cibnvst.com:8081/cibnvst-bi-quartz/push/user/level/channel?startDate={startDate}&endDate={endDate}"
	...
	public String ajaxEditData(){
		...
		try {
			...
			Map<String, Object> params = new HashMap<String, Object>();
			params.put("vst_ulc_id", recordId);
			params.put("vst_ulc_add_amount", ToolsString.checkEmpty(request.getParameter("addAmount")));
			params.put("vst_ulc_season_amount", ToolsString.checkEmpty(request.getParameter("seasonAmount")));
			params.put("vst_ulc_modulus", ToolsString.checkEmpty(request.getParameter("modulus")));

			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			int result = _innerUserLevelChannelService.updateData(params, getUserSession(moduleId));
			if(result > 0){
				json.put("code", 100);
			}else{
				json.put("code", 200);
	...
	public String ajaxGetReportByDim(){
		try{
			...
			ReportBean result = _innerUserLevelChannelService.getReportByDim(queryParam);
			List<Map<String, Object>> data = result.getMapData();
