...
@Scope("session")
@Controller
@RequestMapping("/userSecondaryActiveChannel")
public class VstUserSecondaryActiveChannelController extends BaseController {
    /**
     * 类名
     */
    private static final String _className = VstUserSecondaryActiveChannelController.class.getName();
     ...
     */
    @Resource
    private VstUserSecondaryActiveChannelService _vstUserSecondaryActiveChannelService;
     ...
     * @return
     */
    @RequestMapping("/findUserSecondaryActiveChannel.action")
    public String findUserSecondaryActiveChannel(){
        try {
            // 初始化
            this.initializeAction(_className);

            Object obj = session.getAttribute("cutPage_userSecondaryActiveChannel");
            if(obj != null){
                cutPage = (CutPage) obj;
            }else{
                cutPage = new CutPage();
            ...
            columns.put("vst_usac_date", "日期");
            columns.put("vst_usac_pkg", "包名");
            columns.put("vst_usac_channel", "渠道");
            columns.put("vst_usac_amount", "二次活跃用户数");
            columns.put("vst_usac_dod", "天环比");
            columns.put("vst_usac_wow", "周环比");
            columns.put("vst_usac_mom", "月环比");
            request.setAttribute("columns", columns);
            // 获取表描述
            request.setAttribute("tableDesc", _baseService.getTableDesc("vst_user_secondary_active_channel"));
        } catch (Exception e) {
            logger.error("findUserSecondaryActiveChannel error." + SystemException.getExceptionInfo(e));
            // 转向失败页面
            return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
                    .getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
        } finally {
            // 记得移除session中的分页信息
            session.removeAttribute("cutPage_userSecondaryActiveChannel");
        }

        return "user/userSecondaryActiveChannel_list";
    }
    ...
    public String exportData(){
        try{
            ...
            // 获取模块ID
            String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
            ReportBean bean = _vstUserSecondaryActiveChannelService.getExportData(queryParam, getUserSession(moduleId));
    ...
    public String ajaxGetReportByChannel(){
        try{
            ...
            queryParam.put("pkgName", ToolsString.checkEmpty(request.getParameter("pkgName")));
            queryParam.put("vst_usac_channel", ToolsString.checkEmpty(request.getParameter("vst_usac_channel")));
            queryParam.put("vst_usac_channel_eq", ToolsString.checkEmpty(request.getParameter("vst_usac_channel_eq")));
            queryParam.put("vst_usac_channel_ne", ToolsString.checkEmpty(request.getParameter("vst_usac_channel_ne")));

            JSONObject data = _vstUserSecondaryActiveChannelService.getReportByChannel(queryParam);
