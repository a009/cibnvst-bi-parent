 ...
 * 质量用户(新)
 * @author lhp
 * @date 2017-11-21 下午04:48:42
 * @version
 */
@Controller
@RequestMapping("/oldUserQualityNew")
public class VstOldUserQualityNewController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstOldUserQualityNewController.class.getName();
	 ...
	 * @return
	 */
	@RequestMapping("/findUserQualityNew.action")
	public String findUserQualityNew(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_userQualityNew");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			logger.error("findUserQualityNew error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_userQualityNew");
		}
		
		return "old/userQualityNew_list";
	}
	...
	public String exportData(){
		try{
			...
			File file = ExportUtil.ExportExcel(dataList, PropertiesReader.getProperty("export_dir"), "vst_user_quality_new_data");
			if(file.exists()){
				String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
				UserSession user = getUserSession(moduleId);
				// 写操作日志
				StringBuilder sb = new StringBuilder();
				sb.append("导出表名：vst_user_quality_new_data")
