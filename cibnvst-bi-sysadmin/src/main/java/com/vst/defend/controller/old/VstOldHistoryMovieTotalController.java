 ...
 * 历时影片汇总
 * @author lhp
 * @date 2017-11-23 下午03:45:27
 * @version
 */
@Controller
@RequestMapping("/oldHistoryMovieTotal")
public class VstOldHistoryMovieTotalController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstOldHistoryMovieTotalController.class.getName();
	 ...
	 * @return
	 */
	@RequestMapping("/findHistoryMovieTotal.action")
	public String findHistoryMovieTotal(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_historyMovieTotal");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			logger.error("findHistoryMovieTotal error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_historyMovieTotal");
		}
		
		return "old/historyMovieTotal_list";
	}
	...
	public String exportData(){
		try{
			...
			File file = ExportUtil.ExportExcel(dataList, PropertiesReader.getProperty("export_dir"), "vst_history_movie_total_data");
			if(file.exists()){
				String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
				UserSession user = getUserSession(moduleId);
				// 写操作日志
				StringBuilder sb = new StringBuilder();
				sb.append("导出表名：vst_history_movie_total_data")
