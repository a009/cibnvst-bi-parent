...
import com.vst.defend.service.user.VstUserSumBdmodelService;

/**
 * 累计用户(设备)
 * @author lhp
 * @date 2019-3-6 上午11:28:19
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/userSumBdmodel")
public class VstUserSumBdmodelController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstUserSumBdmodelController.class.getName();
	 ...
	 */
	@Resource
	private VstUserSumBdmodelService _vstUserSumBdmodelService;
	 ...
	 * @return
	 */
	@RequestMapping("/findUserSumBdmodel.action")
	public String findUserSumBdmodel(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_userSumBdmodel");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			columns.put("vst_usb_date", "日期");
			columns.put("vst_usb_pkg", "包名");
			columns.put("vst_usb_bdmodel", "设备名称");
			columns.put("vst_usb_amount", "用户数量");
			columns.put("vst_usb_rate", "用户占比");
			columns.put("vst_usb_dod", "天环比");
			columns.put("vst_usb_wow", "周环比");
			columns.put("vst_usb_mom", "月环比");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc("vst_user_sum_bdmodel"));
		} catch (Exception e) {
			logger.error("findUserSumBdmodel error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_userSumBdmodel");
		}
		
		return "user/userSumBdmodel_list";
	}
	...
	public String exportData(){
		try{
			...
			queryParam.put("vst_usb_bdmodel", ToolsString.checkEmpty(request.getParameter("vst_usb_bdmodel")));
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _vstUserSumBdmodelService.getExportData(queryParam, getUserSession(moduleId));
