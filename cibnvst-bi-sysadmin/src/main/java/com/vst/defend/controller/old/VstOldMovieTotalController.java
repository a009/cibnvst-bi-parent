 ...
 * 影片汇总
 * @author lhp
 * @date 2017-11-23 下午03:18:54
 * @version
 */
@Controller
@RequestMapping("/oldMovieTotal")
public class VstOldMovieTotalController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstOldMovieTotalController.class.getName();
	 ...
	 * @return
	 */
	@RequestMapping("/findMovieTotal.action")
	public String findMovieTotal(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_movieTotal");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			logger.error("findMovieTotal error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_movieTotal");
		}
		
		return "old/movieTotal_list";
	}
	...
	public String exportData(){
		try{
			...
			File file = ExportUtil.ExportExcel(dataList, PropertiesReader.getProperty("export_dir"), "vst_movie_total_data");
			if(file.exists()){
				String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
				UserSession user = getUserSession(moduleId);
				// 写操作日志
				StringBuilder sb = new StringBuilder();
				sb.append("导出表名：vst_movie_total_data")
