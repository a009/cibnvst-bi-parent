 ...
 * 活跃用户
 * @author lhp
 * @date 2017-11-10 下午02:15:01
 * @version
 */
@Controller
@RequestMapping("/oldUserActive")
public class VstOldUserActiveController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstOldUserActiveController.class.getName();
	...
	public String exportData(){
		try{
			...
			File file = ExportUtil.ExportExcel(dataList, PropertiesReader.getProperty("export_dir"), "vst_user_active_data");
			if(file.exists()){
				String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
				UserSession user = getUserSession(moduleId);
				// 写操作日志
				StringBuilder sb = new StringBuilder();
				sb.append("导出表名：vst_user_active_data")
