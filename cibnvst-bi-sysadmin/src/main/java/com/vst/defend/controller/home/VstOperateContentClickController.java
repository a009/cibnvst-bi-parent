...
import com.vst.defend.service.home.VstOperateContentClickService;
import com.vst.defend.service.task.VstExportTaskService;

/**
 * 运营页内容点击
 * @author lhp
 * @date 2018-4-10 下午05:56:16
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/operateContentClick")
public class VstOperateContentClickController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstOperateContentClickController.class.getName();
	
	/**
	 * 表名
	 */
	private static final String _tableName = "vst_operate_content_click";
	 ...
	 */
	@Resource
	private VstOperateContentClickService _vstOperateContentClickService;
	 ...
	 * @return
	 */
	@RequestMapping("/findOperateContentClick.action")
	public String findOperateContentClick(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_operateContentClick");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			params.put("vst_table_name", "vst_operate_content_click");
			params.put("vst_column_name", "vst_occ_type");
			params.put("vst_state", VstConstants.STATE_AVALIABLE);
			request.setAttribute("types", _baseService.getDictionaryForMap(params));
			// 获取影片分类
			params.put("vst_table_name", "vst_operate_content_click");
			params.put("vst_column_name", "vst_occ_cid");
			request.setAttribute("cids", _baseService.getDictionaryForMap(params));
			
			// 获取表字段
			Map<String, String> columns = new LinkedHashMap<String, String>();
			columns.put("vst_occ_date", "日期");
			columns.put("vst_occ_pkg", "包名");
			columns.put("vst_occ_type", "类型");
			columns.put("vst_occ_cid", "影片分类");
			columns.put("vst_occ_uuid", "影片ID");
			columns.put("vst_occ_name", "名称");
			columns.put("vst_occ_vv", "点击量");
			columns.put("vst_occ_vv_dod", "点击量天环比");
			columns.put("vst_occ_vv_wow", "点击量周环比");
			columns.put("vst_occ_uv", "独立用户");
			columns.put("vst_occ_uv_dod", "独立用户天环比");
			columns.put("vst_occ_uv_wow", "独立用户周环比");
			columns.put("vst_occ_avg", "人均点击量");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc(_tableName));
		} catch (Exception e) {
			logger.error("findOperateContentClick error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_operateContentClick");
		}
		
		return "home/operateContentClick_list";
	}
...
//			queryParam.put("vst_occ_type", ToolsString.checkEmpty(request.getParameter("vst_occ_type")));
//			queryParam.put("vst_occ_cid", ToolsString.checkEmpty(request.getParameter("vst_occ_cid")));
//			queryParam.put("vst_occ_uuid", ToolsString.checkEmpty(request.getParameter("vst_occ_uuid")));
//			queryParam.put("export_column", ToolsString.checkEmpty(request.getParameter("export_column")));
//			// 参数&符还原
//			String vst_occ_name = ToolsString.checkEmpty(request.getParameter("vst_occ_name"));
//			vst_occ_name = vst_occ_name.replace("<@_@>", "&");
//			queryParam.put("vst_occ_name", vst_occ_name);
//			// 编码转码
//			VstTools.decodeToUTF_8(queryParam);
//			// 获取模块ID
//			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
//			ReportBean bean = _vstOperateContentClickService.getExportData(queryParam, getUserSession(moduleId));
...
//	}
	...
	public String exportData(){
		...
		try{
			...
			final String vst_occ_type = ToolsString.checkEmpty(request.getParameter("vst_occ_type"));
			final String vst_occ_cid = ToolsString.checkEmpty(request.getParameter("vst_occ_cid"));
			final String vst_occ_uuid = ToolsString.checkEmpty(request.getParameter("vst_occ_uuid"));
			final String vst_occ_name = URLDecoder.decode(ToolsString.checkEmpty(request.getParameter("vst_occ_name")), "UTF-8");
				...
				Runnable runnable = new Runnable() {
					...
					public void run() {
						...
						try{
							...
							queryParam.put("vst_occ_type", vst_occ_type);
							queryParam.put("vst_occ_cid", vst_occ_cid);
							queryParam.put("vst_occ_uuid", vst_occ_uuid);
							// 参数&符还原
							String new_occ_name = vst_occ_name.replace("<@_@>", "&");
							queryParam.put("vst_occ_name", new_occ_name);
							// 1、插入导出任务表
