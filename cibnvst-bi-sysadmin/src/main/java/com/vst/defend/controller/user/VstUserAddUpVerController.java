...
import com.vst.defend.service.user.VstUserAddUpVerService;

/**
 * 新增用户(升级版本)
 * @author lhp
 * @date 2018-1-3 下午07:25:46
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/userAddUpVer")
public class VstUserAddUpVerController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstUserAddUpVerController.class.getName();
	 ...
	 */
	@Resource
	private VstUserAddUpVerService _vstUserAddUpVerService;
	 ...
	 * @return
	 */
	@RequestMapping("/findUserAddUpVer.action")
	public String findUserAddUpVer(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_userAddUpVer");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			columns.put("vst_uau_amount", "新增用户数");
			columns.put("vst_uau_dod", "天环比");
			columns.put("vst_uau_wow", "周环比");
			columns.put("vst_uau_mom", "月环比");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc("vst_user_add_upVer"));
		} catch (Exception e) {
			logger.error("findUserAddUpVer error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_userAddUpVer");
		}
		
		return "user/userAddUpVer_list";
	}
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _vstUserAddUpVerService.getExportData(queryParam, getUserSession(moduleId));
