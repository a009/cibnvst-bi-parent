...
import com.vst.defend.service.user.VstUserSumVersionService;

/**
 * 累计用户(版本)
 * @author lhp
 * @date 2018-1-4 下午03:58:34
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/userSumVersion")
public class VstUserSumVersionController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstUserSumVersionController.class.getName();
	 ...
	 */
	@Resource
	private VstUserSumVersionService _vstUserSumVersionService;
	 ...
	 * @return
	 */
	@RequestMapping("/findUserSumVersion.action")
	public String findUserSumVersion(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_userSumVersion");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			columns.put("vst_usv_date", "日期");
			columns.put("vst_usv_pkg", "包名");
			columns.put("vst_usv_ver", "显示版本");
			columns.put("vst_usv_amount", "累计用户数");
			columns.put("vst_usv_dod", "天环比");
			columns.put("vst_usv_wow", "周环比");
			columns.put("vst_usv_mom", "月环比");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc("vst_user_sum_version"));
		} catch (Exception e) {
			logger.error("findUserSumVersion error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_userSumVersion");
		}
		
		return "user/userSumVersion_list";
	}
	...
	public String exportData(){
		try{
			...
			queryParam.put("vst_usv_ver", ToolsString.checkEmpty(request.getParameter("vst_usv_ver")));
			queryParam.put("export_column", ToolsString.checkEmpty(request.getParameter("export_column")));
			// 编码转码
			VstTools.decodeToUTF_8(queryParam);
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _vstUserSumVersionService.getExportData(queryParam, getUserSession(moduleId));
