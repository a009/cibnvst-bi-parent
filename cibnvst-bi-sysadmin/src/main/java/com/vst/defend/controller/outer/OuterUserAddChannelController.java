 ...
 * 新增渠道用户(对外)
 * @author lhp
 * @date 2018-1-16 下午05:54:59
 * @version
 */
@Scope("prototype")
@Controller
@RequestMapping("/outerUserAddChannel")
public class OuterUserAddChannelController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = OuterUserAddChannelController.class.getName();
	 ...
	 */
	@Resource
	private OuterUserAddChannelService _outerUserAddChannelService;
	 ...
	 * @return
	 */
	@RequestMapping("/findOuterUserAddChannel.action")
	public String findOuterUserAddChannel(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_outerUserAddChannel");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			logger.error("findOuterUserAddChannel error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_outerUserAddChannel");
		}
		
		return "outer/outerUserAddChannel_list";
	}
	...
	public String ajaxGetCutPage(){
		try{
			...
			queryParam.put("vst_uac_channel", vst_sys_channel);
			// 排序信息
			...
			ReportBean bean = _outerUserAddChannelService.getCutPageData(cutPage);
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _outerUserAddChannelService.getExportData(queryParam, getUserSession(moduleId));
	...
	public String ajaxGetReportByDate(){
		try{
			...
			ReportBean result = _outerUserAddChannelService.getReportByDate(queryParam);
	...
	public String ajaxGetReportByChannel(){
		try{
			...
			ReportBean result = _outerUserAddChannelService.getReportByChannel(queryParam);
			List<Map<String, Object>> data = result.getMapData();
