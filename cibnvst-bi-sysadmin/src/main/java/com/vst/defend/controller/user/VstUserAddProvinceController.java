...
import com.vst.defend.service.user.VstUserAddProvinceService;

/**
 * 新增用户(省份)
 * @author lhp
 * @date 2018-1-3 下午05:06:34
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/userAddProvince")
public class VstUserAddProvinceController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstUserAddProvinceController.class.getName();
	 ...
	 */
	@Resource
	private VstUserAddProvinceService _vstUserAddProvinceService;
	 ...
	 * @return
	 */
	@RequestMapping("/findUserAddProvince.action")
	public String findUserAddProvince(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_userAddProvince");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			columns.put("vst_uap_amount", "新增用户数");
			columns.put("vst_uap_dod", "天环比");
			columns.put("vst_uap_wow", "周环比");
			columns.put("vst_uap_mom", "月环比");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc("vst_user_add_province"));
		} catch (Exception e) {
			logger.error("findUserAddProvince error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_userAddProvince");
		}
		
		return "user/userAddProvince_list";
	}
	...
	public String exportData(){
		try{
			...
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _vstUserAddProvinceService.getExportData(queryParam, getUserSession(moduleId));
