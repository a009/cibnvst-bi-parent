...
import com.vst.defend.service.user.VstUserSumUpVerService;

/**
 * 累计用户(升级版本)
 * @author lhp
 * @date 2018-1-4 下午03:57:01
 * @version
 */
@Scope("session")
@Controller
@RequestMapping("/userSumUpVer")
public class VstUserSumUpVerController extends BaseController {
	/**
	 * 类名
	 */
	private static final String _className = VstUserSumUpVerController.class.getName();
	 ...
	 */
	@Resource
	private VstUserSumUpVerService _vstUserSumUpVerService;
	 ...
	 * @return
	 */
	@RequestMapping("/findUserSumUpVer.action")
	public String findUserSumUpVer(){
		try {
			// 初始化
			this.initializeAction(_className);
			
			Object obj = session.getAttribute("cutPage_userSumUpVer");
			if(obj != null){
				cutPage = (CutPage) obj;
			}else{
				cutPage = new CutPage();
			...
			columns.put("vst_usu_date", "日期");
			columns.put("vst_usu_pkg", "包名");
			columns.put("vst_usu_verNum", "升级版本");
			columns.put("vst_usu_amount", "累计用户数");
			columns.put("vst_usu_dod", "天环比");
			columns.put("vst_usu_wow", "周环比");
			columns.put("vst_usu_mom", "月环比");
			request.setAttribute("columns", columns);
			// 获取表描述
			request.setAttribute("tableDesc", _baseService.getTableDesc("vst_user_sum_upVer"));
		} catch (Exception e) {
			logger.error("findUserSumUpVer error." + SystemException.getExceptionInfo(e));
			// 转向失败页面
			return this.moveToError(ErrorCode.SYS_OPERATOR_FAILURE, ErrorInfo
					.getErrorInfo(ErrorCode.SYS_OPERATOR_FAILURE), "sysMain/mainPage.action");
		} finally {
			// 记得移除session中的分页信息
			session.removeAttribute("cutPage_userSumUpVer");
		}
		
		return "user/userSumUpVer_list";
	}
	...
	public String exportData(){
		try{
			...
			queryParam.put("vst_usu_verNum", ToolsString.checkEmpty(request.getParameter("vst_usu_verNum")));
			queryParam.put("export_column", ToolsString.checkEmpty(request.getParameter("export_column")));
			// 编码转码
			VstTools.decodeToUTF_8(queryParam);
			// 获取模块ID
			String moduleId = ToolsString.checkEmpty(request.getParameter("moduleId"));
			ReportBean bean = _vstUserSumUpVerService.getExportData(queryParam, getUserSession(moduleId));
