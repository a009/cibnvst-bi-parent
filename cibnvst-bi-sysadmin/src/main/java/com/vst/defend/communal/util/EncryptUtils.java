...
public class EncryptUtils {
	 ...
	 * @return
	 */
	public static String getMD5Encrypt(File file){
		String value = null;
		try{
			if(file.exists()){
	 ...
	 * @return
	 */
	public static String getMD5Encrypt(FileInputStream in, long size){
		String value = null;
		try{
			if(in != null){
	...
	public static String encryptCookie(String cookie){
		return encryptCookie(cookie.getBytes());
	}
	 ...
	 * @return
	 */
	public static String encryptCookie(byte[] cookie){
		if(cookie == null){
			return null;
		}
		...
		try {
			...
			if(length > 0){
				...
				byte[] addBytes = str.getBytes();
				byte[] newCookie = new byte[cookie.length + addBytes.length];
				System.arraycopy(cookie, 0, newCookie, 0, cookie.length);
				System.arraycopy(addBytes, 0, newCookie, cookie.length, addBytes.length);
				cookie = newCookie;
			}
			// 加密数组
			byte[] encrypt = cipher.doFinal(cookie);
			// 转十六进制
