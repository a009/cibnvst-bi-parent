...
import com.vst.defend.dao2.channel.VstChannelLevelDao;
 ...
 */
@Service
public class OuterChannelLevelServiceImpl implements OuterChannelLevelService {
    /**
     * 写日志
     */
    private static final Log logger = LogFactory.getLog(OuterChannelLevelServiceImpl.class);

    /**
     * 渠道质量Dao接口
     */
    @Resource
    private VstChannelLevelDao _vstChannelLevelDao;
    ...
    public ReportBean getCutPageData(CutPage cutPage) throws SystemException {
        ...
        try{
            ...
            if(params == null){
                ...
                // 渠道
                String vst_cl_channel = params.get("vst_cl_channel")+"";
                if(ToolsString.isEmpty(vst_cl_channel) || "all".equals(vst_cl_channel)){
                    params.remove("vst_cl_channel");
                }else{
                    params.remove("vst_cl_channel");
                    params.put("list_channels", Arrays.asList(vst_cl_channel.split(",")));
                }
            }

            int count = _vstChannelLevelDao.queryCount(params);
            if(count != 0){
                ...
                List<Map<String, Object>> list = _vstChannelLevelDao.queryForList(params);

                for(Map<String, Object> map : list){
                    // 转换日期格式
                    map.put("vst_cl_addtime", ToolsDate.formatDate(ToolsDate.yyyy_MM_dd_HH_mm_ss, map.get("vst_cl_addtime")+""));
                    map.put("vst_cl_uptime", ToolsDate.formatDate(ToolsDate.yyyy_MM_dd_HH_mm_ss, map.get("vst_cl_uptime")+""));
                }
            ...
            logger.error("查询渠道订单(对外)分页数据失败: " + SystemException.getExceptionInfo(e));
    ...
    public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
        ...
        try{
            ...
            List<Map<String, Object>> data = _vstChannelLevelDao.queryExport(params);
            ...
            bean.setTitle("vst_channel_level_data");
            ...
            logger.error("导出渠道订单(对外)数据失败: " + SystemException.getExceptionInfo(e));
            throw new SystemException(ErrorCode.SERVICE_OPERATOR_FAILURE, e);
        }
        return bean;
    }
