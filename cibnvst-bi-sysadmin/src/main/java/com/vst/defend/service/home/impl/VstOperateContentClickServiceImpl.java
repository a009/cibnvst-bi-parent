...
import com.vst.defend.dao2.home.VstOperateContentClickDao;
 ...
 * 
 * @author lhp
 * @date 2018-4-10 下午05:48:47
 * @version
 */
@Service
public class VstOperateContentClickServiceImpl implements VstOperateContentClickService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstOperateContentClickServiceImpl.class);
	
	/**
	 * 运营页内容点击Dao接口
	 */
	@Resource
	private VstOperateContentClickDao _vstOperateContentClickDao;
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			if(params == null){
				...
				// 参数特殊化处理
				if(!ToolsString.isEmpty(params.get("vst_hcc_name")+"")){
					VstTools.paramFormat("vst_hcc_name", params.get("vst_hcc_name")+"", params);
				}
			}
			
			List<Map<String, Object>> data = _vstOperateContentClickDao.queryExport(params);
			if(data != null && data.size() > 0){
				...
				// 获取类型
				temp.put("vst_table_name", "vst_operate_content_click");
				temp.put("vst_column_name", "vst_occ_type");
				Map<String, String> typeMap = _baseService.getDictionaryForMap(temp);
				// 获取影片分类
				temp.put("vst_table_name", "vst_operate_content_click");
				temp.put("vst_column_name", "vst_occ_cid");
				Map<String, String> cidMap = _baseService.getDictionaryForMap(temp);
				...
				for(Map<String, Object> map : data){
					...
					if(cidMap.containsKey(map.get("影片分类")+"")){
						map.put("影片分类", cidMap.get(map.get("影片分类")+""));
					}// 只保留关键字段
			...
			bean.setTitle("vst_operate_content_click_data");
			...
			logger.error("导出运营页内容点击数据失败: " + SystemException.getExceptionInfo(e));
