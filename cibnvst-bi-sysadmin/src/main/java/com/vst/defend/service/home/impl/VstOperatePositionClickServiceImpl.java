...
import com.vst.defend.dao2.home.VstOperatePositionClickDao;
 ...
 * 
 * @author lhp
 * @date 2018-4-12 下午05:35:54
 * @version
 */
@Service
public class VstOperatePositionClickServiceImpl implements VstOperatePositionClickService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstOperatePositionClickServiceImpl.class);
	
	/**
	 * 运营页位置点击Dao接口
	 */
	@Resource
	private VstOperatePositionClickDao _vstOperatePositionClickDao;
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			...
			List<Map<String, Object>> data = _vstOperatePositionClickDao.queryExport(params);
			if(data != null && data.size() > 0){
				...
				// 获取类型
				temp.put("vst_table_name", "vst_operate_position_click");
				temp.put("vst_column_name", "vst_opc_type");
			...
			bean.setTitle("vst_operate_position_click_data");
			...
			logger.error("导出运营页位置点击数据失败: " + SystemException.getExceptionInfo(e));
