...
import com.vst.defend.dao2.user.VstUserSumVersionDao;
 ...
 * 
 * @author lhp
 * @date 2018-1-4 下午03:43:52
 * @version
 */
@Service
public class VstUserSumVersionServiceImpl implements VstUserSumVersionService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstUserSumVersionServiceImpl.class);
	
	/**
	 * 累计用户(版本)Dao接口
	 */
	@Resource
	private VstUserSumVersionDao _vstUserSumVersionDao;
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			if(params == null){
				...
				// 参数特殊化处理
				if(!ToolsString.isEmpty(params.get("vst_usv_ver")+"")){
					VstTools.paramFormat("vst_usv_ver", params.get("vst_usv_ver")+"", params);
				}
			}
			
			List<Map<String, Object>> data = _vstUserSumVersionDao.queryExport(params);
			...
			bean.setTitle("vst_user_sum_version_data");
			...
			logger.error("导出累计用户(版本)数据失败: " + SystemException.getExceptionInfo(e));
