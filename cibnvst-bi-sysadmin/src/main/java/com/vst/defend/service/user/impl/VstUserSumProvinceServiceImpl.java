...
import com.vst.defend.dao2.user.VstUserSumProvinceDao;
 ...
 * 
 * @author lhp
 * @date 2018-1-4 下午03:40:52
 * @version
 */
@Service
public class VstUserSumProvinceServiceImpl implements VstUserSumProvinceService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstUserSumProvinceServiceImpl.class);
	
	/**
	 * 累计用户(省份)Dao接口
	 */
	@Resource
	private VstUserSumProvinceDao _vstUserSumProviceDao;
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			if(params == null){
				...
				// 参数特殊化处理
				if(!ToolsString.isEmpty(params.get("vst_usp_province")+"")){
					VstTools.paramFormat("vst_usp_province", params.get("vst_usp_province")+"", params);
				}
			}
			
			List<Map<String, Object>> data = _vstUserSumProviceDao.queryExport(params);
			...
			bean.setTitle("vst_user_sum_province_data");
			...
			logger.error("导出累计用户(省份)数据失败: " + SystemException.getExceptionInfo(e));
