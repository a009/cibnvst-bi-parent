 ...
 * 
 * @author lhp
 * @date 2017-11-27 下午12:27:08
 * @version
 */
@Service
public class VstMovieClickServiceImpl implements VstMovieClickService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstMovieClickServiceImpl.class);
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			if(params == null){
				...
				// 参数特殊化处理
				if(!ToolsString.isEmpty(params.get("vst_mc_name")+"")){
					VstTools.paramFormat("vst_mc_name", params.get("vst_mc_name")+"", params);
				}
			}
			
			List<Map<String, Object>> data = _vstMovieClickDao.queryExport(params);
			...
			bean.setTitle("vst_movie_click_data");
			...
			logger.error("导出影片点击数据失败: " + SystemException.getExceptionInfo(e));
	...
	public ReportBean getExportSumData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			...
			List<Map<String, Object>> data = _vstMovieClickDao.queryExportSum(params);
			
			bean.setMapData(data);
			bean.setTitle("vst_movie_click_sum_data");
			...
			logger.error("导出影片点击总量失败: " + SystemException.getExceptionInfo(e));
			throw new SystemException(ErrorCode.SERVICE_OPERATOR_FAILURE, e);
		}
		return bean;
	}
