package com.vst.defend.service.channel.impl;
 ...
 * 
 * @author lhp
 * @date 2018-3-9 下午04:03:32
 * @version
 */
@Service
public class VstChannelLevelServiceImpl implements VstChannelLevelService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstChannelLevelServiceImpl.class);
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			if(params == null){
				...
				// 参数特殊化处理
				if(!ToolsString.isEmpty(params.get("vst_cl_channel")+"")){
					VstTools.paramFormat("vst_cl_channel", params.get("vst_cl_channel")+"", params);
				}
			}
			...
			logger.error("导出渠道质量数据失败: " + SystemException.getExceptionInfo(e));
