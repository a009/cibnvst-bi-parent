...
import com.vst.defend.dao3.outer.OuterUserSecondaryActiveChannelDiffDao;
import com.vst.defend.service.outer.OuterUserSecondaryActiveChannelDiffService;
 ...
 */
@Service
public class OuterUserSecondaryActiveChannelDiffServiceImpl implements OuterUserSecondaryActiveChannelDiffService {
    /**
     * 写日志
     */
    private static final Log logger = LogFactory.getLog(OuterUserSecondaryActiveChannelDiffServiceImpl.class);

    /**
     * 二次活跃渠道用户(对外)Dao接口
     */
    @Resource
    private OuterUserSecondaryActiveChannelDiffDao _outerUserSecondaryActiveChannelDiffDao;
    ...
    public ReportBean getCutPageData(CutPage cutPage) throws SystemException {
        ...
        try{
            ...
            if(params == null){
                ...
                // 渠道
                String vst_usacd_channel = params.get("vst_usacd_channel")+"";
                if(ToolsString.isEmpty(vst_usacd_channel) || "all".equals(vst_usacd_channel)){
                    params.remove("vst_usacd_channel");
                }else{
                    params.remove("vst_usacd_channel");
                    params.put("list_channels", Arrays.asList(vst_usacd_channel.split(",")));
                }
            }

            int count = _outerUserSecondaryActiveChannelDiffDao.queryCount(params);
            ...
            if(count != 0){
                ...
                List<Map<String, Object>> list = _outerUserSecondaryActiveChannelDiffDao.queryForList(params);

                for(Map<String, Object> map : list){
                    // 转换日期格式
                    map.put("vst_usacd_addtime", ToolsDate.formatDate(ToolsDate.yyyy_MM_dd_HH_mm_ss, map.get("vst_usacd_addtime")+""));
                    map.put("vst_usacd_uptime", ToolsDate.formatDate(ToolsDate.yyyy_MM_dd_HH_mm_ss, map.get("vst_usacd_uptime")+""));
                }
    ...
    public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
        ...
        try{
            ...
            List<Map<String, Object>> data = _outerUserSecondaryActiveChannelDiffDao.queryExport(params);
            if(data != null && data.size() > 0){
                ...
                for(Map<String, Object> map : data){
                    ...
                    if("-1".equals(vst_usacd_modulus)) {
                        ...
                        map.put("二次活跃用户", Math.round(vst_usacd_umeng_uv * vst_usacd_amount / vst_usacd_uv * modulus));
                    }
                    map.remove("当日新增用户");
                    map.remove("友盟新增用户");
                    map.remove("调整系数");
    ...
    public ReportBean getReportByDate(Map<String, Object> params) throws SystemException {
        ...
        try{
            ...
            List<Map<String, Object>> data = _outerUserSecondaryActiveChannelDiffDao.queryReportByDate(params);
            bean.setMapData(data);
        }catch(Exception e){
    ...
    public ReportBean getReportByChannel(Map<String, Object> params) throws SystemException {
        ...
        try{
            ...
            List<Map<String, Object>> data = _outerUserSecondaryActiveChannelDiffDao.queryReportByDim(params);
            bean.setMapData(data);
        }catch(Exception e){
