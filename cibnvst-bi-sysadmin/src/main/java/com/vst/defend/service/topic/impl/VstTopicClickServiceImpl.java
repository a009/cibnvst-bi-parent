 ...
 * 
 * @author lhp
 * @date 2017-11-28 下午07:31:07
 * @version
 */
@Service
public class VstTopicClickServiceImpl implements VstTopicClickService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstTopicClickServiceImpl.class);
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			if(params == null){
				...
				// 参数特殊化处理
				if(!ToolsString.isEmpty(params.get("vst_tc_name")+"")){
					VstTools.paramFormat("vst_tc_name", params.get("vst_tc_name")+"", params);
				}
			}
			
			List<Map<String, Object>> data = _vstTopicClickDao.queryExport(params);
			...
			bean.setTitle("vst_topic_click_data");
			...
			logger.error("导出专题点击数据失败: " + SystemException.getExceptionInfo(e));
