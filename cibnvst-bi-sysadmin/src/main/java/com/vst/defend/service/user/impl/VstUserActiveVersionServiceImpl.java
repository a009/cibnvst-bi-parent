...
import com.vst.defend.dao2.user.VstUserActiveVersionDao;
 ...
 * 
 * @author lhp
 * @date 2018-1-4 下午07:51:03
 * @version
 */
@Service
public class VstUserActiveVersionServiceImpl implements VstUserActiveVersionService {
	/**
	 * 写日志
	 */
	 ...
	 */
	@Resource
	private VstUserActiveVersionDao _vstUserActiveVersionDao;
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			if(params == null){
				...
				// 参数特殊化处理
				if(!ToolsString.isEmpty(params.get("vst_uav_ver")+"")){
					VstTools.paramFormat("vst_uav_ver", params.get("vst_uav_ver")+"", params);
				}
			}
			
			List<Map<String, Object>> data = _vstUserActiveVersionDao.queryExport(params);
			...
			bean.setTitle("vst_user_active_version_data");
			...
			logger.error("导出活跃用户(版本)数据失败: " + SystemException.getExceptionInfo(e));
