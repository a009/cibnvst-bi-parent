...
import com.vst.defend.dao2.home.VstHomePositionClickDao;
 ...
 * 
 * @author lhp
 * @date 2018-4-12 下午04:25:36
 * @version
 */
@Service
public class VstHomePositionClickServiceImpl implements VstHomePositionClickService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstHomePositionClickServiceImpl.class);
	
	/**
	 * 首页位置点击Dao接口
	 */
	@Resource
	private VstHomePositionClickDao _vstHomePositionClickDao;
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			...
			List<Map<String, Object>> data = _vstHomePositionClickDao.queryExport(params);
			if(data != null && data.size() > 0){
				...
				// 获取类型
				temp.put("vst_table_name", "vst_home_position_click");
				temp.put("vst_column_name", "vst_hpc_type");
			...
			bean.setTitle("vst_home_position_click_data");
			...
			logger.error("导出首页位置点击数据失败: " + SystemException.getExceptionInfo(e));
