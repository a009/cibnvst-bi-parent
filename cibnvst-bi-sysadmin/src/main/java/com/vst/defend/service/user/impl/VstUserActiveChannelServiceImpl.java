...
import com.vst.defend.dao2.user.VstUserActiveChannelDao;
 ...
 * 
 * @author lhp
 * @date 2018-1-4 下午07:44:43
 * @version
 */
@Service
public class VstUserActiveChannelServiceImpl implements VstUserActiveChannelService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstUserActiveChannelServiceImpl.class);
	
	/**
	 * 活跃用户(渠道)Dao接口
	 */
	@Resource
	private VstUserActiveChannelDao _vstUserActiveChannelDao;
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			...
			List<Map<String, Object>> data = _vstUserActiveChannelDao.queryExport(params);
			...
			bean.setTitle("vst_user_active_channel_data");
			...
			logger.error("导出活跃用户(渠道)数据失败: " + SystemException.getExceptionInfo(e));
	...
	public JSONObject getReportByChannel(Map<String, Object> params) throws SystemException {
		...
		try {
			...
			List<Map<String, Object>> list = _vstUserActiveChannelDao.queryReportByChannel(params);
			
			if(list != null && list.size() > 0){
				// 获取Top10的渠道
				List<String> channels = new ArrayList<String>(10);
				for(Map<String, Object> map : list){
					String vst_uac_channel = map.get("vst_uac_channel")+"";
					if(!channels.contains(vst_uac_channel)){
						channels.add(vst_uac_channel);
					}
					if(channels.size() >= 10){
						break;
					}
				...
				for(int day : VstTools.getDateSection(startDay, endDay)){
					if(!dateMap.containsKey(day+"")){
						...
						for(int i=0; i<channels.size(); i++){
							for(Map<String, Object> map : list){
								int vst_uac_date = ToolsNumber.parseInt(map.get("vst_uac_date")+"");
								String vst_uac_channel = map.get("vst_uac_channel")+"";
								if(channels.get(i).equals(vst_uac_channel) && vst_uac_date == day){
									amountList.set(i, ToolsNumber.parseLong(map.get("vst_uac_amount")+""));
									break;
								}
							}
						}
