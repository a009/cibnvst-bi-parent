...
import com.vst.defend.dao2.user.VstUserSumBdmodelDao;
 ...
 * 
 * @author lhp
 * @date 2019-3-6 上午11:26:09
 * @version
 */
@Service
public class VstUserSumBdmodelServiceImpl implements VstUserSumBdmodelService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstUserSumBdmodelServiceImpl.class);
	
	/**
	 * 累计用户(设备)Dao接口
	 */
	@Resource
	private VstUserSumBdmodelDao _vstUserSumBdmodelDao;
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			if(params == null){
				...
				// 参数特殊化处理
				if(!ToolsString.isEmpty(params.get("vst_usb_bdmodel")+"")){
					VstTools.paramFormat("vst_usb_bdmodel", params.get("vst_usb_bdmodel")+"", params);
				}
			}
			
			List<Map<String, Object>> data = _vstUserSumBdmodelDao.queryExport(params);
			...
			bean.setTitle("vst_user_sum_bdmodel_data");
			...
			logger.error("导出累计用户(设备)数据失败: " + SystemException.getExceptionInfo(e));
