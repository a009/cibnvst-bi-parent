...
import com.vst.defend.dao2.user.VstUserActiveDao;
 ...
 * 
 * @author lhp
 * @date 2018-1-3 下午03:48:04
 * @version
 */
@Service
public class VstUserActiveServiceImpl implements VstUserActiveService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstUserActiveServiceImpl.class);
	
	/**
	 * 活跃用户Dao接口
	 */
	@Resource
	private VstUserActiveDao _vstUserActiveDao;
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			...
			List<Map<String, Object>> data = _vstUserActiveDao.queryExport(params);
			...
			bean.setTitle("vst_user_active_data");
			...
			logger.error("导出活跃用户数据失败: " + SystemException.getExceptionInfo(e));
