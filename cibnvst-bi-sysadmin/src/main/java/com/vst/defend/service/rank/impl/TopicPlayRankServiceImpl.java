 ...
 * 
 * @author lhp
 * @date 2018-2-1 下午04:25:11
 * @version
 */
@Service
public class TopicPlayRankServiceImpl implements TopicPlayRankService {
	/**
	 * 日志
	 */
	private static final Log logger = LogFactory.getLog(TopicPlayRankServiceImpl.class);
	...
	public ReportBean getCutPageData(CutPage cutPage) throws SystemException {
		...
		try{
			...
			int count = _vstTopicPlayDao.queryTopCount(params);
			...
			if(count != 0){
				...
				List<Map<String, Object>> list = _vstTopicPlayDao.queryTopList(params);
			...
			logger.error("查询专题播放排行分页数据失败: " + SystemException.getExceptionInfo(e));
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			...
			List<Map<String, Object>> data = _vstTopicPlayDao.queryTopExport(params);
			if(data != null && data.size() > 0){
				...
				for(Map<String, Object> map : data){
					...
					map.put("总播放时长(H)", VstTools.toHour(ToolsNumber.parseLong(map.get("总播放时长(H)")+"")*1000));
				}
			}
			
			bean.setMapData(data);
			bean.setTitle("vst_topic_play_rank_data");
			...
			logger.error("导出专题播放排行数据失败: " + SystemException.getExceptionInfo(e));
			throw new SystemException(ErrorCode.SERVICE_OPERATOR_FAILURE, e);
		}
		return bean;
	}
