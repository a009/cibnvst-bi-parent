...
import com.vst.defend.dao2.home.VstHomeEntryClickDao;
 ...
 */
@Service
public class VstHomeEntryClickServiceImpl implements VstHomeEntryClickService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstHomeEntryClickServiceImpl.class);
	
	/**
	 * 首页入口点击Dao接口
	 */
	@Resource
	private VstHomeEntryClickDao _vstHomeEntryClickDao;
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			...
			List<Map<String, Object>> data = _vstHomeEntryClickDao.queryExport(params);
			if(data != null && data.size() > 0){
				...
				// 获取类型
				temp.put("vst_table_name", "vst_home_entry_click");
				temp.put("vst_column_name", "vst_hec_type");
			...
			bean.setTitle("vst_home_entry_click_data");
			...
			logger.error("导出首页入口点击数据失败: " + SystemException.getExceptionInfo(e));
