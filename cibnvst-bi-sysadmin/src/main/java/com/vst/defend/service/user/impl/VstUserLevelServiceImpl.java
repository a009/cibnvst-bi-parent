...
import com.vst.defend.dao2.user.VstUserLevelDao;
 ...
 * 
 * @author lhp
 * @date 2018-1-4 下午02:30:06
 * @version
 */
@Service
public class VstUserLevelServiceImpl implements VstUserLevelService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstUserLevelServiceImpl.class);
	
	/**
	 * 季质量用户Dao接口
	 */
	@Resource
	private VstUserLevelDao _vstUserLevelDao;
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			...
			List<Map<String, Object>> data = _vstUserLevelDao.queryExport(params);
			...
			bean.setTitle("vst_user_level_data");
			...
			logger.error("导出季质量用户数据失败: " + SystemException.getExceptionInfo(e));
