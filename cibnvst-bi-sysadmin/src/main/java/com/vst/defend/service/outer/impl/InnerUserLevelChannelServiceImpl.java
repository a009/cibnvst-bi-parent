...
import com.vst.defend.dao3.inner.InnerUserLevelChannelDao;
import com.vst.defend.service.outer.InnerUserLevelChannelService;
 ...
 * 
 * @author lhp
 * @date 2018-1-24 上午10:52:36
 * @version
 */
@Service
public class InnerUserLevelChannelServiceImpl implements InnerUserLevelChannelService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(InnerUserLevelChannelServiceImpl.class);
	
	/**
	 * 季质量渠道用户(对内)Dao接口
	 */
	@Resource
	private InnerUserLevelChannelDao _innerUserLevelChannelDao;
	...
	public ReportBean getCutPageData(CutPage cutPage) throws SystemException {
		...
		try{
			...
			int count = _innerUserLevelChannelDao.queryCount(params);
			...
			if(count != 0){
				...
				List<Map<String, Object>> list = _innerUserLevelChannelDao.queryForList(params);
				
				for(Map<String, Object> map : list){
					// 转换日期格式
					map.put("vst_ulc_addtime", ToolsDate.formatDate(ToolsDate.yyyy_MM_dd_HH_mm_ss, ToolsNumber.parseLong(map.get("vst_ulc_addtime")+"")));
					map.put("vst_ulc_uptime", ToolsDate.formatDate(ToolsDate.yyyy_MM_dd_HH_mm_ss, ToolsNumber.parseLong(map.get("vst_ulc_uptime")+"")));
				}
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			...
			List<Map<String, Object>> data = _innerUserLevelChannelDao.queryExport(params);
	...
	public ReportBean getReportByDate(Map<String, Object> params) throws SystemException {
		...
		try{
			...
			List<Map<String, Object>> data = _innerUserLevelChannelDao.queryReportByDate(params);
			bean.setMapData(data);
		}catch(Exception e){
			logger.error("查询季质量渠道用户(按日期统计)失败: " + SystemException.getExceptionInfo(e));
			throw new SystemException(ErrorCode.SERVICE_OPERATOR_FAILURE, e);
		}
		return bean;
	}
	...
	public int auditData(String ids, int state, UserSession user) throws SystemException {
		...
		try{
			...
			params.put("vst_ulc_state", state);
			params.put("vst_ulc_uptime", System.currentTimeMillis());
			params.put("vst_ulc_updator", user.getLoginInfo().getLoginName());
			result = _innerUserLevelChannelDao.modifyState(params);
			
			if(result > 0){
				// 写操作日志
				StringBuilder sb = new StringBuilder();
				sb.append("审核季质量渠道用户数据，ids=").append(ids);
				_baseService.saveLog(user, new VstSysOperateLog(user.getCurrModuleId(), VstConstants.OPERATE_TYPE_UPDATE, sb.toString()));
			}
		}catch(Exception e){
			logger.error("审核季质量渠道用户数据失败: " + SystemException.getExceptionInfo(e));
			throw new SystemException(ErrorCode.SERVICE_OPERATOR_FAILURE, e);
		}
		return result;
	}
	...
	public int batchAuditData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try {
			if(params != null){
				...
				result = _innerUserLevelChannelDao.auditData(params);
				if(result > 0){
					// 写操作日志
					StringBuilder sb = new StringBuilder();
	...
	public int modifyModulus(String ids, String modulus, UserSession user) {
		...
		try {
			Map<String, Object> params = new HashMap<String, Object>(4);
			params.put("list_ids", Arrays.asList(ids.split(",")));
			params.put("vst_ulc_modulus", modulus);
			params.put("vst_ulc_uptime", System.currentTimeMillis());
			params.put("vst_ulc_updator", user.getLoginInfo().getLoginName());
			result = _innerUserLevelChannelDao.modifyState(params);
	...
	public int importData(String date, UserSession user) throws SystemException {
		...
		try {
			if(!ToolsString.isEmpty(date)){
				...
				_innerUserLevelChannelDao.delete(params);
				
				// 将原表数据导入
				List<Map<String, Object>> srcList = _vstUserLevelChannelDao.queryForList(params);
				if(srcList != null && srcList.size() > 0){
					...
					List<OuterVstUserLevelChannel> beanList = new ArrayList<OuterVstUserLevelChannel>();
					for(Map<String, Object> map : srcList){
						OuterVstUserLevelChannel bean = new OuterVstUserLevelChannel();
						bean = (OuterVstUserLevelChannel) BeanUtils.mapToBean(map, bean.getClass());
						// 设置调整系数
						String channel = bean.getVst_ulc_channel();
						if(channelRate.containsKey(channel)){
							bean.setVst_ulc_modulus(channelRate.getString(channel));
						}else{
							bean.setVst_ulc_modulus(channelRate.getString("0"));
						}
						// 设置状态
						bean.setVst_ulc_state(1);
						// 设置同步时间
						bean.setVst_ulc_synctime(System.currentTimeMillis());
						
						beanList.add(bean);
						//_innerUserLevelChannelDao.insert(bean);
						//result ++;
					}
					result = _innerUserLevelChannelDao.batchInsert(beanList);
	...
	public int deleteData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try {
			if(params != null){
				...
				result = _innerUserLevelChannelDao.delete(params);
				if(result > 0){
					// 写操作日志
					StringBuilder sb = new StringBuilder();
					sb.append("删除季质量渠道用户, 条件=").append(params);
					_baseService.saveLog(user, new VstSysOperateLog(user.getCurrModuleId(), VstConstants.OPERATE_TYPE_DEL, sb.toString()));
				}
			}
		} catch (Exception e) {
			logger.error("删除季质量渠道用户失败: " + SystemException.getExceptionInfo(e));
			throw new SystemException(ErrorCode.SERVICE_OPERATOR_FAILURE, e);
		}
		return result;
	}
	...
	public int updateData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try {
			if(params != null){
				OuterVstUserLevelChannel bean = new OuterVstUserLevelChannel();
				bean.setVst_ulc_id(ToolsString.checkEmpty(params.get("vst_ulc_id")));
				bean.setVst_ulc_add_amount(ToolsNumber.parseLong(params.get("vst_ulc_add_amount")+""));
				bean.setVst_ulc_season_amount(ToolsNumber.parseLong(params.get("vst_ulc_season_amount")+""));
				bean.setVst_ulc_modulus(ToolsString.checkEmpty(params.get("vst_ulc_modulus")));
				bean.setVst_ulc_uptime(System.currentTimeMillis());
				bean.setVst_ulc_updator(user.getLoginInfo().getLoginName());

				result = _innerUserLevelChannelDao.update(bean);
				if(result > 0){
					// 写操作日志
					StringBuilder sb = new StringBuilder();
					sb.append("修改季质量渠道用户, 参数=").append(params);
					_baseService.saveLog(user, new VstSysOperateLog(user.getCurrModuleId(), VstConstants.OPERATE_TYPE_UPDATE, sb.toString()));
				}
			}
		} catch (Exception e) {
			logger.error("修改季质量渠道用户失败: " + SystemException.getExceptionInfo(e));
			throw new SystemException(ErrorCode.SERVICE_OPERATOR_FAILURE, e);
		}
		return result;
	}
	...
	public ReportBean getReportByDim(Map<String, Object> params) throws SystemException {
		...
		try{
			...
			List<Map<String, Object>> data = _innerUserLevelChannelDao.queryReportByDim(params);
			bean.setMapData(data);
		}catch(Exception e){
			logger.error("查询季质量渠道用户(按维度统计)失败: " + SystemException.getExceptionInfo(e));
			throw new SystemException(ErrorCode.SERVICE_OPERATOR_FAILURE, e);
		}
		return bean;
	}
