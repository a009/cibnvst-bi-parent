...
import com.vst.defend.dao2.home.VstHomeContentClickDao;
 ...
 * 
 * @author lhp
 * @date 2018-4-10 下午04:51:22
 * @version
 */
@Service
public class VstHomeContentClickServiceImpl implements VstHomeContentClickService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstHomeContentClickServiceImpl.class);
	
	/**
	 * 首页内容点击Dao接口
	 */
	@Resource
	private VstHomeContentClickDao _vstHomeContentClickDao;
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			...
			List<Map<String, Object>> data = _vstHomeContentClickDao.queryExport(params);
			if(data != null && data.size() > 0){
				...
				// 获取类型
				temp.put("vst_table_name", "vst_home_content_click");
				temp.put("vst_column_name", "vst_hcc_type");
				Map<String, String> typeMap = _baseService.getDictionaryForMap(temp);
				// 获取影片分类
				temp.put("vst_table_name", "vst_home_content_click");
				temp.put("vst_column_name", "vst_hcc_cid");
			...
			bean.setTitle("vst_home_content_click_data");
			...
			logger.error("导出首页内容点击数据失败: " + SystemException.getExceptionInfo(e));
