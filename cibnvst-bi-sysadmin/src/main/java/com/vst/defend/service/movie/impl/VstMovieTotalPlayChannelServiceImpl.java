...
import com.vst.defend.dao2.movie.VstMovieTotalPlayChannelDao;
 ...
 * 
 * @author lhp
 * @date 2018-2-24 下午06:08:50
 * @version
 */
@Service
public class VstMovieTotalPlayChannelServiceImpl implements VstMovieTotalPlayChannelService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstMovieTotalPlayChannelServiceImpl.class);
	
	/**
	 * 汇总播放(渠道)Dao接口
	 */
	@Resource
	private VstMovieTotalPlayChannelDao _vstMovieTotalPlayChannelDao;
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			if(params == null){
				...
				// 参数特殊化处理
				if(!ToolsString.isEmpty(params.get("vst_mtpc_channel")+"")){
					VstTools.paramFormat("vst_mtpc_channel", params.get("vst_mtpc_channel")+"", params);
				}
			}
			
			List<Map<String, Object>> data = _vstMovieTotalPlayChannelDao.queryExport(params);
			...
			bean.setTitle("vst_movie_total_play_channel_data");
			...
			logger.error("导出汇总播放(渠道)数据失败: " + SystemException.getExceptionInfo(e));
