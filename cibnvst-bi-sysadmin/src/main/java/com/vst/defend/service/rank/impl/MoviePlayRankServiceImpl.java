 ...
 * 
 * @author lhp
 * @date 2018-2-1 下午02:51:32
 * @version
 */
@Service
public class MoviePlayRankServiceImpl implements MoviePlayRankService {
	/**
	 * 日志
	 */
	private static final Log logger = LogFactory.getLog(MoviePlayRankServiceImpl.class);
	...
	public ReportBean getCutPageData(CutPage cutPage) throws SystemException {
		...
		try{
			...
			if(count != 0){
				...
				List<Map<String, Object>> list = _vstMoviePlayDao.queryTopList(params);
			...
			logger.error("查询影片播放排行分页数据失败: " + SystemException.getExceptionInfo(e));
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			...
			List<Map<String, Object>> data = _vstMoviePlayDao.queryTopExport(params);
			...
			bean.setTitle("vst_movie_play_rank_data");
			...
			logger.error("导出影片播放排行数据失败: " + SystemException.getExceptionInfo(e));
			throw new SystemException(ErrorCode.SERVICE_OPERATOR_FAILURE, e);
		}
		return bean;
	}
