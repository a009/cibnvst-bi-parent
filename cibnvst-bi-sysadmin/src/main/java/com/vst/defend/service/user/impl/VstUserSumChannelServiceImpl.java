...
import com.vst.defend.dao2.user.VstUserSumChannelDao;
 ...
 * 
 * @author lhp
 * @date 2018-1-4 下午03:35:50
 * @version
 */
@Service
public class VstUserSumChannelServiceImpl implements VstUserSumChannelService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstUserSumChannelServiceImpl.class);
	
	/**
	 * 累计用户(渠道)Dao接口
	 */
	@Resource
	private VstUserSumChannelDao _vstUserSumChannelDao;
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			if(params == null){
				...
				// 参数特殊化处理
				if(!ToolsString.isEmpty(params.get("vst_usc_channel")+"")){
					VstTools.paramFormat("vst_usc_channel", params.get("vst_usc_channel")+"", params);
				}
			}
			
			List<Map<String, Object>> data = _vstUserSumChannelDao.queryExport(params);
			...
			bean.setTitle("vst_user_sum_channel_data");
			...
			logger.error("导出累计用户(渠道)数据失败: " + SystemException.getExceptionInfo(e));
	...
	public JSONObject getReportByChannel(Map<String, Object> params) throws SystemException {
		...
		try {
			...
			List<Map<String, Object>> list = _vstUserSumChannelDao.queryReportByChannel(params);
			
			if(list != null && list.size() > 0){
				// 获取Top10的渠道
				List<String> channels = new ArrayList<String>(10);
				for(Map<String, Object> map : list){
					String vst_usc_channel = map.get("vst_usc_channel")+"";
					if(!channels.contains(vst_usc_channel)){
						channels.add(vst_usc_channel);
					}
					if(channels.size() >= 10){
						break;
					}
				...
				for(int day : VstTools.getDateSection(startDay, endDay)){
					if(!dateMap.containsKey(day+"")){
						...
						for(int i=0; i<channels.size(); i++){
							for(Map<String, Object> map : list){
								int vst_usc_date = ToolsNumber.parseInt(map.get("vst_usc_date")+"");
								String vst_usc_channel = map.get("vst_usc_channel")+"";
								if(channels.get(i).equals(vst_usc_channel) && vst_usc_date == day){
									amountList.set(i, ToolsNumber.parseLong(map.get("vst_usc_amount")+""));
									break;
								}
							}
						}
