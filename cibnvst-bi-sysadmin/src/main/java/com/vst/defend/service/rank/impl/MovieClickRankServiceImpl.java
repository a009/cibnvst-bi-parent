 ...
 * 
 * @author lhp
 * @date 2018-2-1 上午11:17:34
 * @version
 */
@Service
public class MovieClickRankServiceImpl implements MovieClickRankService {
	/**
	 * 日志
	 */
	private static final Log logger = LogFactory.getLog(MovieClickRankServiceImpl.class);
	...
	public ReportBean getCutPageData(CutPage cutPage) throws SystemException {
		...
		try{
			...
			if(count != 0){
				...
				List<Map<String, Object>> list = _vstMovieClickDao.queryTopList(params);
			...
			logger.error("查询影片点击排行分页数据失败: " + SystemException.getExceptionInfo(e));
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			...
			List<Map<String, Object>> data = _vstMovieClickDao.queryTopExport(params);
			...
			bean.setTitle("vst_movie_click_rank_data");
			...
			logger.error("导出影片点击排行数据失败: " + SystemException.getExceptionInfo(e));
			throw new SystemException(ErrorCode.SERVICE_OPERATOR_FAILURE, e);
		}
		return bean;
	}
