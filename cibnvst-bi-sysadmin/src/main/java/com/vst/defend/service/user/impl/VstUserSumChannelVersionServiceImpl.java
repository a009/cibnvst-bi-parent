...
import com.vst.defend.dao2.user.VstUserSumChannelVersionDao;
 ...
 * 
 * @author lhp
 * @date 2018-1-4 下午03:37:31
 * @version
 */
@Service
public class VstUserSumChannelVersionServiceImpl implements VstUserSumChannelVersionService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstUserSumChannelVersionServiceImpl.class);
	
	/**
	 * 累计用户(渠道、版本)Dao接口
	 */
	@Resource
	private VstUserSumChannelVersionDao _vstUserSumChannelVersionDao;
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			if(params == null){
				...
				// 参数特殊化处理
				if(!ToolsString.isEmpty(params.get("vst_uscv_channel")+"")){
					VstTools.paramFormat("vst_uscv_channel", params.get("vst_uscv_channel")+"", params);
				}
				if(!ToolsString.isEmpty(params.get("vst_uscv_version")+"")){
					VstTools.paramFormat("vst_uscv_version", params.get("vst_uscv_version")+"", params);
				}
			}
			
			List<Map<String, Object>> data = _vstUserSumChannelVersionDao.queryExport(params);
			...
			bean.setTitle("vst_user_sum_channel_version_data");
			...
			logger.error("导出累计用户(渠道、版本)数据失败: " + SystemException.getExceptionInfo(e));
	...
	public JSONObject getReportByVersion(Map<String, Object> params) throws SystemException {
		...
		try {
			...
			List<Map<String, Object>> list = _vstUserSumChannelVersionDao.queryReportByVersion(params);
			
			if(list != null && list.size() > 0){
				// 获取Top10的版本号
				List<String> versions = new ArrayList<String>(10);
				for(Map<String, Object> map : list){
					String vst_uscv_version = map.get("vst_uscv_version")+"";
					if(!versions.contains(vst_uscv_version)){
						versions.add(vst_uscv_version);
					}
					if(versions.size() >= 10){
						break;
					}
				...
				for(int day : VstTools.getDateSection(startDay, endDay)){
					if(!dateMap.containsKey(day+"")){
						...
						for(int i=0; i<versions.size(); i++){
							for(Map<String, Object> map : list){
								int vst_uscv_date = ToolsNumber.parseInt(map.get("vst_uscv_date")+"");
								String vst_uscv_version = map.get("vst_uscv_version")+"";
								if(versions.get(i).equals(vst_uscv_version) && vst_uscv_date == day){
									amountList.set(i, ToolsNumber.parseLong(map.get("vst_uscv_amount")+""));
									break;
								}
							}
						}
