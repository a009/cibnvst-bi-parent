...
import com.vst.defend.dao2.movie.VstMovieExposureDao;
 ...
 */
@Service
public class VstMovieExposureServiceImpl implements VstMovieExposureService {
    /**
     * 写日志
     */
    private static final Log logger = LogFactory.getLog(VstMovieExposureServiceImpl.class);

    /**
     * 影片曝光Dao接口
     */
    @Resource
    private VstMovieExposureDao _vstMovieExposureDao;
    ...
    public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
        ...
        try{
            if(params == null){
                ...
                // 参数特殊化处理
                if(!ToolsString.isEmpty(params.get("vst_me_name")+"")){
                    VstTools.paramFormat("vst_me_name", params.get("vst_me_name")+"", params);
                }
            }

            List<Map<String, Object>> data = _vstMovieExposureDao.queryExport(params);
            ...
            bean.setTitle("vst_movie_exposure_data");
            ...
            logger.error("导出影片曝光数据失败: " + SystemException.getExceptionInfo(e));
