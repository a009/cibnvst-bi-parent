...
import com.vst.defend.dao2.user.VstUserActiveBdmodelDao;
 ...
 * 
 * @author lhp
 * @date 2019-3-6 上午10:02:01
 * @version
 */
@Service
public class VstUserActiveBdmodelServiceImpl implements VstUserActiveBdmodelService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstUserActiveBdmodelServiceImpl.class);
	
	/**
	 * 活跃用户(设备)Dao接口
	 */
	@Resource
	private VstUserActiveBdmodelDao _vstUserActiveBdmodelDao;
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			if(params == null){
				...
				// 参数特殊化处理
				if(!ToolsString.isEmpty(params.get("vst_uab_bdmodel")+"")){
					VstTools.paramFormat("vst_uab_bdmodel", params.get("vst_uab_bdmodel")+"", params);
				}
			}
			
			List<Map<String, Object>> data = _vstUserActiveBdmodelDao.queryExport(params);
			...
			bean.setTitle("vst_user_active_bdmodel_data");
			...
			logger.error("导出活跃用户(设备)数据失败: " + SystemException.getExceptionInfo(e));
