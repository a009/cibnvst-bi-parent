...
import com.vst.defend.dao2.user.VstUserSumUpVerDao;
 ...
 * 
 * @author lhp
 * @date 2018-1-4 下午03:42:01
 * @version
 */
@Service
public class VstUserSumUpVerServiceImpl implements VstUserSumUpVerService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstUserSumUpVerServiceImpl.class);
	
	/**
	 * 累计用户(升级版本)Dao接口
	 */
	@Resource
	private VstUserSumUpVerDao _vstUserSumUpVerDao;
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			if(params == null){
				...
				// 参数特殊化处理
				if(!ToolsString.isEmpty(params.get("vst_usu_verNum")+"")){
					VstTools.paramFormat("vst_usu_verNum", params.get("vst_usu_verNum")+"", params);
				}
			}
			
			List<Map<String, Object>> data = _vstUserSumUpVerDao.queryExport(params);
			...
			bean.setTitle("vst_user_sum_upVer_data");
			...
			logger.error("导出累计用户(升级版本)数据失败: " + SystemException.getExceptionInfo(e));
