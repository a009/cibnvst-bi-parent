 ...
 * 
 * @author lhp
 * @date 2018-1-29 下午06:44:22
 * @version
 */
@Service
public class VstMovieSearchServiceImpl implements VstMovieSearchService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstMovieSearchServiceImpl.class);
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			if(params == null){
				...
				// 参数特殊化处理
				if(!ToolsString.isEmpty(params.get("vst_mos_name")+"")){
					VstTools.paramFormat("vst_mos_name", params.get("vst_mos_name")+"", params);
				}
			}
			
			List<Map<String, Object>> data = _vstMovieSearchDao.queryExport(params);
			...
			bean.setTitle("vst_movie_search_data");
			...
			logger.error("导出影片搜索数据失败: " + SystemException.getExceptionInfo(e));
