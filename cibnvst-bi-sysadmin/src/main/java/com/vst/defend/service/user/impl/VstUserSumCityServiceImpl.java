...
import com.vst.defend.dao2.user.VstUserSumCityDao;
 ...
 * 
 * @author lhp
 * @date 2018-1-4 下午03:39:23
 * @version
 */
@Service
public class VstUserSumCityServiceImpl implements VstUserSumCityService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstUserSumCityServiceImpl.class);
	
	/**
	 * 累计用户(城市)Dao接口
	 */
	@Resource
	private VstUserSumCityDao _vstUserSumCityDao;
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			if(params == null){
				...
				// 参数特殊化处理
				if(!ToolsString.isEmpty(params.get("vst_usci_city")+"")){
					VstTools.paramFormat("vst_usci_city", params.get("vst_usci_city")+"", params);
				}
			}
			
			List<Map<String, Object>> data = _vstUserSumCityDao.queryExport(params);
			...
			bean.setTitle("vst_user_sum_city_data");
			...
			logger.error("导出累计用户(城市)数据失败: " + SystemException.getExceptionInfo(e));
	...
	public ReportBean getReportByCity(Map<String, Object> params) throws SystemException {
		...
		try {
			...
			List<Map<String, Object>> list = _vstUserSumCityDao.queryReportByDim(params);
			if(list != null && list.size() > 0){
				...
				for(Map<String, Object> map : list){
					String city = map.get("vst_usci_city")+"";
					if(cityMap.containsKey(city)){
						JSONArray value = new JSONArray();
						value.addAll(cityMap.get(city));
						value.add(map.get("vst_usci_amount"));
