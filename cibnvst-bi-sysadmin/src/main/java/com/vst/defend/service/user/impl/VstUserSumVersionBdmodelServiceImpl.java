...
import com.vst.defend.dao2.user.VstUserSumVersionBdmodelDao;
 ...
 * 
 * @author lhp
 * @date 2019-3-12 下午03:48:52
 * @version
 */
@Service
public class VstUserSumVersionBdmodelServiceImpl implements VstUserSumVersionBdmodelService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstUserSumVersionBdmodelServiceImpl.class);
	
	/**
	 * 累计用户(版本、设备)Dao接口
	 */
	@Resource
	private VstUserSumVersionBdmodelDao _vstUserSumVersionBdmodelDao;
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			if(params == null){
				...
				// 参数特殊化处理
				if(!ToolsString.isEmpty(params.get("vst_usvb_version")+"")){
					VstTools.paramFormat("vst_usvb_version", params.get("vst_usvb_version")+"", params);
				}
				if(!ToolsString.isEmpty(params.get("vst_usvb_bdmodel")+"")){
					VstTools.paramFormat("vst_usvb_bdmodel", params.get("vst_usvb_bdmodel")+"", params);
				}
			}
			
			List<Map<String, Object>> data = _vstUserSumVersionBdmodelDao.queryExport(params);
			...
			bean.setTitle("vst_user_sum_version_bdmodel_data");
			...
			logger.error("导出累计用户(版本、设备)数据失败: " + SystemException.getExceptionInfo(e));
	...
	public JSONObject getReportByBdmodel(Map<String, Object> params) throws SystemException {
		...
		try {
			...
			List<Map<String, Object>> list = _vstUserSumVersionBdmodelDao.queryReportByBdmodel(params);
			
			if(list != null && list.size() > 0){
				// 获取Top10的设备
				List<String> bdmodels = new ArrayList<String>(10);
				for(Map<String, Object> map : list){
					String vst_usvb_bdmodel = map.get("vst_usvb_bdmodel")+"";
					if(!bdmodels.contains(vst_usvb_bdmodel)){
						bdmodels.add(vst_usvb_bdmodel);
					}
					if(bdmodels.size() >= 10){
						break;
					}
				...
				for(int day : VstTools.getDateSection(startDay, endDay)){
					if(!dateMap.containsKey(day+"")){
						...
						for(int i=0; i<bdmodels.size(); i++){
							for(Map<String, Object> map : list){
								int vst_usvb_date = ToolsNumber.parseInt(map.get("vst_usvb_date")+"");
								String vst_usvb_bdmodel = map.get("vst_usvb_bdmodel")+"";
								if(bdmodels.get(i).equals(vst_usvb_bdmodel) && vst_usvb_date == day){
									amountList.set(i, ToolsNumber.parseLong(map.get("vst_usvb_amount")+""));
									break;
								}
							}
						}
