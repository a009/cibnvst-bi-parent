...
import com.vst.defend.dao2.user.VstUserSumVersionUpVerDao;
 ...
 * 
 * @author lhp
 * @date 2018-1-4 下午03:45:43
 * @version
 */
@Service
public class VstUserSumVersionUpVerServiceImpl implements VstUserSumVersionUpVerService {
	/**
	 * 写日志
	 */
	private static final Log logger = LogFactory.getLog(VstUserSumVersionUpVerServiceImpl.class);
	
	/**
	 * 累计用户(显示版本、升级版本)Dao接口
	 */
	@Resource
	private VstUserSumVersionUpVerDao _vstUserSumVersionUpVerDao;
	...
	public ReportBean getExportData(Map<String, Object> params, UserSession user) throws SystemException {
		...
		try{
			if(params == null){
				...
				// 参数特殊化处理
				if(!ToolsString.isEmpty(params.get("vst_usvu_version")+"")){
					VstTools.paramFormat("vst_usvu_version", params.get("vst_usvu_version")+"", params);
				}
				if(!ToolsString.isEmpty(params.get("vst_usvu_upVer")+"")){
					VstTools.paramFormat("vst_usvu_upVer", params.get("vst_usvu_upVer")+"", params);
				}
			}
			
			List<Map<String, Object>> data = _vstUserSumVersionUpVerDao.queryExport(params);
			...
			bean.setTitle("vst_user_sum_version_upVer_data");
			...
			logger.error("导出累计用户(显示版本、升级版本)数据失败: " + SystemException.getExceptionInfo(e));
	...
	public JSONObject getReportByUpVer(Map<String, Object> params) throws SystemException {
		...
		try {
			...
			List<Map<String, Object>> list = _vstUserSumVersionUpVerDao.queryReportByUpVer(params);
			
			if(list != null && list.size() > 0){
				// 获取Top10的升级版本
				List<String> upVers = new ArrayList<String>(10);
				for(Map<String, Object> map : list){
					String vst_usvu_upVer = map.get("vst_usvu_upVer")+"";
					if(!upVers.contains(vst_usvu_upVer)){
						upVers.add(vst_usvu_upVer);
					}
					if(upVers.size() >= 10){
						break;
					}
				...
				for(int day : VstTools.getDateSection(startDay, endDay)){
					if(!dateMap.containsKey(day+"")){
						...
						for(int i=0; i<upVers.size(); i++){
							for(Map<String, Object> map : list){
								int vst_usvu_date = ToolsNumber.parseInt(map.get("vst_usvu_date")+"");
								String vst_usvu_upVer = map.get("vst_usvu_upVer")+"";
								if(upVers.get(i).equals(vst_usvu_upVer) && vst_usvu_date == day){
									amountList.set(i, ToolsNumber.parseLong(map.get("vst_usvu_amount")+""));
									break;
								}
							}
						}
